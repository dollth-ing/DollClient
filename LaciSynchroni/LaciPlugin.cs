...
public class LaciPlugin : MediatorSubscriberBase, IHostedService
{
        ...
        Mediator.Publish(
            new EventMessage(
                new Services.Events.Event(
                    ...
                    $"Starting Laci Synchroni {version.ToString()}"
                )
            )
        );

        Mediator.Subscribe<SwitchToMainUiMessage>(
            this,
            (msg) =>
            {
    ...
    public Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        Logger.LogDebug("Halting LaciPlugin");
    ...
    private async Task WaitForPlayerAndLaunchCharacterManager()
    {
        ...
        try
        {
            ...
            if (
                !_syncConfigService.Current.HasValidSetup()
            ...
            if (_syncConfigService.Current.LogLevel != LogLevel.Information)
            {
                Mediator.Publish(
                    new NotificationMessage(
                        "Abnormal Log Level",
                        $"Your log level is set to '{_syncConfigService.Current.LogLevel}' which is not recommended for normal usage. Set it to '{LogLevel.Information}' in \"Laci Settings -> Debug\" unless instructed otherwise.",
