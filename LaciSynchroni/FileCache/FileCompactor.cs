...
public sealed class FileCompactor
{
    ...
    public long GetFileSizeOnDisk(FileInfo fileInfo, bool? isNTFS = null)
    {
        bool ntfs =
            isNTFS
            ?? string.Equals(
                new DriveInfo(fileInfo.Directory!.Root.FullName).DriveFormat,
                "NTFS",
    ...
    private static extern int WofIsExternalFile(
        ...
        out uint Provider,
        out WOF_FILE_COMPRESSION_INFO_V1 Info,
    ...
    private void CompactFile(string filePath)
    {
        ...
        {
            _logger.LogDebug(
                "File {file} is smaller than cluster size ({size}), ignoring",
                filePath,
                clusterSize
            );
            return;
        ...
        if (!IsCompactedFile(filePath))
        {
            ...
            _logger.LogDebug(
                "Compressed {file} from {orig}b to {comp}b",
                filePath,
                oldSize,
                newSize
            );
...
#pragma warning restore S3869 // "SafeHandle.DangerousGetHandle" should not be called
    ...
    private int GetClusterSize(FileInfo fi)
    {
        ...
        _logger.LogDebug(
            "Determined Cluster Size for root {root}: {cluster}",
            root,
            _clusterSizes[root]
        );
