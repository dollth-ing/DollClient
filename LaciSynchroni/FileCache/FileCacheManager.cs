...
public sealed class FileCacheManager : IHostedService
{
    ...
    private readonly ConcurrentDictionary<string, List<FileCacheEntity>> _fileCaches = new(
    ...
    public FileCacheEntity? CreateCacheEntry(string path)
    {
        ...
        if (
            !fullName.Contains(
                _configService.Current.CacheFolder.ToLowerInvariant(),
    ...
    public List<FileCacheEntity> GetAllFileCachesByHash(
        string hash,
        bool ignoreCacheEntries = false,
        bool validate = true
    )
    {
        ...
        {
            foreach (
                var fileCache in fileCacheEntities
                    .Where(c => ignoreCacheEntries ? !c.IsCacheEntry : true)
                    .ToList()
            )
            {
                if (!validate)
    ...
    public Task<List<FileCacheEntity>> ValidateLocalIntegrity(
        IProgress<(int, int, FileCacheEntity)> progress,
    ...
    )
    {
        ...
        foreach (var fileCache in cacheEntries)
        {
            ...
            try
            {
                ...
                if (!string.Equals(computedHash, fileCache.Hash, StringComparison.Ordinal))
                {
                    _logger.LogInformation(
                        "Failed to validate {file}, got hash {hash}, expected hash {hash}",
            ...
            catch (Exception e)
            {
                _logger.LogWarning(
                    e,
                    "Error during validation of {file}",
                    fileCache.ResolvedFilepath
                );
                brokenEntities.Add(fileCache);
            }
    ...
    public async Task<(string, byte[])> GetCompressedFileData(
        string fileHash,
    ...
    )
    {
        ...
        return (
            fileHash,
            LZ4Wrapper.WrapHC(
                await File.ReadAllBytesAsync(fileCache, uploadToken).ConfigureAwait(false),
                0,
    ...
    public FileCacheEntity? GetFileCacheByHash(string hash)
    {
        ...
        {
            var item = hashes
                .OrderBy(p => p.PrefixedFilePath.Contains(PenumbraPrefix) ? 0 : 1)
                .FirstOrDefault();
            if (item != null)
    ...
    {
        var cleanedPath = path.Replace("/", "\\", StringComparison.OrdinalIgnoreCase)
            .ToLowerInvariant()
            .Replace(
                _ipcManager.Penumbra.ModDirectory!.ToLowerInvariant(),
                "",
        ...
        var entry = _fileCaches
            ...
            .FirstOrDefault(f =>
                f.ResolvedFilepath.EndsWith(cleanedPath, StringComparison.OrdinalIgnoreCase)
            );
        ...
        try
        {
            var cleanedPaths = paths
                ...
                .ToDictionary(
                    p => p,
                    p =>
                        p.Replace("/", "\\", StringComparison.OrdinalIgnoreCase)
                            .Replace(
    ...
    public void RemoveHashedFile(string hash, string prefixedFilePath)
    {
        if (_fileCaches.TryGetValue(hash, out var caches))
        {
            var removedCount = caches?.RemoveAll(c =>
                string.Equals(c.PrefixedFilePath, prefixedFilePath, StringComparison.Ordinal)
            );
            _logger.LogTrace(
    ...
    public void UpdateHashedFile(FileCacheEntity fileCache, bool computeProperties = true)
    {
        ...
        if (computeProperties)
        {
            ...
            fileCache.LastModifiedDateTicks = fi.LastWriteTimeUtc.Ticks.ToString(
    ...
    public (FileState State, FileCacheEntity FileCache) ValidateFileCacheEntity(
        FileCacheEntity fileCache
    )
    {
    ...
    internal FileCacheEntity MigrateFileHashToExtension(FileCacheEntity fileCache, string ext)
    {
        try
        {
            ...
            var extensionPath =
                fileCache.ResolvedFilepath.ToUpper(CultureInfo.InvariantCulture) + "." + ext;
            ...
            _logger.LogTrace(
                "Migrated from {oldPath} to {newPath}",
                fileCache.ResolvedFilepath,
                newHashedEntity.ResolvedFilepath
            );
    ...
    private FileCacheEntity? CreateFileCacheEntity(
        FileInfo fileInfo,
        string prefixedPath,
        string? hash = null
    )
    {
        ...
        _logger.LogTrace(
            "Creating cache entity for {name} success: {success}",
            fileInfo.FullName,
            (result != null)
        );
        return result;
    ...
    {
        if (
            fileCache.PrefixedFilePath.StartsWith(
                PenumbraPrefix,
                StringComparison.OrdinalIgnoreCase
            )
        )
        {
            fileCache.SetResolvedFilePath(
                fileCache.PrefixedFilePath.Replace(
                    PenumbraPrefix,
                    _ipcManager.Penumbra.ModDirectory,
                    StringComparison.Ordinal
                )
            ...
            fileCache.PrefixedFilePath.StartsWith(CachePrefix, StringComparison.OrdinalIgnoreCase)
        )
        {
            fileCache.SetResolvedFilePath(
                fileCache.PrefixedFilePath.Replace(
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        lock (_fileWriteLock)
        {
            try
            {
                ...
                {
                    _logger.LogInformation(
                        "{bakPath} found, moving to {csvPath}",
                        CsvBakPath,
                        _csvPath
                    );
        ...
        if (File.Exists(_csvPath))
        {
            if (
                ...
                || string.IsNullOrEmpty(_ipcManager.Penumbra.ModDirectory)
            )
            {
                _syncMediator.Publish(
                    new NotificationMessage(
                        "Penumbra not connected",
                        "Could not load local file cache data. Penumbra is not connected or not properly set up. Please enable and/or configure Penumbra properly to use Laci. After, reload Laci in the Plugin installer.",
                        SyncConfiguration.Models.NotificationType.Error
                    )
                );
            }
            ...
            {
                _logger.LogWarning(
                    "Could not load entries from {path}, continuing with empty file cache",
                    _csvPath
                );
            }
                ...
                try
                {
                    ...
                    if (hash.Length != 40)
                        throw new InvalidOperationException(
                            "Expected Hash length of 40, received " + hash.Length
                        );
                    ...
                    AddHashedFile(
                        ReplacePathPrefixes(new FileCacheEntity(hash, path, time, size, compressed))
                    );
                }
                catch (Exception ex)
                {
