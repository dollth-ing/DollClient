...
public sealed class CacheMonitor : DisposableMediatorSubscriberBase
{
        ...
        : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<PenumbraInitializedMessage>(
            ...
            (_) =>
            {
                ...
                StartSyncWatcher(configService.Current.CacheFolder);
                InvokeScan();
            }
        );
        ...
        Mediator.Subscribe<DalamudLoginMessage>(
            this,
            (_) =>
            {
                StartSyncWatcher(configService.Current.CacheFolder);
        ...
        Mediator.Subscribe<PenumbraDirectoryChangedMessage>(
            this,
            (msg) =>
            {
    ...
    public ConcurrentDictionary<string, int> HaltScanLocks { get; set; } =
    ...
    private readonly Dictionary<string, WatcherChange> _watcherChanges = new Dictionary<
        string,
        WatcherChange
    >(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, WatcherChange> _syncChanges = new Dictionary<
        string,
        WatcherChange
    >(StringComparer.OrdinalIgnoreCase);
    ...
    public void StartSyncWatcher(string? syncPath)
    {
        SyncWatcher?.Dispose();
        ...
        Logger.LogDebug("Initializing Laci FSW on {path}", syncPath);
        SyncWatcher = new()
        {
            Path = syncPath,
        ...
        SyncWatcher.Deleted += SyncWatcher_FileChanged;
        SyncWatcher.Created += SyncWatcher_FileChanged;
        SyncWatcher.EnableRaisingEvents = true;
    }

    private void SyncWatcher_FileChanged(object sender, FileSystemEventArgs e)
    {
        Logger.LogTrace("Laci FSW: FileChanged: {change} => {path}", e.ChangeType, e.FullPath);

        if (
            !AllowedFileExtensions.Any(ext =>
                e.FullPath.EndsWith(ext, StringComparison.OrdinalIgnoreCase)
            )
        )
            return;
        ...
        {
            _syncChanges[e.FullPath] = new(e.ChangeType);
        }
    ...
    public void StartPenumbraWatcher(string? penumbraPath)
    {
        ...
        if (string.IsNullOrEmpty(penumbraPath))
        {
            PenumbraWatcher = null;
            Logger.LogWarning(
                "Penumbra is not connected or the path is not set, cannot start FSW for Penumbra."
            );
            return;
        }
    ...
    private void HandleChanges(Dictionary<string, WatcherChange> changes)
    {
        lock (_fileDbManager)
        {
            var deletedEntries = changes
                .Where(c => c.Value.ChangeType == WatcherChangeTypes.Deleted)
                .Select(c => c.Key);
            var renamedEntries = changes.Where(c =>
                c.Value.ChangeType == WatcherChangeTypes.Renamed
            );
            var remainingEntries = changes
                .Where(c => c.Value.ChangeType != WatcherChangeTypes.Deleted)
                .Select(c => c.Key);
    ...
    private async Task PenumbraWatcherExecution()
    {
        ...
        Dictionary<string, WatcherChange> changes;
        lock (_watcherChanges)
            changes = _watcherChanges.ToDictionary(
                t => t.Key,
                t => t.Value,
    ...
    public void InvokeScan()
    {
        ...
        _scanCancellationTokenSource =
            _scanCancellationTokenSource?.CancelRecreate() ?? new CancellationTokenSource();
        ...
        _ = Task.Run(
            async () =>
            {
                ...
                {
                    Logger.LogWarning(
                        "Scanner is on framework, waiting for leaving thread before continuing"
                    );
    ...
    public void RecalculateFileCacheSize(CancellationToken token)
    {
        ...
        var files = Directory
            .EnumerateFiles(_configService.Current.CacheFolder)
            .Select(f => new FileInfo(f))
            .OrderBy(f => f.LastAccessTime)
            .ToList();
        FileCacheSize = files.Sum(f =>
        {
        ...
        var maxCacheInBytes = (long)(
            _configService.Current.MaxLocalCacheInGiB * 1024d * 1024d * 1024d
        );
    ...
    private void FullFileScan(CancellationToken ct)
    {
        ...
        Logger.LogDebug(
            "Getting files from {penumbra} and {storage}",
            penumbraDir,
            _configService.Current.CacheFolder
        );
        ...
        var allCacheFiles = Directory
            .GetFiles(_configService.Current.CacheFolder, "*.*", SearchOption.TopDirectoryOnly)
            .AsParallel()
            .Where(f =>
            {
                var val = f.Split('\\')[^1];
                ...
                (tcounter) =>
                {
                    ...
                    while (!ct.IsCancellationRequested && fileCaches.TryDequeue(out var workload))
                    {
                        try
                        {
                            ...
                            var validatedCacheResult = _fileDbManager.ValidateFileCacheEntity(
                                workload
                            );
                            if (validatedCacheResult.State != FileState.RequireDeletion)
                            {
                                ...
                                {
                                    allScannedFiles[
                                        validatedCacheResult.FileCache.ResolvedFilepath
                                    ] = true;
                                }
                            }
        ...
        {
            Parallel.ForEach(
                allScannedFiles.Where(c => !c.Value).Select(c => c.Key),
                new ParallelOptions()
                {
            ...
            Logger.LogTrace(
                "Scanner added {notScanned} new files to db",
                allScannedFiles.Count(c => !c.Value)
            );
        }
        ...
        if (!_configService.Current.InitialScanComplete)
        {
            ...
            _configService.Save();
            StartSyncWatcher(_configService.Current.CacheFolder);
