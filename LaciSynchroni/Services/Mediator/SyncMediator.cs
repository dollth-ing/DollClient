...
public sealed class SyncMediator : IHostedService
{
    ...
    {
        foreach (
            var subscriber in _subscriberDict
        ...
        )
        {
            ...
            sb.Append("=> ");
            foreach (
                var item in _subscriberDict
                    .Where(item => item.Value.Any(v => v.Subscriber == subscriber))
                    .ToList()
            )
            {
    ...
    {
        _logger.LogInformation("Starting LaciMediator");
        ...
        _logger.LogInformation("Started LaciMediator");
    ...
    internal void UnsubscribeAll(IMediatorSubscriber subscriber)
    {
        lock (_addRemoveLock)
        {
            ...
            {
                int unSubbed =
                    _subscriberDict[kvp]?.RemoveWhere(p => p.Subscriber == subscriber) ?? 0;
                if (unSubbed > 0)
                {
                    _logger.LogDebug(
                        "{sub} unsubscribed from {msg}",
                        subscriber.GetType().Name,
                        kvp.Name
                    );
                }
    ...
    {
        if (
            !_subscriberDict.TryGetValue(
                message.GetType(),
                out HashSet<SubscriberAction>? subscribers
            )
            || subscribers == null
            || !subscribers.Any()
        )
...
#pragma warning restore S3011 // Reflection should not be used to increase accessibility of classes, methods, or fields
    ...
    private void ExecuteReflected<T>(List<SubscriberAction> subscribers, T message)
        where T : MessageBase
    {
        foreach (SubscriberAction subscriber in subscribers)
        {
            try
            {
                if (_syncConfigService.Current.LogPerformance)
                {
                    ...
                    _performanceCollector.LogPerformance(
                        this,
                        $"{isSameThread}Execute>{message.GetType().Name}+{subscriber.Subscriber.GetType().Name}>{subscriber.Subscriber}",
            ...
            {
                if (
                    _lastErrorTime.TryGetValue(subscriber, out var lastErrorTime)
