...
public class PluginWarningNotificationService
{
    private readonly ConcurrentDictionary<
        UserData,
        OptionalPluginWarning
    > _cachedOptionalPluginWarnings = new(UserDataComparer.Instance);
    private readonly IpcManager _ipcManager;
    private readonly SyncConfigService _syncConfigService;
    private readonly SyncMediator _mediator;

    public PluginWarningNotificationService(
    ...
    public void NotifyForMissingPlugins(
        UserData user,
        string playerName,
    ...
    )
    {
        if (!_cachedOptionalPluginWarnings.TryGetValue(user, out var warning))
        {
            ...
            {
                ShownCustomizePlusWarning = _syncConfigService
                    .Current
                    .DisableOptionalPluginWarnings,
        ...
        if (
            changes.Contains(PlayerChanges.Heels)
            && !warning.ShownHeelsWarning
            && !_ipcManager.Heels.APIAvailable
        )
        {
        ...
        if (
            ...
            && !warning.ShownCustomizePlusWarning
            && !_ipcManager.CustomizePlus.APIAvailable
        )
        {
        ...
        if (
            changes.Contains(PlayerChanges.Honorific)
            && !warning.ShownHonorificWarning
            && !_ipcManager.Honorific.APIAvailable
        )
        {
        ...
        if (
            changes.Contains(PlayerChanges.Moodles)
            && !warning.ShownMoodlesWarning
            && !_ipcManager.Moodles.APIAvailable
        )
        {
        ...
        if (
            changes.Contains(PlayerChanges.PetNames)
            && !warning.ShowPetNicknamesWarning
            && !_ipcManager.PetNames.APIAvailable
        )
        {
