...
public sealed class XivDataAnalyzer
{
    ...
    public XivDataAnalyzer(
        ILogger<XivDataAnalyzer> logger,
    ...
    public unsafe Dictionary<string, List<ushort>>? GetSkeletonBoneIndices(
    ...
    )
    {
        ...
        try
        {
            for (int i = 0; i < chara->Skeleton->PartialSkeletonCount; i++)
            {
                ...
                _logger.LogTrace(
                    "Iterating over SkeletonResourceHandle #{i}:{x}",
                    i,
                    ((nint)handle).ToString("X")
                );
        ...
        return (outputIndices.Count != 0 && outputIndices.Values.All(u => u.Count > 0))
            ? outputIndices
            : null;
    }
    ...
    public unsafe Dictionary<string, List<ushort>>? GetBoneIndicesFromPap(string hash)
    {
        ...
        using BinaryReader reader = new BinaryReader(
            File.Open(cacheEntity.ResolvedFilepath, FileMode.Open, FileAccess.Read, FileShare.Read)
        );
        ...
        if (type != 0)
            return null; // it's not human, just ignore it, whatever
        ...
        try
        {
            ...
            loadoptions->ClassNameRegistry = hkBuiltinTypeRegistry
                .Instance()
            ...
            var resource = hkSerializeUtil.LoadFromFile(
                (byte*)tempHavokDataPathAnsi,
                null,
                loadoptions
            );
            if (resource == null)
            ...
            {
                var container = (hkRootLevelContainer*)
                    resource->GetContentsPointer(
                        n1,
                ...
                {
                    var animContainer = (hkaAnimationContainer*)
                        container->findObjectByName(n2, null);
    ...
    {
        if (
            _configService.Current.TriangleDictionary.TryGetValue(hash, out var cachedTris)
            && cachedTris > 0
        )
            return cachedTris;
        ...
        try
        {
            ...
            for (int i = 0; i < file.LodCount; i++)
            {
                ...
                catch (Exception ex)
                {
                    _logger.LogDebug(
                        ex,
                        "Could not load lod mesh {mesh} from path {path}",
                        i,
                        filePath
                    );
                    continue;
