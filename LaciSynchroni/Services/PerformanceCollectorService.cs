...
public sealed class PerformanceCollectorService : IHostedService
{
    ...
    public ConcurrentDictionary<
        string,
        RollingList<(TimeOnly, long)>
    > PerformanceCounters { get; } = new(StringComparer.Ordinal);
    ...
    public T LogPerformance<T>(
        object sender,
        CustomInterpolatedStringHandler counterName,
        Func<T> func,
    ...
    )
    {
        ...
        finally
        {
            ...
            if (TimeSpan.FromTicks(elapsed) > TimeSpan.FromMilliseconds(10))
                _logger.LogWarning(
                    ">10ms spike on {counterName}: {time}",
                    cn,
    ...
    internal void PrintPerformanceStats(int limitBySeconds = 0)
    {
        ...
        {
            sb.AppendLine(
                $"Performance Metrics over the past {limitBySeconds} seconds of each counter"
            );
        }
        else
        {
        ...
        var previousCaller = orderedData[0]
            .Key.Split(_counterSplit, StringSplitOptions.RemoveEmptyEntries)[0];
        foreach (var entry in orderedData)
        {
            var newCaller = entry.Key.Split(_counterSplit, StringSplitOptions.RemoveEmptyEntries)[
                0
            ];
            ...
            var pastEntries =
                limitBySeconds > 0
                    ? entry
                        .Value.Where(e =>
                            e.Item1.AddMinutes(limitBySeconds / 60.0d)
            ...
            if (pastEntries.Any())
            {
                sb.Append(
                    (
                        ...
                        + TimeSpan
                            .FromTicks(
                                pastEntries.LastOrDefault() == default
                                    ? 0
                                    : pastEntries.Last().Item2
                            )
                            .TotalMilliseconds.ToString("0.00000", CultureInfo.InvariantCulture)
                    ).PadRight(15)
                );
                sb.Append('|');
                sb.Append(
                    (
                        " "
                        + TimeSpan
                            .FromTicks(pastEntries.Max(m => m.Item2))
                            .TotalMilliseconds.ToString("0.00000", CultureInfo.InvariantCulture)
                    ).PadRight(15)
                );
                sb.Append('|');
                sb.Append(
                    (
                        " "
                        + TimeSpan
                            .FromTicks((long)pastEntries.Average(m => m.Item2))
                            .TotalMilliseconds.ToString("0.00000", CultureInfo.InvariantCulture)
                    ).PadRight(15)
                );
                sb.Append('|');
                    ...
                    (
                        ...
                        + (
                            pastEntries.LastOrDefault() == default
                                ...
                                : pastEntries
                                    .Last()
                                    .Item1.ToString("HH:mm:ss.ffff", CultureInfo.InvariantCulture)
                        )
                    ).PadRight(15, ' ')
                );
                sb.Append('|');
    ...
    private async Task PeriodicLogPrune()
    {
        ...
        {
            await Task.Delay(TimeSpan.FromMinutes(10), _periodicLogPruneTaskCts.Token)
            ...
            foreach (var entries in PerformanceCounters.ToList())
            {
                try
                {
                    ...
                    if (
                        last.Item1.AddMinutes(10) < TimeOnly.FromDateTime(DateTime.Now)
                        && !PerformanceCounters.TryRemove(entries.Key, out _)
                    )
                    {
                        _logger.LogDebug(
                            "Could not remove performance counter {counter}",
                            entries.Key
                        );
                    }
                }
                catch (Exception e)
                {
                    _logger.LogWarning(
                        e,
                        "Error removing performance counter {counter}",
                        entries.Key
                    );
                }
            }
