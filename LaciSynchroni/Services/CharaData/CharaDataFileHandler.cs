...
public sealed class CharaDataFileHandler : IDisposable
{
    ...
    public void ComputeMissingFiles(
        CharaDataDownloadDto charaDataDownloadDto,
        out Dictionary<string, string> modPaths,
        out List<FileReplacementData> missingFiles
    )
    {
        modPaths = [];
        missingFiles = [];
        foreach (var file in charaDataDownloadDto.FileGamePaths)
        {
            ...
            {
                var existingFile = missingFiles.Find(f =>
                    string.Equals(f.Hash, file.HashOrFileSwap, StringComparison.Ordinal)
                );
                if (existingFile == null)
                {
                    missingFiles.Add(
                        new FileReplacementData()
                        {
                ...
                {
                    existingFile.GamePaths = existingFile
                        .GamePaths.Concat([file.GamePath])
                        .ToArray();
                }
            }
            else
    ...
    public async Task<CharacterData?> CreatePlayerData()
    {
        ...
        using var tempHandler = await _gameObjectHandlerFactory
            .Create(
                ObjectKind.Player,
                () =>
                    _dalamudUtilService
                        .GetCharacterFromObjectTableByIndex(chara.ObjectIndex)
                        ?.Address ?? IntPtr.Zero,
                isWatched: false
            )
            .ConfigureAwait(false);
        PlayerData.Data.CharacterData newCdata = new();
        var fragment = await _playerDataFactory
            .BuildCharacterData(tempHandler, CancellationToken.None)
            .ConfigureAwait(false);
        newCdata.SetFragment(ObjectKind.Player, fragment);
        if (
            newCdata.FileReplacements.TryGetValue(ObjectKind.Player, out var playerData)
            && playerData != null
        )
        {
            ...
            {
                data.RemoveWhere(g =>
                    g.EndsWith(".pap", StringComparison.OrdinalIgnoreCase)
    ...
    {
        await _fileDownloadManager
            .InitiateDownloadList(tempHandler, missingFiles, token)
            .ConfigureAwait(false);
        await _fileDownloadManager
            .DownloadFiles(tempHandler, missingFiles, token)
            .ConfigureAwait(false);
        token.ThrowIfCancellationRequested();
        foreach (
            var file in missingFiles.SelectMany(
                m => m.GamePaths,
                (FileEntry, GamePath) => (FileEntry.Hash, GamePath)
            )
        )
        {
    ...
    public Task<(CharaFileHeader loadedCharaFile, long expectedLength)> LoadCharaFileHeader(
        string filePath
    )
    {
        try
        {
            ...
            if (loadedCharaFile != null)
            {
                ...
                foreach (var item in loadedCharaFile.CharaFileData.Files)
                {
                    ...
                    {
                        _logger.LogTrace(
                            "File {itemNr}: {gamePath} = {len}",
                            itemNr,
                            gamePath,
                            item.Length.ToByteString()
                        );
                    }
                }
    ...
    public Dictionary<string, string> McdfExtractFiles(
        CharaFileHeader? charaFileHeader,
        long expectedLength,
    ...
    )
    {
        ...
        using var lz4Stream = new LZ4Stream(
            File.OpenRead(charaFileHeader.FilePath),
            LZ4StreamMode.Decompress,
            LZ4StreamFlags.HighCompression
        );
        using var reader = new BinaryReader(lz4Stream);
        ...
        foreach (var fileData in charaFileHeader.CharaFileData.Files)
        {
            var fileName = Path.Combine(
                _fileCacheManager.CacheFolder,
                "laci_" + _globalFileCounter++ + ".tmp"
            );
            ...
            totalRead += length;
            _logger.LogTrace(
                "Read {read}/{expected} bytes",
                totalRead.ToByteString(),
                expectedLength.ToByteString()
            );
        }
    ...
    public async Task UpdateCharaDataAsync(CharaDataExtendedUpdateDto updateDto)
    {
        ...
        if (data != null)
        {
            var hasGlamourerData = data.GlamourerData.TryGetValue(
                ObjectKind.Player,
                out var playerDataString
            );
            if (!hasGlamourerData)
            ...
            var hasCustomizeData = data.CustomizePlusData.TryGetValue(
                ObjectKind.Player,
                out var customizeDataString
            );
            if (!hasCustomizeData)
            ...
            var hasFiles = data.FileReplacements.TryGetValue(
                ObjectKind.Player,
    ...
    internal async Task SaveCharaFileAsync(string description, string filePath)
    {
        ...
        try
        {
            ...
            CharaFileHeader output = new(
                CharaFileHeader.CurrentVersion,
                charaFileData
            );
        ...
        {
            _logger.LogError(ex, "Failure Saving Chara File, deleting output");
            File.Delete(tempFilePath);
        }
    }

    internal async Task<List<string>> UploadFiles(
        List<string> fileList,
        ValueProgress<string> uploadProgress,
        CancellationToken token
    )
    {
        return await _fileUploadManager
            .UploadFiles(fileList, uploadProgress, token)
            .ConfigureAwait(false);
    }
}