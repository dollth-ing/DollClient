...
public sealed class CharaDataNearbyManager : DisposableMediatorSubscriberBase
{
    ...
    public CharaDataNearbyManager(
        ILogger<CharaDataNearbyManager> logger,
    ...
    internal void SetHoveredVfx(PoseEntryExtended? hoveredPose)
    {
        ...
        if (_hoveredVfx == null)
        {
            var vfxGuid = _vfxSpawnManager.SpawnObject(
                hoveredPose.Position,
                hoveredPose.Rotation,
                Vector3.One * 4,
                1,
                0.2f,
                0.2f,
    ...
    private static float GetAngleToTarget(
    ...
    private async Task FilterEntriesAsync(Vector3 cameraPos, Vector3 cameraLookAt)
    {
        ...
        var ownLocation = await _dalamudUtilService
            .RunOnFrameworkThread(() => _dalamudUtilService.GetMapData())
            .ConfigureAwait(false);
        var player = await _dalamudUtilService
            .RunOnFrameworkThread(() => _dalamudUtilService.GetPlayerCharacter())
            .ConfigureAwait(false);
        ...
        foreach (
            var data in _metaInfoCache
                .Where(d =>
                    (
                        string.IsNullOrWhiteSpace(UserNoteFilter)
                        || (
                            (d.Key.Alias ?? string.Empty).Contains(
                                UserNoteFilter,
                            ...
                            || (
                                _serverConfigurationManager.GetNoteForUid(UserNoteFilter)
                                ?? string.Empty
                            ).Contains(UserNoteFilter, StringComparison.OrdinalIgnoreCase)
                        )
                    )
                )
        ...
        )
        {
            ...
            foreach (
                var pose in data
                    .Value.Where(v => v.HasPoses && v.HasWorldData && (showOwnData || !v.IsOwnData))
            ...
            )
            {
                ...
                if (distance > _charaDataConfigService.Current.NearbyDistanceFilter)
                    continue;

                bool addEntry =
                    (
                        !isInHousing
                        && poseLocation.MapId == ownLocation.MapId
                    ...
                    || (
                        ...
                        && (
                            (
                                ...
                                && poseLocation.DivisionId == ownLocation.DivisionId
                                && (
                            ...
                            || (
                                ...
                                && (
                                    ignoreHousingLimits
                                    || (
                                        poseLocation.HouseId == ownLocation.HouseId
                                        && poseLocation.WardId == ownLocation.WardId
                                        && poseLocation.DivisionId == ownLocation.DivisionId
                                        && poseLocation.RoomId == ownLocation.RoomId
                                    )
                                )
                            )
                        )
        ...
        if (
            _charaDataConfigService.Current.NearbyDrawWisps
    ...
    private unsafe void HandleFrameworkUpdate()
    {
        ...
        Vector3 lookAt = new(
            camera->LookAtVector.X,
            camera->LookAtVector.Y,
            camera->LookAtVector.Z
        );
    ...
    private void ManageWispsNearby(List<PoseEntryExtended> previousPoses)
    {
        foreach (var data in _nearbyData.Keys)
        {
            ...
            {
                vfxGuid = _vfxSpawnManager.SpawnObject(
                    data.Position,
                    data.Rotation,
                    Vector3.One * 2,
                    0.8f,
