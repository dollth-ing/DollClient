...
public sealed class CharaDataCharacterHandler : DisposableMediatorSubscriberBase
{
    ...
    public CharaDataCharacterHandler(
        ILogger<CharaDataCharacterHandler> logger,
        ...
        : base(logger, mediator)
    {
        ...
        mediator.Subscribe<CutsceneFrameworkUpdateMessage>(
            this,
            (_) => HandleCutsceneFrameworkUpdate()
        );
    }
    ...
    private void HandleCutsceneFrameworkUpdate()
    {
        ...
        {
            var chara = _dalamudUtilService.GetGposeCharacterFromObjectTableByName(
                entry.Name,
                onlyGposeCharacters: true
            );
            if (chara is null)
            {
    ...
    public async Task RevertChara(string name, Guid? cPlusId)
    {
        ...
        using var handler = await _gameObjectHandlerFactory
            .Create(
                ObjectKind.Player,
                () =>
                    _dalamudUtilService
                        .GetGposeCharacterFromObjectTableByName(name, _dalamudUtilService.IsInGpose)
                        ?.Address ?? IntPtr.Zero,
                false
            )
            .ConfigureAwait(false);
        if (handler.Address != nint.Zero)
            await _ipcManager
                .Penumbra.RedrawAsync(Logger, handler, applicationId, CancellationToken.None)
                .ConfigureAwait(false);
    }

    public async Task<bool> RevertHandledChara(string name)
    {
        var handled = _handledCharaData.FirstOrDefault(f =>
        ...
        await _dalamudUtilService
            .RunOnFrameworkThread(() => RevertChara(handled.Name, handled.CustomizePlus))
            .ConfigureAwait(false);
        return true;
    }
    ...
    public Task RevertHandledChara(HandledCharaDataEntry? handled)
    {
        ...
        _handledCharaData.Remove(handled);
        return _dalamudUtilService.RunOnFrameworkThread(() =>
            RevertChara(handled.Name, handled.CustomizePlus)
        );
    }
    ...
    public void UpdateHandledData(Dictionary<string, CharaDataMetaInfoExtendedDto?> newData)
    {
        ...
        {
            if (
                newData.TryGetValue(handledData.MetaInfo.FullId, out var metaInfo)
                && metaInfo != null
            )
            {
    ...
    public async Task<GameObjectHandler?> TryCreateGameObjectHandler(
        string name,
        bool gPoseOnly = false
    )
    {
        var handler = await _gameObjectHandlerFactory
            .Create(
                ObjectKind.Player,
                () =>
                    _dalamudUtilService
                        .GetGposeCharacterFromObjectTableByName(
                            name,
                            gPoseOnly && _dalamudUtilService.IsInGpose
                        )
                        ?.Address ?? IntPtr.Zero,
                false
            )
    ...
    public async Task<GameObjectHandler?> TryCreateGameObjectHandler(int index)
    {
        var handler = await _gameObjectHandlerFactory
            .Create(
                ObjectKind.Player,
                () =>
                    _dalamudUtilService.GetCharacterFromObjectTableByIndex(index)?.Address
                    ?? IntPtr.Zero,
                false
            )
            .ConfigureAwait(false);
