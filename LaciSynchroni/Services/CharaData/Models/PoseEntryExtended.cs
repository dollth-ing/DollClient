...
public sealed record PoseEntryExtended : PoseEntry
{
        ...
        : base(basePose)
    {
        ...
        {
            Position = new(
                basePose.WorldData!.Value.PositionX,
                basePose.WorldData!.Value.PositionY,
                basePose.WorldData!.Value.PositionZ
            );
            Rotation = new(
                basePose.WorldData!.Value.RotationX,
                basePose.WorldData!.Value.RotationY,
                basePose.WorldData!.Value.RotationZ,
                basePose.WorldData!.Value.RotationW
            );
        }
        MetaInfo = parent;
    }
    ...
    public static async Task<PoseEntryExtended> Create(
        PoseEntry baseEntry,
        CharaDataMetaInfoExtendedDto parent,
    ...
    )
    {
        ...
        if (newPose.HasWorldData)
        {
            ...
            newPose.MapCoordinates = await dalamudUtilService
                .RunOnFrameworkThread(() =>
                    MapUtil.WorldToMap(
                        new Vector2(worldData.PositionX, worldData.PositionY),
                        dalamudUtilService.MapData.Value[worldData.LocationInfo.MapId].Map
                    )
                )
            ...
            sb.AppendLine(
                "Server: "
                    + dalamudUtilService.WorldData.Value[(ushort)worldData.LocationInfo.ServerId]
            );
            sb.AppendLine(
                "Territory: "
                    + dalamudUtilService.TerritoryData.Value[worldData.LocationInfo.TerritoryId]
            );
            sb.AppendLine(
                "Map: " + dalamudUtilService.MapData.Value[worldData.LocationInfo.MapId].MapName
            );
            ...
            if (worldData.LocationInfo.DivisionId != 0)
            {
                sb.AppendLine(
                    "Subdivision: "
                        + worldData.LocationInfo.DivisionId switch
                        {
                            1 => "No",
                            2 => "Yes",
                            _ => "-",
            ...
            if (worldData.LocationInfo.HouseId != 0)
            {
                sb.AppendLine(
                    "House #: "
                        + (
                            worldData.LocationInfo.HouseId == 100
                                ? "Apartments"
                                : worldData.LocationInfo.HouseId.ToString()
                        )
                );
            }
            ...
            sb.AppendLine(
                "Coordinates: X: "
                    + newPose.MapCoordinates.X.ToString("0.0", CultureInfo.InvariantCulture)
                    + ", Y: "
                    + newPose.MapCoordinates.Y.ToString("0.0", CultureInfo.InvariantCulture)
            );
