...
public sealed record GposeLobbyUserData(UserData UserData)
{
    ...
    {
        if (DeltaPoseData == null && FullPoseData != null)
            return FullPoseData;
    ...
    public async Task SetWorldDataDescriptor(DalamudUtilService dalamudUtilService)
    {
        ...
        MapCoordinates = await dalamudUtilService
        ...
        sb.AppendLine(
            "Server: " + dalamudUtilService.WorldData.Value[(ushort)worldData.LocationInfo.ServerId]
        );
        sb.AppendLine(
            "Territory: "
        ...
        sb.AppendLine(
            "Coordinates: X: "
                + MapCoordinates.X.ToString("0.0", CultureInfo.InvariantCulture)
                + ", Y: "
                + MapCoordinates.Y.ToString("0.0", CultureInfo.InvariantCulture)
        );
