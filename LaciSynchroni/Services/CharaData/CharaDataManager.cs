...
public sealed partial class CharaDataManager : DisposableMediatorSubscriberBase
{
    ...
    public CharaDataManager(
        ILogger<CharaDataManager> logger,
        ...
        : base(logger, syncMediator)
    {
        ...
        syncMediator.Subscribe<ConnectedMessage>(
            this,
            (msg) =>
            {
                _connectCts?.Cancel();
        ...
        syncMediator.Subscribe<DisconnectedMessage>(
            this,
            (msg) =>
            {
                _ownCharaData.Clear();
    ...
    public IEnumerable<HandledCharaDataEntry> HandledCharaData =>
        _characterHandler.HandledCharaData;
    public bool Initialized { get; private set; }
    public CharaDataMetaInfoExtendedDto? LastDownloadedMetaInfo { get; private set; }
    public Task<(CharaFileHeader LoadedFile, long ExpectedLength)>? LoadedMcdfHeader
    {
        get;
        private set;
    }
    ...
    public IDictionary<UserData, List<CharaDataMetaInfoExtendedDto>> SharedWithYouData =>
        _sharedWithYouData;
    ...
    {
        return UiBlockingComputation = DataApplicationTask =
            Task.Run(async () =>
            {
                ...
                CharaDataMetaInfoDto metaInfo = new(dataDownloadDto.Id, dataDownloadDto.Uploader)
                {
                    ...
                    Description =
                        $"Data from {dataDownloadDto.Uploader.AliasOrUID} for {dataDownloadDto.Id}",
                    UpdatedDate = dataDownloadDto.UpdatedDate,
                };

                await DownloadAndAplyDataAsync(charaName, dataDownloadDto, metaInfo, false)
        ...
        return UiBlockingComputation = DataApplicationTask =
            Task.Run(async () =>
            {
                ...
                var download = await _apiController
                    .CharaDataDownload(dataMetaInfoDto.Uploader.UID + ":" + dataMetaInfoDto.Id)
                    .ConfigureAwait(false);
                if (download == null)
                {
                ...
                await DownloadAndAplyDataAsync(charaName, download, dataMetaInfoDto, false)
                    .ConfigureAwait(false);
            });
    }
    ...
    public Task ApplyCharaDataToGposeTarget(CharaDataMetaInfoDto dataMetaInfoDto)
    {
        return UiBlockingComputation = DataApplicationTask =
            Task.Run(async () =>
            {
                var obj = await _dalamudUtilService
    ...
    public async Task ApplyOwnDataToGposeTarget(CharaDataFullExtendedDto dataDto)
    {
        ...
        UiBlockingComputation = DataApplicationTask = DownloadAndAplyDataAsync(
            charaName,
            downloadDto,
            metaInfoDto,
            false
    ...
    public Task ApplyPoseData(PoseEntry pose, string targetName)
    {
        ...
        {
            if (
                string.IsNullOrEmpty(pose.PoseData)
                || !(await CanApplyInGpose().ConfigureAwait(false)).CanApply
            )
                return;
            ...
            var poseJson = Encoding.UTF8.GetString(
                LZ4Wrapper.Unwrap(Convert.FromBase64String(pose.PoseData))
            );
    ...
    public Task ApplyWorldDataToTarget(PoseEntry pose, string targetName)
    {
        return UiBlockingComputation = Task.Run(async () =>
        {
            ...
            if (pose.WorldData == null || pose.WorldData == default)
                return;

            Logger.LogDebug("Applying World data {data}", pose.WorldData);

            await _ipcManager
                .Brio.ApplyTransformAsync(gposeChara.Address, pose.WorldData.Value)
                .ConfigureAwait(false);
        });
    }
    ...
    public void AttachWorldData(PoseEntry pose, CharaDataExtendedUpdateDto updateDto)
    {
        ...
        {
            ICharacter? playerChar = await _dalamudUtilService
                .GetPlayerCharacterAsync()
                .ConfigureAwait(false);
            if (playerChar == null)
                return;
            if (_dalamudUtilService.IsInGpose)
            {
                playerChar = await _dalamudUtilService
                    .GetGposeCharacterFromObjectTableByNameAsync(playerChar.Name.TextValue, true)
                    .ConfigureAwait(false);
            }
            if (playerChar == null)
                return;
            var worldData = await _ipcManager
                .Brio.GetTransformAsync(playerChar.Address)
                .ConfigureAwait(false);
            ...
            worldData.LocationInfo = await _dalamudUtilService
                .GetMapDataAsync()
                .ConfigureAwait(false);
    ...
    public void DownloadMetaInfo(string importCode, bool store = true)
    {
        DownloadMetaInfoTask = Task.Run(async () =>
        {
            try
            {
                ...
                var metaInfo = await _apiController
                    .CharaDataGetMetainfo(importCode)
                    .ConfigureAwait(false);
                ...
                if (store)
                {
                    LastDownloadedMetaInfo = await CharaDataMetaInfoExtendedDto
                        .Create(metaInfo, _dalamudUtilService)
                        .ConfigureAwait(false);
                }
                return ("Ok", true);
            }
    ...
    public async Task GetAllData(CancellationToken cancelToken)
    {
        ...
        foreach (
            var id in _updateDtos
                .Keys.Where(r =>
                    !result.Exists(res => string.Equals(res.Id, r, StringComparison.Ordinal))
                )
                .ToList()
        )
        {
    ...
    {
        if (!_sharedMetaInfoTimeoutTasks.TryGetValue(key, out var task))
            return false;
        ...
        UiBlockingComputation = McdfApplicationTask = Task.Run(async () =>
        {
            ...
            try
            {
                using GameObjectHandler? tempHandler = await _characterHandler
                    .TryCreateGameObjectHandler(charaName, true)
                    .ConfigureAwait(false);
                if (tempHandler == null)
                    return;
                var playerChar = await _dalamudUtilService
                    .GetPlayerCharacterAsync()
                    .ConfigureAwait(false);
                bool isSelf =
                    playerChar != null
                    && string.Equals(
                        playerChar.Name.TextValue,
                        tempHandler.Name,
                ...
                var extractedFiles = _fileHandler.McdfExtractFiles(
                    charaFile,
                    expectedExtractedSize,
                    actuallyExtractedFiles
                );

                foreach (
                    var entry in charaFile.CharaFileData.FileSwaps.SelectMany(
                        k => k.GamePaths,
                        (k, p) => new KeyValuePair<string, string>(p, k.FileSwapPath)
                    )
                )
                {
                ...
                var extended = await CharaDataMetaInfoExtendedDto
                    .Create(
                        new(charaFile.FilePath, new UserData(string.Empty)),
                        _dalamudUtilService
                    )
                    .ConfigureAwait(false);
    ...
    public void SaveLaciCharaFile(string description, string filePath)
    {
    ...
    public void SetAppearanceData(string dtoId)
    {
        ...
        if (!hasUpdateDto || updateDto == null)
            return;
    ...
    public Task<HandledCharaDataEntry?> SpawnAndApplyData(CharaDataDownloadDto charaDataDownloadDto)
    {
        var task = Task.Run(async () =>
        {
            ...
            await ApplyCharaData(charaDataDownloadDto, newActor.Name.TextValue)
                .ConfigureAwait(false);
    ...
    public Task<HandledCharaDataEntry?> SpawnAndApplyData(CharaDataMetaInfoDto charaDataMetaInfoDto)
    {
        var task = Task.Run(async () =>
        {
            ...
            await ApplyCharaData(charaDataMetaInfoDto, newActor.Name.TextValue)
                .ConfigureAwait(false);
    ...
    private async Task<CharaDataMetaInfoExtendedDto> CacheData(
        CharaDataFullExtendedDto ownCharaData
    )
    {
        var metaInfo = new CharaDataMetaInfoDto(ownCharaData.Id, ownCharaData.Uploader)
        {
            ...
            CanBeDownloaded =
                !string.IsNullOrEmpty(ownCharaData.GlamourerData)
                && (ownCharaData.OriginalFiles.Count == ownCharaData.FileGamePaths.Count),
            PoseData = ownCharaData.PoseData,
        };

        var extended = await CharaDataMetaInfoExtendedDto
            .Create(metaInfo, _dalamudUtilService, isOwnData: true)
            .ConfigureAwait(false);
    ...
    private async Task<CharaDataMetaInfoExtendedDto> CacheData(
        CharaDataMetaInfoDto metaInfo,
        bool isOwnData = false
    )
    {
        var extended = await CharaDataMetaInfoExtendedDto
            .Create(metaInfo, _dalamudUtilService, isOwnData)
            .ConfigureAwait(false);
        _metaInfoCache[extended.FullId] = extended;
        DistributeMetaInfo();
    ...
    private async Task<(string Output, bool Success)> RestoreThenUpload(
    ...
    )
    {
        ...
        var result = await UploadFiles(
                ...
                async () =>
                {
                    ...
                    CharaDataUpdateDto updateDto = new(dto.Id) { FileGamePaths = newFilePaths };
    ...
    internal void AttachPoseData(PoseEntry pose, CharaDataExtendedUpdateDto updateDto)
    {
        AttachingPoseTask = Task.Run(async () =>
        {
            ...
            if (playerChar == null)
                return;
            var poseData = await _ipcManager
                .Brio.GetPoseAsync(playerChar.Address)
                .ConfigureAwait(false);
            if (poseData == null)
                return;
    ...
    {
        var gposeActor = _dalamudUtilService.GetGposeCharacterFromObjectTableByName(
            actor.Name,
            true
        );
        if (gposeActor != null)
    ...
    )
    {
        ...
        try
        {
            ...
            bool reverted = await _characterHandler
                .RevertHandledChara(tempHandler.Name)
                .ConfigureAwait(false);
            if (reverted)
            ...
            penumbraCollection = await _ipcManager
                .Penumbra.CreateTemporaryCollectionAsync(
                    Logger,
                    metaInfo.Uploader.UID + metaInfo.Id
                )
                .ConfigureAwait(false);
            var idx =
            ...
            await _ipcManager
                .Penumbra.AssignTemporaryCollectionAsync(Logger, penumbraCollection, idx)
                .ConfigureAwait(false);
            await _ipcManager
                .Penumbra.SetTemporaryModsAsync(Logger, applicationId, penumbraCollection, modPaths)
                .ConfigureAwait(false);
            await _ipcManager
            ...
            await _ipcManager
                .Glamourer.ApplyAllAsync(Logger, tempHandler, glamourerData, applicationId, token)
                .ConfigureAwait(false);
            await _ipcManager
                .Penumbra.RedrawAsync(Logger, tempHandler, applicationId, token)
                .ConfigureAwait(false);
            await _dalamudUtilService
            ...
            await _ipcManager
                .Penumbra.RemoveTemporaryCollectionAsync(Logger, applicationId, penumbraCollection)
                .ConfigureAwait(false);
            ...
            {
                cPlusId = await _ipcManager
                    .CustomizePlus.SetBodyScaleAsync(tempHandler.Address, customizeData)
                    .ConfigureAwait(false);
            }
            else
            {
                cPlusId = await _ipcManager
                    .CustomizePlus.SetBodyScaleAsync(
                        tempHandler.Address,
            ...
            if (autoRevert)
            {
                ...
                while (i > 0)
                {
                    DataApplicationProgress =
                        $"All data applied. Reverting automatically in {i} seconds.";
            ...
            {
                Logger.LogTrace(
                    "[{appId}] Adding {name} to handled objects",
                    applicationId,
                    tempHandler.Name
                );
        ...
        finally
        {
            ...
            if (autoRevert)
            {
                await _characterHandler
                    .RevertChara(tempHandler.Name, cPlusId)
                    .ConfigureAwait(false);
            }
            ...
            if (
                metaInfo != null
                && _configService.Current.FavoriteCodes.TryGetValue(
                    metaInfo.Uploader.UID + ":" + metaInfo.Id,
                    out var favorite
                )
                && favorite != null
    ...
    private async Task DownloadAndAplyDataAsync(
        string charaName,
        CharaDataDownloadDto charaDataDownloadDto,
        CharaDataMetaInfoDto metaInfo,
    ...
    )
    {
        ...
        ICharacter? chara = (
            await _dalamudUtilService
                .GetGposeCharacterFromObjectTableByNameAsync(
                    charaName,
                    _dalamudUtilService.IsInGpose
                )
                .ConfigureAwait(false)
        ...
        if (missingFiles.Any())
        {
            try
            {
                ...
                await _fileHandler
                    .DownloadFilesAsync(tempHandler, missingFiles, modPaths, token)
                    .ConfigureAwait(false);
            }
        ...
        await ApplyDataAsync(
                ...
                extendedMetaInfo,
                modPaths,
                charaDataDownloadDto.ManipulationData,
                charaDataDownloadDto.GlamourerData,
                charaDataDownloadDto.CustomizeData,
                token
            )
            .ConfigureAwait(false);
    }

    public async Task<(string Result, bool Success)> UploadFiles(
        List<GamePathEntry> missingFileList,
        Func<Task>? postUpload = null
    )
    {
        ...
        try
        {
            ...
            var missingFiles = await _fileHandler
                .UploadFiles(
                    [.. missingFileList.Select(k => k.HashOrFileSwap)],
                    UploadProgress,
                    _uploadCts.Token
                )
                .ConfigureAwait(false);
            if (missingFiles.Any())
            {
                ...
                return (
                    $"Upload failed: {missingFiles.Count} missing or forbidden to upload local files.",
                    false
                );
            }
    ...
    internal void RemoveChara(string handledActor)
    {
        ...
        UiBlockingComputation = Task.Run(async () =>
        {
            ...
            var gposeChara = await _dalamudUtilService
                .GetGposeCharacterFromObjectTableByNameAsync(handledActor, true)
                .ConfigureAwait(false);
            if (gposeChara != null)
