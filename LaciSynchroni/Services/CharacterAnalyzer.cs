...
public sealed class CharacterAnalyzer : MediatorSubscriberBase, IDisposable
{
    ...
    public CharacterAnalyzer(
        ILogger<CharacterAnalyzer> logger,
    ...
    private async Task BaseAnalysis(CharacterData charaData, CancellationToken token)
    {
        ...
        foreach (var obj in charaData.FileReplacements)
        {
            ...
            foreach (var fileEntry in obj.Value)
            {
                ...
                var tris = await _xivDataAnalyzer
                    .GetTrianglesByHash(fileEntry.Hash)
    ...
    private void PrintAnalysis()
    {
        ...
        foreach (var kvp in LastAnalysis)
        {
            ...
            foreach (
                var entry in kvp.Value.OrderBy(
                    b => b.Value.GamePaths.OrderBy(p => p, StringComparer.Ordinal).First(),
            ...
            )
            {
                ...
                if (entry.Value.FilePaths.Count > 1)
                    Logger.LogInformation("  Multiple fitting files detected for {key}", entry.Key);
                ...
                Logger.LogInformation(
                    ...
                    UiSharedService.ByteToString(entry.Value.OriginalSize),
        ...
        foreach (var kvp in LastAnalysis)
        {
            ...
            foreach (
                var entry in kvp
                    ...
                    .GroupBy(v => v.FileType, StringComparer.Ordinal)
            )
            {
                Logger.LogInformation(
                    "{ext} files: {count}, size extracted: {size}, size compressed: {sizeComp}",
                    entry.Key,
                    entry.Count(),
                    UiSharedService.ByteToString(entry.Sum(v => v.OriginalSize)),
                    UiSharedService.ByteToString(entry.Sum(v => v.CompressedSize))
                );
            }
            Logger.LogInformation("=== Total summary for {obj} ===", kvp.Key);
            Logger.LogInformation(
                "Total files: {count}, size extracted: {size}, size compressed: {sizeComp}",
                kvp.Value.Count,
                UiSharedService.ByteToString(kvp.Value.Sum(v => v.Value.OriginalSize)),
                UiSharedService.ByteToString(kvp.Value.Sum(v => v.Value.CompressedSize))
            );
        }
        ...
        Logger.LogInformation(
            ...
            UiSharedService.ByteToString(
                LastAnalysis.Values.Sum(c => c.Values.Sum(v => v.OriginalSize))
            ),
            UiSharedService.ByteToString(
                LastAnalysis.Values.Sum(c => c.Values.Sum(v => v.CompressedSize))
            )
        );
        Logger.LogInformation(
            "IMPORTANT NOTES:\n\r- For Laci up- and downloads only the compressed size is relevant.\n\r- An unusually high total files count beyond 200 and up will also increase your download time to others significantly."
        );
    }
    ...
    )
    {
        ...
        {
            var compressedsize = await fileCacheManager
                .GetCompressedFileData(Hash, token)
                .ConfigureAwait(false);
