...
public sealed class UiService : DisposableMediatorSubscriberBase
{
        ...
        : base(logger, syncMediator)
    {
        ...
        Mediator.Subscribe<ProfileOpenStandaloneMessage>(
            this,
            (msg) =>
            {
                if (
                    !_createdWindows.Exists(p =>
                        p is StandaloneProfileUi ui
                        && string.Equals(
                            ui.Pair.UserData.AliasOrUID,
                            msg.Pair.UserData.AliasOrUID,
        ...
        Mediator.Subscribe<OpenSyncshellAdminPanel>(
            this,
            (msg) =>
            {
                if (
                    !_createdWindows.Exists(p =>
                        p is SyncshellAdminUI ui
                        && string.Equals(
                            ui.GroupFullInfo.GID,
                            msg.GroupInfo.GID,
                            StringComparison.Ordinal
                        )
                    )
        ...
        Mediator.Subscribe<OpenPermissionWindow>(
            this,
            (msg) =>
            {
                if (!_createdWindows.Exists(p => p is PermissionWindowUI ui && msg.Pair == ui.Pair))
                {
        ...
        Mediator.Subscribe<RemoveWindowMessage>(
            this,
            (msg) =>
            {
