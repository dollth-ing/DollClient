...
public class DalamudUtilService : IHostedService, IMediatorSubscriber
{
    ...
    private readonly Dictionary<string, (string Name, nint Address)> _playerCharas = new(
    ...
    )
    {
        ...
        {
            return gameData
                .GetExcelSheet<Lumina.Excel.Sheets.World>(Dalamud.Game.ClientLanguage.English)!
                .Where(w =>
                    !w.Name.IsEmpty
                    && w.DataCenter.RowId != 0
        ...
        {
            return gameData
                .GetExcelSheet<ClassJob>(Dalamud.Game.ClientLanguage.English)!
        ...
        {
            return gameData
                .GetExcelSheet<Map>(Dalamud.Game.ClientLanguage.English)!
                .Where(w => w.RowId != 0)
                .ToDictionary(
                    w => w.RowId,
                    w =>
                    {
                        ...
                        if (
                            w.PlaceNameSub.ValueNullable != null
        ...
        mediator.Subscribe<TargetPairMessage>(
            this,
            (msg) =>
            {
                ...
                if (string.IsNullOrEmpty(name))
                    return;
                var addr = _playerCharas
                    .FirstOrDefault(f =>
                        string.Equals(f.Value.Name, name, StringComparison.Ordinal)
                    )
                    .Value.Address;
                if (addr == nint.Zero)
                    return;
    ...
    public async Task<IGameObject?> GetGposeTargetGameObjectAsync()
    {
        ...
        return await _framework
            .RunOnFrameworkThread(() => _objectTable[GPoseTargetIdx])
            .ConfigureAwait(true);
    }
    ...
    {
        return await RunOnFrameworkThread(() => _objectTable.CreateObjectReference(reference))
            .ConfigureAwait(false);
    }
    ...
    {
        return await RunOnFrameworkThread(() => GetCompanionPtr(playerPointer))
            .ConfigureAwait(false);
    }

    public async Task<ICharacter?> GetGposeCharacterFromObjectTableByNameAsync(
        string name,
        bool onlyGposeCharacters = false
    )
    {
        return await RunOnFrameworkThread(() =>
                GetGposeCharacterFromObjectTableByName(name, onlyGposeCharacters)
            )
            .ConfigureAwait(false);
    }

    public ICharacter? GetGposeCharacterFromObjectTableByName(
        string name,
        bool onlyGposeCharacters = false
    )
    {
    ...
    {
        return await RunOnFrameworkThread(() => GetMinionOrMountPtr(playerPointer))
            .ConfigureAwait(false);
    }
    ...
    {
        EnsureIsOnFramework();
        if (_classJobIdsIgnoredForPets.Contains(_classJobId ?? 0))
            return IntPtr.Zero;
        var mgr = CharacterManager.Instance();
        playerPointer ??= GetPlayerPtr();
        if (playerPointer == IntPtr.Zero || (IntPtr)mgr == IntPtr.Zero)
    ...
    {
        if (_playerCharas.TryGetValue(characterName, out var pchar))
    ...
    {
        return await RunOnFrameworkThread(() => _cid.Value.ToString().GetHash256())
            .ConfigureAwait(false);
    }

    private static unsafe string GetHashedCIDFromPlayerPointer(nint ptr)
    {
    ...
    public unsafe LocationInfo GetMapData()
    {
        ...
        if (_clientState.LocalPlayer == null)
            ...
            serverId = _clientState.LocalPlayer.CurrentWorld.RowId;
    ...
    public async Task RunOnFrameworkThread(
        System.Action act,
    ...
    )
    {
        ...
        await _performanceCollector
            .LogPerformance(
                this,
                $"RunOnFramework:Act/{fileName}>{callerMember}:{callerLineNumber}",
                async () =>
                {
                    if (!_framework.IsInFrameworkUpdateThread)
                    {
    ...
    public async Task<T> RunOnFrameworkThread<T>(
        Func<T> func,
        [CallerMemberName] string callerMember = "",
        [CallerFilePath] string callerFilePath = "",
        [CallerLineNumber] int callerLineNumber = 0
    ...
    )
    {
        ...
        catch (NullReferenceException ex)
        {
            logger.LogWarning(
                ex,
                "Error accessing {handler}, object does not exist anymore?",
                handler
            );
        }
    ...
    private unsafe void CheckCharacterForDrawing(nint address, string characterName)
    {
        ...
        if ((nint)drawObj != IntPtr.Zero)
        {
            ...
            if (!isDrawing)
            {
                ...
                if (!isDrawing)
                {
                    ...
                    if (
                        isDrawing
                        && !string.Equals(
                            _lastGlobalBlockPlayer,
                            characterName,
                            StringComparison.Ordinal
                        )
                        && !string.Equals(
                            _lastGlobalBlockReason,
            ...
            {
                if (
                    !string.Equals(_lastGlobalBlockPlayer, characterName, StringComparison.Ordinal)
                    && !string.Equals(
                        _lastGlobalBlockReason,
                        "RenderFlags",
                        StringComparison.Ordinal
        ...
        {
            _logger.LogTrace(
                "Global draw block: START => {name} ({reason})",
                characterName,
                _lastGlobalBlockReason
            );
        }
    ...
    private unsafe void FrameworkOnUpdateInternal()
    {
        ...
        _performanceCollector.LogPerformance(
            this,
            $"FrameworkOnUpdateInternal+{(isNormalFrameworkUpdate ? "Regular" : "Delayed")}",
            () =>
            {
                ...
                _performanceCollector.LogPerformance(
                    ...
                    () =>
                    {
                        ...
                        for (int i = 0; i < 200; i += 2)
                        {
                            ...
                            if (
                                _blockedCharacterHandler.IsCharacterBlocked(
                                    chara.Address,
                                    out bool firstTime
                                ) && firstTime
                            )
                            {
                                _logger.LogTrace(
                                    "Skipping character {addr}, blocked/muted",
                                    chara.Address.ToString("X")
                                );
                                continue;
                            }
                ...
                if (
                    (_condition[ConditionFlag.Performing] || _condition[ConditionFlag.InCombat])
                    && !IsInCombatOrPerforming
                )
                {
                ...
                )
                {
                    ...
                    if (_lastZone != zone)
                    {
                        ...
                        if (!_sentBetweenAreas)
                        {
                            ...
                            Mediator.Publish(
                                new HaltScanMessage(nameof(ConditionFlag.BetweenAreas))
                            );
                        }
                    }
                ...
                if (
                    _gameConfig != null
                    && _gameConfig.TryGet(
                        Dalamud.Game.Config.SystemConfigOption.LodType_DX11,
                        out bool lodEnabled
                    )
                )
                {
