...
public sealed class GameObjectHandler
    : DisposableMediatorSubscriberBase,
        IHighPriorityMediatorSubscriber
{
        ...
        : base(logger, mediator)
    {
        ...
        if (ownedObject)
        {
            Mediator.Subscribe<TransientResourceChangedMessage>(
                this,
                (msg) =>
                {
        ...
        Mediator.Subscribe<CutsceneStartMessage>(
            this,
            (_) =>
            {
        ...
        Mediator.Subscribe<CutsceneEndMessage>(
            this,
            (_) =>
            {
        ...
        Mediator.Subscribe<PenumbraStartRedrawMessage>(
            this,
            (msg) =>
            {
                if (msg.Address == Address)
        ...
        Mediator.Subscribe<PenumbraEndRedrawMessage>(
            this,
            (msg) =>
            {
                if (msg.Address == Address)
    ...
    public async Task ActOnFrameworkAfterEnsureNoDrawAsync(
        Action<Dalamud.Game.ClientState.Objects.Types.ICharacter> act,
    ...
    public void CompareNameAndThrow(string name)
    {
        if (!string.Equals(Name, name, StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException(
                "Player name not equal to requested name, pointer invalid"
            );
        }
    ...
    private unsafe void CheckAndUpdateObject()
    {
        ...
        if (Address != IntPtr.Zero)
        {
            var drawObjAddr = (IntPtr)
                ((FFXIVClientStructs.FFXIV.Client.Game.Object.GameObject*)Address)->DrawObject;
        ...
        if (Address != IntPtr.Zero && DrawObjectAddress != IntPtr.Zero)
        {
            ...
            if (
                ((DrawObject*)DrawObjectAddress)->Object.GetObjectType() == ObjectType.CharacterBase
                && ((CharacterBase*)DrawObjectAddress)->GetModelType()
                    == CharacterBase.ModelType.Human
            )
            {
                ...
                {
                    Logger.LogTrace(
                        "[{this}] classjob changed from {old} to {new}",
                        this,
                        _classJob,
                        classJob
                    );
                ...
                equipDiff = CompareAndUpdateEquipByteData(
                    (byte*)&((Human*)DrawObjectAddress)->Head
                );
                ...
                if (equipDiff)
                    Logger.LogTrace(
                        "Checking [{this}] equip data as human from draw obj, result: {diff}",
                        this,
                        equipDiff
                    );
            }
            ...
            {
                equipDiff = CompareAndUpdateEquipByteData(
                    (byte*)Unsafe.AsPointer(ref chara->DrawData.EquipmentModelIds[0])
                );
                if (equipDiff)
                    Logger.LogTrace(
                        "Checking [{this}] equip data from game obj, result: {diff}",
                        this,
                        equipDiff
                    );
            }
            ...
            )
            {
                ...
                customizeDiff = CompareAndUpdateCustomizeData(
                    ((Human*)DrawObjectAddress)->Customize.Data
                );
                if (customizeDiff)
                    Logger.LogTrace(
                        "Checking [{this}] customize data as human from draw obj, result: {diff}",
                        this,
                        customizeDiff
                    );
            }
            else
            {
                ...
                if (customizeDiff)
                    Logger.LogTrace(
                        "Checking [{this}] customize data from game obj, result: {diff}",
                        this,
                        equipDiff
                    );
            }
            ...
            if (
                (addrDiff || drawObjDiff || equipDiff || customizeDiff || nameChange)
                && _isOwnedObject
            )
            {
    ...
    {
        if (!_delayedZoningTask?.IsCompleted ?? false)
            return;
        ...
        try
        {
            _performanceCollector.LogPerformance(
                this,
                $"CheckAndUpdateObject>{(_isOwnedObject ? "Self" : "Other")}+{ObjectKind}/{(string.IsNullOrEmpty(Name) ? "Unk" : Name)}"
                    + $"+{Address.ToString("X")}",
                CheckAndUpdateObject
            );
        }
    ...
    private unsafe DrawCondition IsBeingDrawnUnsafe()
    {
        ...
        var renderFlags =
            (((FFXIVClientStructs.FFXIV.Client.Game.Object.GameObject*)Address)->RenderFlags)
            != 0x0;
        if (renderFlags)
        ...
        {
            var modelInSlotLoaded = (
                ((CharacterBase*)DrawObjectAddress)->HasModelInSlotLoaded != 0
            );
            if (modelInSlotLoaded)
                return DrawCondition.ModelInSlotLoaded;
            var modelFilesInSlotLoaded = (
