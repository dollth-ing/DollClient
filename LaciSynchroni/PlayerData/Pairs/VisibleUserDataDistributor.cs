...
public class VisibleUserDataDistributor : DisposableMediatorSubscriberBase
{
    ...
    public VisibleUserDataDistributor(
        ILogger<VisibleUserDataDistributor> logger,
        ...
        : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<CharacterDataCreatedMessage>(
            ...
            (msg) =>
            {
                ...
                {
                    Logger.LogTrace(
                        "Data hash {hash} equal to stored data",
                        newData.DataHash.Value
                    );
                }
            }
    ...
    private void PushToAllVisibleUsers(bool forced = false)
    {
        ...
        {
            Logger.LogDebug(
                "Pushing data {hash} for {count} visible players",
                _lastCreatedData?.DataHash.Value ?? "UNKNOWN",
                _usersToPushDataTo.Count
            );
    ...
    private void FrameworkOnUpdate()
    {
        ...
        Logger.LogDebug(
            "Scheduling character data push of {data} to {users}",
    ...
    private void PushCharacterData(bool forced = false)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            if (_fileUploadTask == null || (_fileUploadTask?.IsCompleted ?? false) || forced)
            {
                ...
                Logger.LogDebug(
                    "Starting UploadTask for {hash}, Reason: TaskIsNull: {task}, TaskIsCompleted: {taskCpl}, Forced: {frc}",
                    _lastCreatedData.DataHash,
                    _fileUploadTask == null,
                    _fileUploadTask?.IsCompleted ?? false,
                    forced
                );
                _fileUploadTask = _fileTransferManager.UploadFiles(
                    _uploadingCharacterData,
                    [.. _usersToPushDataTo]
                );
            }
            ...
            if (_fileUploadTask != null)
            {
                ...
                try
                {
                    if (_usersToPushDataTo.Count == 0)
                        return;
                    Logger.LogDebug(
                        "Pushing {data} to {users}",
                        dataToSend.DataHash,
                        string.Join(", ", _usersToPushDataTo.Select(k => k.AliasOrUID))
                    );
                    await _apiController
                        .PushCharacterData(dataToSend, [.. _usersToPushDataTo])
