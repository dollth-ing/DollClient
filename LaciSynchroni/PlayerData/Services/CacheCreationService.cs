...
public sealed class CacheCreationService : DisposableMediatorSubscriberBase
{
    ...
    public CacheCreationService(
        ...
        PlayerDataFactory characterDataFactory,
        ...
        : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<HaltCharaDataCreation>(
            this,
            (msg) =>
            {
        ...
        Mediator.Subscribe<CreateCacheForObjectMessage>(
            this,
            (msg) =>
            {
                Logger.LogDebug(
                    "Received CreateCacheForObject for {handler}, updating",
                    msg.ObjectToCreateFor
                );
                AddCacheToCreate(msg.ObjectToCreateFor.ObjectKind);
            }
        );

        _playerRelatedObjects[ObjectKind.Player] = gameObjectHandlerFactory
            .Create(ObjectKind.Player, dalamudUtil.GetPlayerPtr, isWatched: true)
            .GetAwaiter()
            .GetResult();
        _playerRelatedObjects[ObjectKind.MinionOrMount] = gameObjectHandlerFactory
            .Create(
                ObjectKind.MinionOrMount,
                () => dalamudUtil.GetMinionOrMountPtr(),
                isWatched: true
            )
            .GetAwaiter()
            .GetResult();
        _playerRelatedObjects[ObjectKind.Pet] = gameObjectHandlerFactory
            .Create(ObjectKind.Pet, () => dalamudUtil.GetPetPtr(), isWatched: true)
            .GetAwaiter()
            .GetResult();
        _playerRelatedObjects[ObjectKind.Companion] = gameObjectHandlerFactory
            .Create(ObjectKind.Companion, () => dalamudUtil.GetCompanionPtr(), isWatched: true)
            .GetAwaiter()
            .GetResult();
        ...
        Mediator.Subscribe<ClearCacheForObjectMessage>(
            this,
            (msg) =>
            {
                ...
                {
                    Logger.LogTrace(
                        "Received clear cache for {obj}, ignoring",
                        msg.ObjectToCreateFor
                    );
                    return;
                }
        ...
        Mediator.Subscribe<CustomizePlusMessage>(
            this,
            (msg) =>
            {
                if (_isZoning)
                    return;
                foreach (
                    var item in _playerRelatedObjects
        ...
        Mediator.Subscribe<HeelsOffsetMessage>(
            this,
            (msg) =>
            {
                if (_isZoning)
        ...
        Mediator.Subscribe<GlamourerChangedMessage>(
            this,
            (msg) =>
            {
                if (_isZoning)
                    return;
                var changedType = _playerRelatedObjects.FirstOrDefault(f =>
                    f.Value.Address == msg.Address
                );
        ...
        Mediator.Subscribe<HonorificMessage>(
            this,
            (msg) =>
            {
                if (_isZoning)
                ...
                if (
                    !string.Equals(
                        msg.NewHonorificTitle,
                        _playerData.HonorificData,
        ...
        Mediator.Subscribe<MoodlesMessage>(
            this,
            (msg) =>
            {
                if (_isZoning)
                ...
                if (
                    !default(KeyValuePair<ObjectKind, GameObjectHandler>).Equals(changedType)
                    && changedType.Key == ObjectKind.Player
                )
                {
        ...
        Mediator.Subscribe<PetNamesMessage>(
            this,
            (msg) =>
            {
                if (_isZoning)
                ...
                if (
                    !string.Equals(
                        msg.PetNicknamesData,
                        _playerData.PetNamesData,
                        StringComparison.Ordinal
                    )
                )
                {
    ...
    private void AddCacheToCreate(ObjectKind kind = ObjectKind.Player)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            Logger.LogTrace(
                "Debounce complete, inserting objects to create for: {obj}",
                string.Join(", ", _debouncedObjectCache)
            );
    ...
    private void ProcessCacheCreation()
    {
        ...
        if (
            _playerRelatedObjects.Any(p =>
                p.Value.CurrentDrawCondition
                    is not (
                        GameObjectHandler.DrawCondition.None
        ...
        _ = Task.Run(async () =>
        {
            ...
            Logger.LogDebug(
                "Creating Caches for {objectKinds}",
                string.Join(", ", objectKindsToCreate)
            );
            ...
            try
            {
                ...
                {
                    createdData[objectKind] = await _characterDataFactory
                        .BuildCharacterData(_playerRelatedObjects[objectKind], linkedCts.Token)
