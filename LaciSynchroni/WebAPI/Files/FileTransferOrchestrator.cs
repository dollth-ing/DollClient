...
public class FileTransferOrchestrator : DisposableMediatorSubscriberBase
{
    ...
    private int CurrentlyUsedDownloadSlots =>
        ...
        : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<DownloadReadyMessage>(
            this,
            (msg) =>
            {
    ...
    )
    {
        ...
        return await SendRequestInternalAsync(requestMessage, ct, httpCompletionOption)
    ...
    public long DownloadLimitPerSlot()
    {
        ...
        {
            SyncConfiguration.Models.DownloadSpeeds.Bps => limit,
            SyncConfiguration.Models.DownloadSpeeds.KBps => limit * 1024,
            SyncConfiguration.Models.DownloadSpeeds.MBps => limit * 1024 * 1024,
            _ => limit,
        };
        ...
        {
            Logger.LogWarning(
                "Calculated Bandwidth Limit is negative, returning Infinity: {value}, CurrentlyUsedDownloadSlots is {currentSlots}, "
                    + "DownloadSpeedLimit is {limit}, available slots: {avail}, current count: {count}",
                dividedLimit,
                currentUsedDlSlots,
                limit,
                avaialble,
    ...
    )
    {
        ...
        {
            var content = await ((JsonContent)requestMessage.Content)
                .ReadAsStringAsync()
                .ConfigureAwait(false);
            Logger.LogDebug(
                "Sending {method} to {uri} (Content: {content})",
        ...
        try
        {
            if (ct != null)
                return await _httpClient
                    .SendAsync(requestMessage, httpCompletionOption, ct.Value)
                    .ConfigureAwait(false);
            return await _httpClient
                .SendAsync(requestMessage, httpCompletionOption)
                .ConfigureAwait(false);
        }
        ...
        catch (Exception ex)
        {
            Logger.LogWarning(
                ex,
                "Error during SendRequestInternal for {uri}",
                requestMessage.RequestUri
            );
            throw;
        }
