...
public sealed class TokenProvider : IDisposable, IMediatorSubscriber
{
    ...
    )
    {
        ...
        Mediator.Subscribe<DalamudLogoutMessage>(
            this,
            (_) =>
            {
    ...
    )
    {
        ...
        try
        {
            if (!isRenewal)
            {
                ...
                {
                    tokenUri = AuthRoutes.AuthFullPath(
                        new Uri(
                            _serverManager
                                .CurrentApiUrl.Replace(
                                    "wss://",
                                    "https://",
                    ...
                    _logger.LogInformation(
                        "Sending SecretKey Request to server with auth {auth}",
                ...
                {
                    tokenUri = AuthRoutes.AuthWithOauthFullPath(
                        new Uri(
                            _serverManager
                                .CurrentApiUrl.Replace(
                                    "wss://",
                    ...
                    request.Headers.Authorization = new AuthenticationHeaderValue(
                        "Bearer",
                        identifier.SecretKeyOrOAuth
                    );
                    _logger.LogInformation(
                        "Sending OAuth Request to server with auth {auth}",
                        string.Join("", identifier.SecretKeyOrOAuth.Take(10))
                    );
            ...
            else
            {
                ...
                tokenUri = AuthRoutes.RenewTokenFullPath(
                    new Uri(
                        _serverManager
                            .CurrentApiUrl.Replace(
                                "wss://",
                ...
                if (isRenewal)
                    Mediator.Publish(
                        new NotificationMessage(
                            ...
                            "Your authentication token could not be renewed. Try reconnecting to Laci manually.",
                    ...
                    Mediator.Publish(
                        new NotificationMessage(
                            ...
                            "Your authentication token could not be generated. Check Laci Main UI (/laci in chat) to see the error message.",
                            NotificationType.Error
                        )
                    );
        ...
        _logger.LogDebug(
            "GetNewToken: Valid until {date}, ValidClaim until {date}",
            jwtToken.ValidTo,
            new DateTime(
                long.Parse(
                    jwtToken
                        .Claims.Single(c =>
                            string.Equals(c.Type, "expiration_date", StringComparison.Ordinal)
                        )
                        .Value
                ),
                DateTimeKind.Utc
        ...
        if (tokenTime <= dateTimeMinus10 || tokenTime >= dateTimePlus10)
        {
            ...
            Mediator.Publish(
                new NotificationMessage(
                    ...
                    "The clock of your computer is invalid. "
                        + "Laci will not function properly if the time zone is not set correctly. "
    ...
    private async Task<JwtIdentifier?> GetIdentifier()
    {
        ...
        try
        {
            var playerIdentifier = await _dalamudUtil
                .GetPlayerNameHashedAsync()
                .ConfigureAwait(false);
            ...
            {
                _logger.LogTrace(
                    "GetIdentifier: PlayerIdentifier was null, returning last identifier {identifier}",
                    _lastJwtIdentifier
                );
            ...
            {
                var (OAuthToken, UID) =
                    _serverManager.GetOAuth2(out _)
    ...
    public async Task<string?> GetOrUpdateToken(CancellationToken ct)
    {
        ...
        if (_tokenCache.TryGetValue(jwtIdentifier, out var token))
        {
            ...
            if (
                ...
                || jwt.ValidTo.Subtract(TimeSpan.FromMinutes(5)) > DateTime.UtcNow
            )
            {
                return token;
            }
    ...
    )
    {
        ...
        if (!forced)
        {
            if (
                jwt.ValidTo == DateTime.MinValue
                || jwt.ValidTo.Subtract(TimeSpan.FromDays(7)) > DateTime.Now
            )
                return true;
        ...
        var tokenUri = AuthRoutes.RenewOAuthTokenFullPath(
            new Uri(
                currentServer
                    .ServerUri.Replace("wss://", "https://", StringComparison.OrdinalIgnoreCase)
                    .Replace("ws://", "http://", StringComparison.OrdinalIgnoreCase)
            )
        );
        ...
        request.Headers.Authorization = new AuthenticationHeaderValue(
            "Bearer",
            oauth2.Value.OAuthToken
        );
        _logger.LogInformation(
            "Sending Request to server with auth {auth}",
        ...
        {
            _logger.LogWarning(
                "Could not renew OAuth2 Login token, error code {error}",
                result.StatusCode
            );
