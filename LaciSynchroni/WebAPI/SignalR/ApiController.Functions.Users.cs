...
public partial class ApiController
{
    ...
    public async Task UserAddPair(UserDto user)
    {
        if (!IsConnected)
            return;
        await _serverHub!.SendAsync(nameof(UserAddPair), user).ConfigureAwait(false);
    }

    public async Task UserDelete()
    {
        CheckConnection();
        await _serverHub!.SendAsync(nameof(UserDelete)).ConfigureAwait(false);
    ...
    public async Task<List<OnlineUserIdentDto>> UserGetOnlinePairs(CensusDataDto? censusDataDto)
    {
        return await _serverHub!
            .InvokeAsync<List<OnlineUserIdentDto>>(nameof(UserGetOnlinePairs), censusDataDto)
    ...
    {
        return await _serverHub!
            .InvokeAsync<List<UserFullPairDto>>(nameof(UserGetPairedClients))
            .ConfigureAwait(false);
    }
    ...
    public async Task<UserProfileDto> UserGetProfile(UserDto dto)
    {
        ...
        return await _serverHub!
            .InvokeAsync<UserProfileDto>(nameof(UserGetProfile), dto)
            .ConfigureAwait(false);
    }
    ...
    public async Task UserPushData(UserCharaDataMessageDto dto)
    {
        try
        {
            await _serverHub!.InvokeAsync(nameof(UserPushData), dto).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    public async Task SetBulkPermissions(BulkPermissionsDto dto)
    {
        ...
        try
        {
            await _serverHub!.InvokeAsync(nameof(SetBulkPermissions), dto).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    public async Task UserRemovePair(UserDto userDto)
    {
        if (!IsConnected)
            return;
        await _serverHub!.SendAsync(nameof(UserRemovePair), userDto).ConfigureAwait(false);
    }
    ...
    public async Task UserSetProfile(UserProfileDto userDescription)
    {
        if (!IsConnected)
            return;
        await _serverHub!.InvokeAsync(nameof(UserSetProfile), userDescription).ConfigureAwait(false);
    }
    ...
    public async Task UserUpdateDefaultPermissions(DefaultPermissionsDto defaultPermissionsDto)
    {
        CheckConnection();
        await _serverHub!
            .InvokeAsync(nameof(UserUpdateDefaultPermissions), defaultPermissionsDto)
            .ConfigureAwait(false);
    }

    private async Task PushCharacterDataInternal(
        CharacterData character,
        List<UserData> visibleCharacters
    )
    {
        Logger.LogInformation(
            "Pushing character data for {hash} to {charas}",
            character.DataHash.Value,
