...
public abstract class ConfigurationServiceBase<T> : IConfigService<T>
    where T : ISyncConfiguration
{
    ...
    {
        var configBackupFolder = Path.Join(
            ConfigurationDirectory,
            ConfigurationSaveService.BackupFolder
        );
    ...
    private async Task CheckForConfigUpdatesInternal()
    {
        ...
        {
            await Task.Delay(TimeSpan.FromSeconds(5), _periodicCheckCts.Token)
