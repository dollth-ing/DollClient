...
public class ConfigurationSaveService : IHostedService
{
    ...
    public ConfigurationSaveService(
        ILogger<ConfigurationSaveService> logger,
        IEnumerable<IConfigService<ISyncConfiguration>> configs
    )
    {
...
#pragma warning disable S3011 // Reflection should not be used to increase accessibility of classes, methods, or fields
        _saveMethod = GetType()
            .GetMethod(nameof(SaveConfig), BindingFlags.Instance | BindingFlags.NonPublic)!;
#pragma warning restore S3011 // Reflection should not be used to increase accessibility of classes, methods, or fields
    ...
    private async Task SaveConfig<T>(IConfigService<T> config)
        where T : ISyncConfiguration
    {
        ...
        try
        {
            ...
            var existingConfigs = Directory
                .EnumerateFiles(configBackupFolder, configNameSplit[0] + "*")
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogWarning(
                ex,
                "Could not create backup for {config}",
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(
                ex,
                "Error during config save of {config}",
