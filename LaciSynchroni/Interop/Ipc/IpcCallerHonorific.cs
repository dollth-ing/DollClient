...
public sealed class IpcCallerHonorific : IIpcCaller
{
    ...
    public IpcCallerHonorific(
        ILogger<IpcCallerHonorific> logger,
    ...
    )
    {
        ...
        _honorificGetLocalCharacterTitle = pi.GetIpcSubscriber<string>(
            "Honorific.GetLocalCharacterTitle"
        );
        _honorificClearCharacterTitle = pi.GetIpcSubscriber<int, object>(
            "Honorific.ClearCharacterTitle"
        );
        _honorificSetCharacterTitle = pi.GetIpcSubscriber<int, string, object>(
            "Honorific.SetCharacterTitle"
        );
        _honorificLocalCharacterTitleChanged = pi.GetIpcSubscriber<string, object>(
            "Honorific.LocalCharacterTitleChanged"
        );
    ...
    public async Task<string> GetTitle()
    {
        if (!APIAvailable)
            return string.Empty;
        string title = await _dalamudUtil
            .RunOnFrameworkThread(() => _honorificGetLocalCharacterTitle.InvokeFunc())
            .ConfigureAwait(false);
        ...
        try
        {
            await _dalamudUtil
                .RunOnFrameworkThread(() =>
                {
                    ...
                    {
                        string honorificData = string.IsNullOrEmpty(honorificDataB64)
                            ? string.Empty
                            : Encoding.UTF8.GetString(Convert.FromBase64String(honorificDataB64));
                        ...
                        {
                            _honorificSetCharacterTitle!.InvokeAction(
                                pc.ObjectIndex,
                                honorificData
                            );
                        }
    ...
    {
        string titleData = string.IsNullOrEmpty(titleJson)
            ? string.Empty
            : Convert.ToBase64String(Encoding.UTF8.GetBytes(titleJson));
