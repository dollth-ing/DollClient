...
public sealed class IpcCallerCustomize : IIpcCaller
{
    ...
    private readonly ICallGateSubscriber<
        ...
        string,
        (int, Guid?)
    > _customizePlusSetBodyScaleToCharacter;
    ...
    public IpcCallerCustomize(
        ILogger<IpcCallerCustomize> logger,
        IDalamudPluginInterface dalamudPluginInterface,
        DalamudUtilService dalamudUtil,
        SyncMediator syncMediator
    )
    {
        _customizePlusApiVersion = dalamudPluginInterface.GetIpcSubscriber<(int, int)>(
            "CustomizePlus.General.GetApiVersion"
        );
        _customizePlusGetActiveProfile = dalamudPluginInterface.GetIpcSubscriber<
            ushort,
            (int, Guid?)
        >("CustomizePlus.Profile.GetActiveProfileIdOnCharacter");
        _customizePlusGetProfileById = dalamudPluginInterface.GetIpcSubscriber<
            Guid,
            (int, string?)
        >("CustomizePlus.Profile.GetByUniqueId");
        _customizePlusRevertCharacter = dalamudPluginInterface.GetIpcSubscriber<ushort, int>(
            "CustomizePlus.Profile.DeleteTemporaryProfileOnCharacter"
        );
        _customizePlusSetBodyScaleToCharacter = dalamudPluginInterface.GetIpcSubscriber<
            ushort,
            string,
            (int, Guid?)
        >("CustomizePlus.Profile.SetTemporaryProfileOnCharacter");
        _customizePlusOnScaleUpdate = dalamudPluginInterface.GetIpcSubscriber<ushort, Guid, object>(
            "CustomizePlus.Profile.OnUpdate"
        );
        _customizePlusDeleteByUniqueId = dalamudPluginInterface.GetIpcSubscriber<Guid, int>(
    ...
    public async Task RevertAsync(nint character)
    {
        ...
        await _dalamudUtil
            .RunOnFrameworkThread(() =>
            {
                ...
                {
                    _logger.LogTrace(
                        "CustomizePlus reverting for {chara}",
                        c.Address.ToString("X")
                    );
        ...
        return await _dalamudUtil
            .RunOnFrameworkThread(() =>
            {
                ...
                if (gameObj is ICharacter c)
                {
                    ...
                    {
                        var result = _customizePlusSetBodyScaleToCharacter!.InvokeFunc(
                            c.ObjectIndex,
                            decodedScale
                        );
                        return result.Item2;
    ...
    {
        if (!APIAvailable || profileId == null)
            return;
