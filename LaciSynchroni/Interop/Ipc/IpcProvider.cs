...
public class IpcProvider : IHostedService, IMediatorSubscriber
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _loadFileProvider = _pi.GetIpcProvider<string, IGameObject, bool>(
            "LaciSynchroni.LoadMcdf"
        );
        _loadFileProvider.RegisterFunc(LoadMcdf);
        _loadFileAsyncProvider = _pi.GetIpcProvider<string, IGameObject, Task<bool>>(
            "LaciSynchroni.LoadMcdfAsync"
        );
        _loadFileAsyncProvider.RegisterFunc(LoadMcdfAsync);
        _handledGameAddresses = _pi.GetIpcProvider<List<nint>>(
    ...
    {
        _ = Task.Run(async () => await ApplyFileAsync(path, target).ConfigureAwait(false))
    ...
    {
        return _activeGameObjectHandlers
            .Where(g => g.Address != nint.Zero)
            .Select(g => g.Address)
            .Distinct()
            .ToList();
    }
