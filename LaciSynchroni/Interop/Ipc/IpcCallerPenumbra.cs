...
public sealed class IpcCallerPenumbra : DisposableMediatorSubscriberBase, IIpcCaller
{
        ...
        : base(logger, syncMediator)
    {
        ...
        _penumbraModSettingChanged = ModSettingChanged.Subscriber(
            pi,
            (change, arg1, arg, b) =>
            {
        ...
        _penumbraGameObjectResourcePathResolved = GameObjectResourcePathResolved.Subscriber(
            pi,
            ResourceLoaded
        );
        ...
        Mediator.Subscribe<PenumbraRedrawCharacterMessage>(
            this,
            (msg) =>
            {
    ...
    public void CheckAPI()
    {
        ...
        {
            var penumbraVersion = (
                _pi.InstalledPlugins.FirstOrDefault(p =>
                    string.Equals(p.InternalName, "Penumbra", StringComparison.OrdinalIgnoreCase)
            ...
            if (!penumbraAvailable && !_shownPenumbraUnavailable)
            {
                ...
                _syncMediator.Publish(
                    new NotificationMessage(
                        "Penumbra inactive",
                        "Your Penumbra installation is not active or out of date. Update Penumbra and/or the Enable Mods setting in Penumbra to continue to use Laci. If you just updated Penumbra, ignore this message.",
    ...
    public async Task AssignTemporaryCollectionAsync(ILogger logger, Guid collName, int idx)
    {
        ...
        await _dalamudUtil
            ...
            {
                var retAssign = _penumbraAssignTemporaryCollection.Invoke(
                    collName,
                    idx,
                    forceAssignment: true
                );
                logger.LogTrace(
                    "Assigning Temp Collection {collName} to index {idx}, Success: {ret}",
                    collName,
                    idx,
                    retAssign
                );
    ...
    public async Task ConvertTextureFiles(
        ILogger logger,
    ...
    )
    {
        ...
        foreach (var texture in textures)
        {
            ...
            logger.LogInformation(
                "Converting Texture {path} to {type}",
                texture.Key,
                TextureType.Bc7Tex
            );
            var convertTask = _penumbraConvertTextureFile.Invoke(
                texture.Key,
                texture.Key,
                TextureType.Bc7Tex,
                mipMaps: true
        ...
        await _dalamudUtil
            .RunOnFrameworkThread(async () =>
            {
                var gameObject = await _dalamudUtil
                    .CreateGameObjectAsync(
                        await _dalamudUtil.GetPlayerPointerAsync().ConfigureAwait(false)
                    )
                    .ConfigureAwait(false);
    ...
    public async Task<Guid> CreateTemporaryCollectionAsync(ILogger logger, string uid)
    {
        ...
        return await _dalamudUtil
            .RunOnFrameworkThread(() =>
            {
                ...
                var _error = _penumbraCreateNamedTemporaryCollection.Invoke(
                    "LaciSynchroni",
                    collName,
                    out var collId
                );
                ...
                {
                    logger.LogError(
                        "Failed to create temporary collection for {collName}. Penumbra returned the error code {_error} ({_errorCode}).",
                        collName,
                        nameof(_error),
                        _error
                    );
                ...
                logger.LogTrace(
                    "Creating Temp Collection {collName}, GUID: {collId}",
                    collName,
                    collId
                );
                return collId;
    ...
    public async Task<Dictionary<string, HashSet<string>>?> GetCharacterData(
        ILogger logger,
    ...
    )
    {
        ...
        try
        {
            ...
            await _redrawManager
                .PenumbraRedrawInternalAsync(
                    logger,
                    handler,
                    applicationId,
                    (chara) =>
    ...
    public async Task RemoveTemporaryCollectionAsync(
        ILogger logger,
        Guid applicationId,
        Guid collId
    )
    {
        ...
        await _dalamudUtil
            ...
            {
                logger.LogTrace(
                    "[{applicationId}] Removing temp collection for {collId}",
                    applicationId,
                    collId
                );
                var ret2 = _penumbraRemoveTemporaryCollection.Invoke(collId);
                logger.LogTrace(
                    "[{applicationId}] RemoveTemporaryCollection: {ret2}",
                    applicationId,
                    ret2
                );
            })
    ...
    public async Task<(string[] forward, string[][] reverse)> ResolvePathsAsync(
        string[] forward,
        string[] reverse
    )
    {
    ...
    public async Task SetManipulationDataAsync(
        ILogger logger,
        Guid applicationId,
        Guid collId,
    ...
    )
    {
        ...
        await _dalamudUtil
            .RunOnFrameworkThread(() =>
            {
                ...
                var retAdd = _penumbraAddTemporaryMod.Invoke(
                    "LaciChara_Meta",
                    collId,
                    [],
                    manipulationData,
                ...
                logger.LogTrace(
                    "[{applicationId}] Setting temp meta mod for {collId}, Success: {ret}",
                    applicationId,
                    collId,
                    retAdd
                );
    ...
    )
    {
        ...
        await _dalamudUtil
            .RunOnFrameworkThread(() =>
            {
                ...
                logger.LogTrace(
                    "[{applicationId}] Removing temp files mod for {collId}, Success: {ret}",
                    applicationId,
                    collId,
                    retRemove
                );
                ...
                logger.LogTrace(
                    "[{applicationId}] Setting temp files mod for {collId}, Success: {ret}",
                    applicationId,
                    collId,
                    retAdd
                );
    ...
    private void RedrawEvent(IntPtr objectAddress, int objectTableIndex)
    {
        ...
        if (
            _penumbraRedrawRequests.TryGetValue(objectAddress, out var redrawRequest)
            && redrawRequest
        )
        {
        ...
        {
            _syncMediator.Publish(
                new PenumbraRedrawMessage(objectAddress, objectTableIndex, wasRequested)
            );
        }
    }
