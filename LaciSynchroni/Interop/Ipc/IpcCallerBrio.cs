...
public sealed class IpcCallerBrio : IIpcCaller
{
    ...
    private readonly ICallGateSubscriber<
        ...
        bool,
        bool
    > _brioSetModelTransform;
    private readonly ICallGateSubscriber<
        IGameObject,
        (Vector3?, Quaternion?, Vector3?)
    > _brioGetModelTransform;
    ...
    public IpcCallerBrio(
        ILogger<IpcCallerBrio> logger,
    ...
    )
    {
        ...
        _brioSpawnActorAsync = dalamudPluginInterface.GetIpcSubscriber<
            bool,
            bool,
            bool,
            Task<IGameObject>
        >("Brio.Actor.SpawnExAsync");
        _brioDespawnActor = dalamudPluginInterface.GetIpcSubscriber<IGameObject, bool>(
            "Brio.Actor.Despawn"
        );
        _brioSetModelTransform = dalamudPluginInterface.GetIpcSubscriber<
            IGameObject,
            Vector3?,
            Quaternion?,
            Vector3?,
        ...
        _brioGetModelTransform = dalamudPluginInterface.GetIpcSubscriber<
            IGameObject,
            (Vector3?, Quaternion?, Vector3?)
        >("Brio.Actor.GetModelTransform");
        _brioGetPoseAsJson = dalamudPluginInterface.GetIpcSubscriber<IGameObject, string>(
            "Brio.Actor.Pose.GetPoseAsJson"
        );
        _brioSetPoseFromJson = dalamudPluginInterface.GetIpcSubscriber<
            IGameObject,
            string,
            bool,
            bool
        >("Brio.Actor.Pose.LoadFromJson");
        _brioFreezeActor = dalamudPluginInterface.GetIpcSubscriber<IGameObject, bool>(
            "Brio.Actor.Freeze"
        );
    ...
    public async Task<bool> DespawnActorAsync(nint address)
    {
        if (!APIAvailable)
            return false;
        var gameObject = await _dalamudUtilService
            .CreateGameObjectAsync(address)
            .ConfigureAwait(false);
        if (gameObject == null)
            return false;
        _logger.LogDebug("Despawning Brio Actor {actor}", gameObject.Name.TextValue);
        return await _dalamudUtilService
            .RunOnFrameworkThread(() => _brioDespawnActor.InvokeFunc(gameObject))
            .ConfigureAwait(false);
    }
    ...
    public async Task<bool> ApplyTransformAsync(nint address, WorldData data)
    {
        ...
        return await _dalamudUtilService
            .RunOnFrameworkThread(() =>
                _brioSetModelTransform.InvokeFunc(
                    gameObject,
    ...
    public async Task<WorldData> GetTransformAsync(nint address)
    {
        ...
        if (gameObject == null)
            return default;
        var data = await _dalamudUtilService
            .RunOnFrameworkThread(() => _brioGetModelTransform.InvokeFunc(gameObject))
            .ConfigureAwait(false);
    ...
    public async Task<string?> GetPoseAsync(nint address)
    {
        ...
        return await _dalamudUtilService
            .RunOnFrameworkThread(() => _brioGetPoseAsJson.InvokeFunc(gameObject))
            .ConfigureAwait(false);
    }
    ...
    public async Task<bool> SetPoseAsync(nint address, string pose)
    {
        ...
        var currentPose = await _dalamudUtilService
            .RunOnFrameworkThread(() => _brioGetPoseAsJson.InvokeFunc(gameObject))
            .ConfigureAwait(false);
        applicablePose["ModelDifference"] = JsonNode.Parse(
            JsonNode.Parse(currentPose)!["ModelDifference"]!.ToJsonString()
        );
