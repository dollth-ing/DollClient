...
public unsafe class BlockedCharacterHandler
{
    ...
    public BlockedCharacterHandler(
        ILogger<BlockedCharacterHandler> logger,
    ...
    public bool IsCharacterBlocked(nint ptr, out bool firstTime)
    {
        ...
        firstTime = true;
        var blockStatus = InfoProxyBlacklist
            .Instance()
            ->GetBlockResultType(combined.AccId, combined.ContentId);
        _logger.LogTrace("CharaPtr {ptr} is BlockStatus: {status}", ptr, blockStatus);
        if ((int)blockStatus == 0)
            return false;
        return _blockedCharacterCache[combined] =
            blockStatus != InfoProxyBlacklist.BlockResultType.NotBlocked;
    }
}