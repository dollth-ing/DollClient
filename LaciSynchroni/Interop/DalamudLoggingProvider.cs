...
public sealed class DalamudLoggingProvider : ILoggerProvider
{
    ...
    public ILogger CreateLogger(string categoryName)
    {
        ...
        {
            catName =
                string.Join("", catName.Take(6)) + "..." + string.Join("", catName.TakeLast(6));
        }
        else
        {
            catName =
                string.Join("", Enumerable.Range(0, 15 - catName.Length).Select(_ => " "))
                + catName;
        }

        return _loggers.GetOrAdd(
            catName,
            name => new DalamudLogger(name, _syncConfigService, _pluginLog, _hasModifiedGameFiles)
        );
    }
