...
internal sealed partial class CharaDataHubUi
{
    ...
    private void DrawEditCharaData(CharaDataFullExtendedDto? dataDto)
    {
        ...
        if (dataDto == null)
        {
            ...
            UiSharedService.DrawGroupedCenteredColorText(
                "Select an entry above to edit its data.",
        ...
        {
            UiSharedService.DrawGroupedCenteredColorText(
                "Something went awfully wrong and there's no update DTO. Try updating Character Data via the button above.",
                ImGuiColors.DalamudYellow
            );
            return;
        }
        ...
        int otherUpdates = 0;
        foreach (
            var item in _charaDataManager.OwnCharaData.Values.Where(v =>
                !string.Equals(v.Id, dataDto.Id, StringComparison.Ordinal)
            )
        )
        {
        ...
        if (
            canUpdate
            || otherUpdates > 0
            || (!_charaDataManager.CharaUpdateTask?.IsCompleted ?? false)
        )
        {
            ImGuiHelpers.ScaledDummy(5);
        }
        ...
        if (canUpdate || _charaDataManager.UploadTask != null)
        {
            ...
            UiSharedService.DrawGrouped(() =>
            {
                if (canUpdate)
                {
                    ...
                    using (
                        ImRaii.Disabled(
                            _charaDataManager.CharaUpdateTask != null
                                && !_charaDataManager.CharaUpdateTask.IsCompleted
                        )
                    )
                    {
                        if (
                            _uiSharedService.IconTextButton(
                                FontAwesomeIcon.ArrowCircleUp,
                                "Save to Server"
                            )
                        )
                        {
                    ...
                    {
                        UiSharedService.ColorTextWrapped(
                            "Updating data on server, please wait.",
                            ImGuiColors.DalamudYellow
                        );
                    }
                }
                ...
                if (!_charaDataManager.UploadTask?.IsCompleted ?? false)
                {
                    DisableDisabled(() =>
                    {
                        ...
                        {
                            UiSharedService.ColorTextWrapped(
                                _charaDataManager.UploadProgress.Value ?? string.Empty,
                                ImGuiColors.DalamudYellow
                            );
                        }
                        if (
                            (!_charaDataManager.UploadTask?.IsCompleted ?? false)
                            && _uiSharedService.IconTextButton(FontAwesomeIcon.Ban, "Cancel Upload")
                        )
                        {
                        ...
                        {
                            var color = UiSharedService.GetBoolColor(
                                _charaDataManager.UploadTask.Result.Success
                            );
                            UiSharedService.ColorTextWrapped(
                                _charaDataManager.UploadTask.Result.Output,
                                color
                            );
                        }
                    });
        ...
        if (otherUpdates > 0)
        {
            ...
            UiSharedService.DrawGrouped(() =>
            {
                ImGui.AlignTextToFramePadding();
                UiSharedService.ColorTextWrapped(
                    $"You have {otherUpdates} other entries with unsaved changes.",
                    ImGuiColors.DalamudYellow
                );
                ImGui.SameLine();
                using (
                ...
                )
                {
                    if (
                        _uiSharedService.IconTextButton(
                            FontAwesomeIcon.ArrowAltCircleUp,
                            "Save all to server"
                        )
                    )
                    {
    ...
    private void DrawEditCharaDataAccessAndSharing(CharaDataExtendedUpdateDto updateDto)
    {
        ...
        _uiSharedService.DrawHelpText(
            "You can control who has access to your character data based on the access restrictions."
                + UiSharedService.TooltipSeparator
                + "Specified: Only people and syncshells you directly specify in 'Specific Individuals / Syncshells' can access this character data"
                + Environment.NewLine
                + "Direct Pairs: Only people you have directly paired can access this character data"
                + Environment.NewLine
                + "All Pairs: All people you have paired can access this character data"
                + Environment.NewLine
                + "Everyone: Everyone can access this character data"
                + UiSharedService.TooltipSeparator
                + "Note: To access your character data the person in question requires to have the code. Exceptions for 'Shared' data, see 'Sharing' below."
                + Environment.NewLine
                + "Note: For 'Direct' and 'All Pairs' the pause state plays a role. Paused people will not be able to access your character data."
                + Environment.NewLine
        ...
        _uiSharedService.DrawHelpText(
            "This regulates how you want to distribute this character data."
                + UiSharedService.TooltipSeparator
                + "Code Only: People require to have the code to download this character data"
                + Environment.NewLine
                + "Shared: People that are allowed through 'Access Restrictions' will have this character data entry displayed in 'Shared with You' (it can also be accessed through the code)"
                + UiSharedService.TooltipSeparator
    ...
    private void DrawEditCharaDataAppearance(
        CharaDataFullExtendedDto dataDto,
    ...
    )
    {
        ...
        if (
            _uiSharedService.IconTextButton(
                ...
                "Set Appearance to Current Appearance"
            )
        )
        {
        ...
        _uiSharedService.DrawHelpText(
            "This will overwrite the appearance data currently stored in this Character Data entry with your current appearance."
        );
        ImGui.SameLine();
        using (
            ImRaii.Disabled(
                dataDto.HasMissingFiles
                    || !updateDto.IsAppearanceEqual
                    || _charaDataManager.DataApplicationTask != null
            )
        )
        {
            if (
                _uiSharedService.IconTextButton(
                    ...
                    "Preview Saved Apperance on Self"
                )
            )
            {
        ...
        _uiSharedService.DrawHelpText(
            "This will download and apply the saved character data to yourself. Once loaded it will automatically revert itself within 15 seconds."
                + UiSharedService.TooltipSeparator
        ...
        if (hasFiles && updateDto.IsAppearanceEqual)
        {
            ...
            ImGui.TextUnformatted(
                $"{dataDto.FileGamePaths.DistinctBy(k => k.HashOrFileSwap).Count()} unique file hashes (original upload: {dataDto.OriginalFiles.DistinctBy(k => k.HashOrFileSwap).Count()} file hashes)"
            );
            ImGui.NewLine();
            ImGui.SameLine(pos);
            ...
            {
                UiSharedService.ColorTextWrapped(
                    "All files to download this character data are present on the server",
            ...
            {
                UiSharedService.ColorTextWrapped(
                    $"{dataDto.MissingFiles.DistinctBy(k => k.HashOrFileSwap).Count()} files to download this character data are missing on the server",
                    ImGuiColors.DalamudRed
                );
                ImGui.NewLine();
                ImGui.SameLine(pos);
                if (
                    _uiSharedService.IconTextButton(
                        FontAwesomeIcon.ArrowCircleUp,
                        "Attempt to upload missing files and restore Character Data"
                    )
                )
                {
        ...
        else if (hasFiles && !updateDto.IsAppearanceEqual)
        {
            ...
            ImGui.SameLine();
            UiSharedService.ColorTextWrapped(
                "New data was set. It may contain files that require to be uploaded (will happen on Saving to server)",
                ImGuiColors.DalamudYellow
            );
        }
    ...
    private void DrawEditCharaDataGeneral(
        CharaDataFullExtendedDto dataDto,
        CharaDataExtendedUpdateDto updateDto
    )
    {
        ...
        _uiSharedService.DrawHelpText(
            "Description for this Character Data."
                + UiSharedService.TooltipSeparator
        ...
        _uiSharedService.DrawHelpText(
            "If expiration is enabled, the uploaded character data will be automatically deleted from the server at the specified date."
        );
        ...
        if (!UiSharedService.CtrlPressed())
        {
            UiSharedService.AttachToolTip(
                "Hold CTRL and click to delete the current data. This operation is irreversible."
            );
        }
    }
    ...
    private void DrawEditCharaDataPoses(CharaDataExtendedUpdateDto updateDto)
    {
        ...
        if (!_uiSharedService.IsInGpose && _charaDataManager.BrioAvailable)
        {
            ImGuiHelpers.ScaledDummy(5);
            UiSharedService.DrawGroupedCenteredColorText(
                "To attach pose and world data you need to be in GPose.",
                ImGuiColors.DalamudYellow
            );
            ImGuiHelpers.ScaledDummy(5);
        }
        else if (!_charaDataManager.BrioAvailable)
        {
            ImGuiHelpers.ScaledDummy(5);
            UiSharedService.DrawGroupedCenteredColorText(
                "To attach pose and world data Brio requires to be installed.",
                ImGuiColors.DalamudRed
            );
            ImGuiHelpers.ScaledDummy(5);
        }
        ...
        foreach (var pose in updateDto.PoseList)
        {
            ...
            if (pose.Id == null)
            {
                ...
                UiSharedService.AttachToolTip(
                    "This pose has not been added to the server yet. Save changes to upload this Pose data."
                );
            }
            ...
            if (poseHasChanges)
            {
                ...
                UiSharedService.AttachToolTip(
                    "This pose has changes that have not been saved to the server yet."
                );
            }
            ...
            else
            {
                ...
                _uiSharedService.IconText(
                    ...
                    UiSharedService.GetBoolColor(hasPoseData)
                );
                UiSharedService.AttachToolTip(
                    hasPoseData
                ...
                using (
                    ImRaii.Disabled(
                        !_uiSharedService.IsInGpose
                            || !(_charaDataManager.AttachingPoseTask?.IsCompleted ?? true)
                            || !_charaDataManager.BrioAvailable
                    )
                )
                {
                ...
                _uiSharedService.IconText(
                    ...
                    UiSharedService.GetBoolColor(hasWorldData)
                );
                var tooltipText = !hasWorldData
                    ? "This Pose has no world data attached."
                    : "This Pose has world data attached.";
                if (hasWorldData)
                {
                    tooltipText +=
                        UiSharedService.TooltipSeparator + "Click to show location on map";
                }
                UiSharedService.AttachToolTip(tooltipText);
                if (hasWorldData && ImGui.IsItemClicked(ImGuiMouseButton.Left))
                {
                    _dalamudUtilService.SetMarkerAndOpenMap(
                        position: new Vector3(
                            worldData.Value.PositionX,
                            worldData.Value.PositionY,
                            worldData.Value.PositionZ
                        ),
    ...
    private void DrawMcdOnline()
    {
        ...
        DrawHelpFoldout(
            "In this tab you can create, view and edit your own Character Data that is stored on the server."
                + Environment.NewLine
                + Environment.NewLine
        ...
        using (
            ImRaii.Disabled(
                (!_charaDataManager.GetAllDataTask?.IsCompleted ?? false)
                    || (
        ...
        )
        {
            if (
                _uiSharedService.IconTextButton(
                    ...
                    "Download your Character Data from Server"
                )
            )
            {
        ...
        )
        {
            if (table)
            {
                ...
                foreach (
                    var entry in _charaDataManager
                        .OwnCharaData.Values.Where(v =>
                        {
                            bool show = true;
                            if (!string.IsNullOrWhiteSpace(_createCodeFilter))
                            {
                                show &= v.FullId.Contains(
                                    _createCodeFilter,
                            ...
                            {
                                show &= !(
                                    !v.HasMissingFiles && !string.IsNullOrEmpty(v.GlamourerData)
                                );
                            }
                ...
                )
                {
                    ...
                    ImGui.TableNextColumn();
                    bool isDownloadable =
                        !entry.HasMissingFiles && !string.IsNullOrEmpty(entry.GlamourerData);
                    ...
                    if (ImGui.IsItemClicked())
                        SelectedDtoId = entry.Id;
                    UiSharedService.AttachToolTip(
                        count == 0 ? "No File data attached" : "Has File data attached"
                    );
                    ...
                    UiSharedService.AttachToolTip(
                        string.IsNullOrEmpty(entry.GlamourerData)
                            ? "No Glamourer data attached"
                            : "Has Glamourer data attached"
                    );
                    ...
                    if (ImGui.IsItemClicked())
                        SelectedDtoId = entry.Id;
                    UiSharedService.AttachToolTip(
                        string.IsNullOrEmpty(entry.CustomizeData)
                            ? "No Customize+ data attached"
                            : "Has Customize+ data attached"
                    );
                    ...
                    {
                        UiSharedService.AttachToolTip(
                            $"This entry will expire on {entry.ExpiryDate.ToLocalTime()}"
                        );
                    }
                }
            }
        ...
        using (
            ImRaii.Disabled(
                !_charaDataManager.Initialized
                    || _charaDataManager.DataCreationTask != null
                    || _charaDataManager.OwnCharaData.Count
                        == _charaDataManager.MaxCreatableCharaData
            )
        )
        {
        ...
        {
            UiSharedService.AttachToolTip(
                "You can only create new character data every few seconds. Please wait."
            );
        }
        if (!_charaDataManager.Initialized)
        {
            UiSharedService.AttachToolTip(
                "Please use the button \"Get Own Chara Data\" once before you can add new data entries."
            );
        }
        ...
        if (_charaDataManager.Initialized)
        {
            ...
            UiSharedService.TextWrapped(
                $"Chara Data Entries on Server: {_charaDataManager.OwnCharaData.Count}/{_charaDataManager.MaxCreatableCharaData}"
            );
            if (_charaDataManager.OwnCharaData.Count == _charaDataManager.MaxCreatableCharaData)
            {
                ImGui.AlignTextToFramePadding();
                UiSharedService.ColorTextWrapped(
                    "You have reached the maximum Character Data entries and cannot create more.",
                    ImGuiColors.DalamudYellow
                );
            }
        }
        ...
        if (
            _charaDataManager.DataCreationTask != null
            && !_charaDataManager.DataCreationTask.IsCompleted
        )
        {
            UiSharedService.ColorTextWrapped(
        ...
        {
            var color = _charaDataManager.DataCreationTask.Result.Success
                ? ImGuiColors.HealerGreen
                : ImGuiColors.DalamudRed;
            UiSharedService.ColorTextWrapped(
                _charaDataManager.DataCreationTask.Result.Output,
                color
            );
        }
        ...
        if (
            charaDataEntries != _dataEntries
            && _selectNewEntry
            && _charaDataManager.OwnCharaData.Any()
        )
        {
            SelectedDtoId = _charaDataManager
                .OwnCharaData.OrderBy(o => o.Value.CreatedDate)
                .Last()
                .Value.Id;
    ...
    private void DrawSpecific(CharaDataExtendedUpdateDto updateDto)
    {
        UiSharedService.DrawTree(
            "Access for Specific Individuals / Syncshells",
            () =>
            {
                using (ImRaii.PushId("user"))
                {
                    using (ImRaii.Group())
                    {
                        InputComboHybrid(
                            "##AliasToAdd",
                            "##AliasToAddPicker",
                            ref _specificIndividualAdd,
                            _pairManager.PairsWithGroups.Keys,
                            static pair =>
                                (
                                    pair.UserData.UID,
                                    pair.UserData.Alias,
                        ...
                        using (
                            ImRaii.Disabled(
                                string.IsNullOrEmpty(_specificIndividualAdd)
                                    || updateDto.UserList.Any(f =>
                                        string.Equals(
                                            f.UID,
                                            _specificIndividualAdd,
                        ...
                        _uiSharedService.DrawHelpText(
                            "Users added to this list will be able to access this character data regardless of your pause or pair state with them."
                                + UiSharedService.TooltipSeparator
                                + "Note: Mistyped entries will be automatically removed on updating data to server."
                        );

                        using (
                            var lb = ImRaii.ListBox(
                                "Allowed Individuals",
                                new(200 * ImGuiHelpers.GlobalScale, 200 * ImGuiHelpers.GlobalScale)
                            )
                        )
                        {
                            foreach (var user in updateDto.UserList)
                            {
                                var userString = string.IsNullOrEmpty(user.Alias)
                                    ? user.UID
                                    : $"{user.Alias} ({user.UID})";
                                if (
                                    ImGui.Selectable(
                                        ...
                                        string.Equals(
                                            user.UID,
                                            _selectedSpecificUserIndividual,
                                            StringComparison.Ordinal
                                        )
                                    )
                                )
                        ...
                        using (
                            ImRaii.Disabled(string.IsNullOrEmpty(_selectedSpecificUserIndividual))
                        )
                        {
                            if (
                                _uiSharedService.IconTextButton(
                        ...
                        using (ImRaii.Disabled(!UiSharedService.CtrlPressed()))
                        {
                            if (
                                _uiSharedService.IconTextButton(
                                    FontAwesomeIcon.ExclamationTriangle,
                                    "Apply current Allowed Individuals to all MCDO entries"
                                )
                            )
                            {
                                foreach (
                                    var own in _charaDataManager.OwnCharaData.Values.Where(k =>
                                        !string.Equals(k.Id, updateDto.Id, StringComparison.Ordinal)
                                    )
                                )
                                {
                                    var otherUpdateDto = _charaDataManager.GetUpdateDto(own.Id);
                                    if (otherUpdateDto == null)
                                        continue;
                                    foreach (
                                        var user in otherUpdateDto
                                            .UserList.Select(k => k.UID)
                                            .Concat(otherUpdateDto.AllowedUsers ?? [])
                                    ...
                                    foreach (
                                        var user in updateDto
                                            .UserList.Select(k => k.UID)
                                            .Concat(updateDto.AllowedUsers ?? [])
                                            .Distinct(StringComparer.Ordinal)
                                            .ToList()
                                    )
                                    {
                        ...
                        UiSharedService.AttachToolTip(
                            "This will apply the current list of allowed specific individuals to ALL of your MCDO entries."
                                + UiSharedService.TooltipSeparator
                                + "Hold CTRL to enable."
                        );
                    }
                ...
                using (ImRaii.PushId("group"))
                {
                    using (ImRaii.Group())
                    {
                        InputComboHybrid(
                            "##GroupAliasToAdd",
                            "##GroupAliasToAddPicker",
                            ref _specificGroupAdd,
                            _pairManager.Groups.Keys,
                            group =>
                                (
                                    group.GID,
                                    group.Alias,
                                    group.AliasOrGID,
                                    _serverConfigurationManager.GetNoteForGid(group.GID)
                                )
                        );
                        ImGui.SameLine();
                        using (
                            ImRaii.Disabled(
                                string.IsNullOrEmpty(_specificGroupAdd)
                                    || updateDto.GroupList.Any(f =>
                                        string.Equals(
                                            f.GID,
                                            _specificGroupAdd,
                                            StringComparison.Ordinal
                        ...
                        _uiSharedService.DrawHelpText(
                            "Users in Syncshells added to this list will be able to access this character data regardless of your pause or pair state with them."
                                + UiSharedService.TooltipSeparator
                                + "Note: Mistyped entries will be automatically removed on updating data to server."
                        );
                        ...
                        )
                        {
                            ...
                            {
                                var userString = string.IsNullOrEmpty(group.Alias)
                                    ? group.GID
                                    : $"{group.Alias} ({group.GID})";
                                if (
                                    ImGui.Selectable(
                                        userString,
                                        string.Equals(
                        ...
                        using (
                            ImRaii.Disabled(string.IsNullOrEmpty(_selectedSpecificGroupIndividual))
                        )
                        {
                            if (
                                _uiSharedService.IconTextButton(
                                    FontAwesomeIcon.Trash,
                                    "Remove selected Syncshell"
                                )
                            )
                            {
                        ...
                        using (ImRaii.Disabled(!UiSharedService.CtrlPressed()))
                        {
                            if (
                                _uiSharedService.IconTextButton(
                                    FontAwesomeIcon.ExclamationTriangle,
                                    "Apply current Allowed Syncshells to all MCDO entries"
                                )
                            )
                            {
                                foreach (
                                ...
                                )
                                {
                                    ...
                                    if (otherUpdateDto == null)
                                        continue;
                                    foreach (
                                        var group in otherUpdateDto
                                            .GroupList.Select(k => k.GID)
                                            .Concat(otherUpdateDto.AllowedGroups ?? [])
                                            .Distinct(StringComparer.Ordinal)
                                            .ToList()
                                    )
                                    {
                                    ...
                                    foreach (
                                        var group in updateDto
                                            .GroupList.Select(k => k.GID)
                                            .Concat(updateDto.AllowedGroups ?? [])
                                            .Distinct(StringComparer.Ordinal)
                                            .ToList()
                                    )
                                    {
                        ...
                        UiSharedService.AttachToolTip(
                            "This will apply the current list of allowed specific syncshells to ALL of your MCDO entries."
                                + UiSharedService.TooltipSeparator
                                + "Hold CTRL to enable."
                        );
                    }
    ...
    private void InputComboHybrid<T>(
        string inputId,
        string comboId,
        ref string value,
        IEnumerable<T> comboEntries,
    ...
    )
    {
        ...
        using var combo = ImRaii.Combo(
            comboId,
            string.Empty,
            ImGuiComboFlags.NoPreview | ImGuiComboFlags.PopupAlignLeft
        );
        if (!combo)
        {
            return;
        ...
        if (
            _openComboHybridEntries is null
            || !string.Equals(_openComboHybridId, comboId, StringComparison.Ordinal)
        )
        {
            ...
            _openComboHybridEntries = comboEntries
                .Select(parseEntry)
                .Where(entry =>
                    entry.Id.Contains(valueSnapshot, StringComparison.OrdinalIgnoreCase)
                    || (
                        entry.Alias is not null
                        && entry.Alias.Contains(valueSnapshot, StringComparison.OrdinalIgnoreCase)
                    )
                    || (
                        entry.Note is not null
                        && entry.Note.Contains(valueSnapshot, StringComparison.OrdinalIgnoreCase)
                    )
                )
                .OrderBy(
                    entry =>
                        entry.Note is null ? entry.AliasOrId : $"{entry.Note} ({entry.AliasOrId})",
        ...
        var width =
            ...
            - (_openComboHybridEntries.Length > 8 ? ImGui.GetStyle().ScrollbarSize : 0);
        foreach (var (id, alias, aliasOrId, note) in _openComboHybridEntries)
        {
            var selected =
                !string.IsNullOrEmpty(value)
                && (
                    string.Equals(id, value, StringComparison.Ordinal)
                    || string.Equals(alias, value, StringComparison.Ordinal)
                );
            using var font = ImRaii.PushFont(UiBuilder.MonoFont, note is null);
            if (
                ImGui.Selectable(
                    note is null ? aliasOrId : $"{note} ({aliasOrId})",
                    selected,
