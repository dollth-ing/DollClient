...
internal partial class CharaDataHubUi
{
    private void DrawNearbyPoses()
    {
        ...
        DrawHelpFoldout(
            "This tab will show you all Shared World Poses nearby you."
                + Environment.NewLine
                + Environment.NewLine
                + "Shared World Poses are poses in character data that have world data attached to them and are set to shared. "
                + "This means that all data that is in 'Shared with You' that has a pose with world data attached to it will be shown here if you are nearby."
                + Environment.NewLine
                + "By default all poses that are shared will be shown. Poses taken in housing areas will by default only be shown on the correct server and location."
                + Environment.NewLine
                + Environment.NewLine
                + "Shared World Poses will appear in the world as floating wisps, as well as in the list below. You can mouse over a Shared World Pose in the list for it to get highlighted in the world."
                + Environment.NewLine
                + Environment.NewLine
                + "You can apply Shared World Poses to yourself or spawn the associated character to pose with them."
                + Environment.NewLine
                + Environment.NewLine
        ...
        UiSharedService.DrawTree(
            ...
            () =>
            {
                ...
                _uiSharedService.DrawHelpText(
                    "Toggling this off will show you the location of all shared Poses with World Data from all Servers"
                );
                ...
                _uiSharedService.DrawHelpText(
                    "Toggling this on will also show you the location of your own Poses"
                );
                ...
                _uiSharedService.DrawHelpText(
                    "This will display all poses in their location regardless of housing limitations. (Ignoring Ward, Plot, Room)"
                ...
                _uiSharedService.DrawHelpText(
                    "When enabled, Laci will draw floating wisps where other's poses are in the world."
                );
                ...
                _uiSharedService.DrawHelpText(
                    "This setting allows you to change the maximum distance in which poses will be shown. Set it to the maximum if you want to see all poses on the current map."
                );
                ...
                _uiSharedService.DrawHelpText(
                    "This will allow Laci to continue the calculation of position of wisps etc. active outside of the 'Poses Nearby' tab."
                        + UiSharedService.TooltipSeparator
        ...
        if (!_uiSharedService.IsInGpose)
        {
            ...
            UiSharedService.DrawGroupedCenteredColorText(
                "Spawning and applying pose data is only available in GPose.",
        ...
        {
            UiSharedService.DrawGroupedCenteredColorText(
                "No Shared World Poses found nearby.",
                ImGuiColors.DalamudYellow
            );
        }
        ...
        foreach (var pose in _charaDataNearbyManager.NearbyData.OrderBy(v => v.Value.Distance))
        {
            ...
            {
                string? userNote = _serverConfigurationManager.GetNoteForUid(
                    pose.Key.MetaInfo.Uploader.UID
                );
                var noteText = pose.Key.MetaInfo.IsOwnData
                    ? "YOU"
                    : (
                        userNote == null
                            ? pose.Key.MetaInfo.Uploader.AliasOrUID
                            : $"{userNote} ({pose.Key.MetaInfo.Uploader.AliasOrUID})"
                    );
                ...
                UiSharedService.TextWrapped(
                    pose.Key.Description ?? "No Pose Description was set",
                    circleOriginX
                );
                ...
                UiSharedService.AttachToolTip(
                    "Click to open corresponding map and set map marker"
                        + UiSharedService.TooltipSeparator
                ...
                if (_uiSharedService.IsInGpose)
                {
                    GposePoseAction(
                        ...
                        $"Apply pose and position to {CharaName(_gposeTarget)}",
                    ...
                    GposeMetaInfoAction(
                        (_) =>
                        {
                            ...
                            {
                                _charaDataManager.SpawnAndApplyWorldTransform(
                                    pose.Key.MetaInfo,
                                    pose.Key
                                );
                            }
                        },
                        "Spawn actor and apply pose and position",
                        pose.Key.MetaInfo,
                        _hasValidGposeTarget,
                        true
                    );
            ...
            var circleCenter = new Vector2(
                windowPos.X + circleOriginX + circleRadius - scrollX,
                windowPos.Y + pos.Y + circleRadius + circleOffsetY - scrollY
            );
            var rads = pose.Value.Direction * (Math.PI / 180);

            float halfConeAngleRadians = 15f * (float)Math.PI / 180f;
            Vector2 baseDir1 = new Vector2(
                (float)Math.Sin(rads - halfConeAngleRadians),
                -(float)Math.Cos(rads - halfConeAngleRadians)
            );
            Vector2 baseDir2 = new Vector2(
                (float)Math.Sin(rads + halfConeAngleRadians),
                -(float)Math.Cos(rads + halfConeAngleRadians)
            );
            ...
            drawList.AddTriangleFilled(
                ...
                coneBase1,
                coneBase2,
                UiSharedService.Color(ImGuiColors.ParsedGreen)
            );
            drawList.AddCircle(
                circleCenter,
                circleDiameter / 2,
            ...
            drawList.AddText(
                new Vector2(circleCenter.X - textSize.X / 2, circleCenter.Y + textSize.Y / 3f),
                UiSharedService.Color(ImGuiColors.DalamudWhite),
                distance
            );
        ...
        using (
            ImRaii.Disabled(
                _charaDataManager.GetAllDataTask != null
                    || (
                        _charaDataManager.GetSharedWithYouTimeoutTask != null
                        && !_charaDataManager.GetSharedWithYouTimeoutTask.IsCompleted
                    )
            )
        )
        {
            if (
                _uiSharedService.IconTextButton(
                    ...
                    "Update Data Shared With You"
                )
            )
            {
                _ = _charaDataManager
                    .GetAllSharedData(_disposalCts.Token)
                    .ContinueWith(u => UpdateFilteredItems());
            }
        }
        if (
            _charaDataManager.GetSharedWithYouTimeoutTask != null
