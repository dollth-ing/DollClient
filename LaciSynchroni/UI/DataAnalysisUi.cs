...
public class DataAnalysisUi : WindowMediatorSubscriberBase
{
    ...
    private Dictionary<
        ObjectKind,
        Dictionary<string, CharacterAnalyzer.FileDataEntry>
    >? _cachedAnalysis;
    ...
    public DataAnalysisUi(
        ILogger<DataAnalysisUi> logger,
        ...
        : base(logger, mediator, "Laci Character Data Analysis", performanceCollectorService)
    {
        ...
        Mediator.Subscribe<CharacterDataAnalyzedMessage>(
            this,
            (_) =>
            {
                _hasUpdate = true;
        ...
        SizeConstraints = new()
        {
            MinimumSize = new() { X = 800, Y = 600 },
            MaximumSize = new() { X = 3840, Y = 2160 },
        };
        ...
        if (_conversionTask != null && !_conversionTask.IsCompleted)
        {
            ...
            if (ImGui.BeginPopupModal("BC7 Conversion in Progress"))
            {
                ImGui.TextUnformatted(
                    "BC7 Conversion in progress: "
                        + _conversionCurrentFileProgress
                        + "/"
                        + _texturesToConvert.Count
                );
                UiSharedService.TextWrapped("Current file: " + _conversionCurrentFileName);
                if (
                    _uiSharedService.IconTextButton(FontAwesomeIcon.StopCircle, "Cancel conversion")
                )
                {
            ...
            && _conversionTask.IsCompleted
            && _texturesToConvert.Count > 0
        )
        {
            _conversionTask = null;
    ...
    private void DrawStoredData()
    {
        UiSharedService.DrawTree(
            ...
            () =>
            {
                UiSharedService.TextWrapped(
                    "This tab allows you to see which transient files are attached to your character."
                );
                UiSharedService.TextWrapped(
                    "Transient files are files that cannot be resolved to your character permanently. Laci gathers these files in the background while you execute animations, VFX, sound effects, etc."
                );
                UiSharedService.TextWrapped(
                    "When sending your character data to others, Laci will combine the files listed in \"All Jobs\" and the corresponding currently used job."
                );
                UiSharedService.TextWrapped(
                    "The purpose of this tab is primarily informational for you to see which files you are carrying with you. You can remove added game paths, however if you are using the animations etc. again, "
        ...
        using (ImRaii.Group())
        {
            ...
            using (ImRaii.ListBox("##characters", new Vector2(200, availableContentRegion.Y)))
            {
                foreach (var entry in config)
                {
                    ...
                    if (
                        !_uiSharedService.WorldData.TryGetValue(
                            ushort.Parse(name[1]),
                            out var worldname
                        )
                    )
        ...
        ImGui.SameLine();
        bool selectedData =
            config.TryGetValue(_selectedStoredCharacter, out var transientStorage)
            ...
            using (ImRaii.ListBox("##data", new Vector2(150, availableContentRegion.Y)))
            {
                if (selectedData)
                {
                    if (
                        ImGui.Selectable(
                            "All Jobs",
                            string.Equals(_selectedJobEntry, "alljobs", StringComparison.Ordinal)
                        )
                    )
                    {
        ...
        using (ImRaii.Group())
        {
            var selectedList = string.Equals(_selectedJobEntry, "alljobs", StringComparison.Ordinal)
                ...
                : (
                    string.IsNullOrEmpty(_selectedJobEntry)
                        ? []
                        : config[_selectedStoredCharacter].JobSpecificCache[
            ...
            using (ImRaii.Disabled(string.IsNullOrEmpty(_selectedJobEntry)))
            {
                ...
                if (
                    _uiSharedService.IconTextButton(
                        ...
                        "Resolve Game Paths to used File Paths"
                    )
                )
                {
                    _ = Task.Run(async () =>
                    {
                        ...
                        var resolved = await _ipcManager
                            .Penumbra.ResolvePathsAsync(paths, [])
                ...
                UiSharedService.AttachToolTip(
                    "Hold CTRL to delete all game paths from the displayed list"
                        + UiSharedService.TooltipSeparator
                        + "You usually do not need to do this. All animation and VFX data will be automatically handled through Laci."
                );
                ImGuiHelpers.ScaledDummy(5);
                ...
                )
                {
                    if (dataTable)
                    {
                        ...
                        {
                            if (
                                !string.IsNullOrWhiteSpace(_filterGamePath)
                                && !entry.Contains(
                                    _filterGamePath,
                                ...
                                continue;
                            bool hasFileResolve = _filePathResolve.TryGetValue(
                                entry,
                                out var filePath
                            );
                            ...
                            bool isSelected = _storedPathsToRemove.Contains(
                                entry,
                            ...
                            else
                            {
                                ImGui.TextUnformatted("-");
                                UiSharedService.AttachToolTip(
                                    "Resolve Game Paths to used File Paths to display the associated file paths."
                                );
                            }
                        }
                    }
    ...
    private void DrawRecording()
    {
        UiSharedService.DrawTree(
            ...
            {
                UiSharedService.TextWrapped(
                    "This tab allows you to attempt to fix mods that do not sync correctly, especially those with modded models and animations."
                        + Environment.NewLine
                        + Environment.NewLine
                        + "To use this, start the recording, execute one or multiple emotes/animations you want to attempt to fix and check if new data appears in the table below."
                        + Environment.NewLine
                        + "If it doesn't, Laci is not able to catch the data or already has recorded the animation files (check 'Show previously added transient files' to see if not all is already present)."
                        + Environment.NewLine
                        + Environment.NewLine
                ...
                UiSharedService.DrawGroupedCenteredColorText(
                    "Important Note: If you need to fix an animation that should apply across multiple jobs, you need to repeat this process with at least one additional job, "
                ...
                ImGuiHelpers.ScaledDummy(5);
                UiSharedService.DrawGroupedCenteredColorText(
                    "WARNING: WHILE RECORDING TRANSIENT DATA, DO NOT CHANGE YOUR APPEARANCE, ENABLED MODS OR ANYTHING. JUST DO THE ANIMATION(S) OR WHATEVER YOU NEED DOING AND STOP THE RECORDING.",
        ...
        if (_transientResourceManager.IsTransientRecording)
        {
            ...
            UiSharedService.ColorText(
                $"RECORDING - Time Remaining: {_transientResourceManager.RecordTimeRemaining.Value}",
                ImGuiColors.DalamudYellow
            );
            ImGuiHelpers.ScaledDummy(5);
            UiSharedService.DrawGroupedCenteredColorText(
                "DO NOT CHANGE YOUR APPEARANCE OR MODS WHILE RECORDING, YOU CAN ACCIDENTALLY MAKE SOME OF YOUR APPEARANCE RELATED MODS PERMANENT.",
                ImGuiColors.DalamudRed,
                800
            );
        }
        ...
        ImGuiHelpers.ScaledDummy(5);
        ImGui.Checkbox(
            "Show previously added transient files in the recording",
            ref _showAlreadyAddedTransients
        );
        _uiSharedService.DrawHelpText(
            "Use this only if you want to see what was previously already caught by Laci"
        );
        ImGuiHelpers.ScaledDummy(5);

        using (
            ImRaii.Disabled(
                _transientResourceManager.IsTransientRecording
                    || _transientResourceManager.RecordedTransients.All(k => !k.AddTransient)
                    || !_acknowledgeReview
            )
        )
        {
        ...
        if (_transientResourceManager.RecordedTransients.Any(k => !k.AlreadyTransient))
        {
            ImGuiHelpers.ScaledDummy(5);
            UiSharedService.DrawGroupedCenteredColorText(
                "Please review the recorded mod files before saving and deselect files that got into the recording on accident.",
                ImGuiColors.DalamudYellow
            );
            ImGuiHelpers.ScaledDummy(5);
        }
        ...
        using var table = ImRaii.Table(
            "Recorded Transients",
            4,
            ImGuiTableFlags.ScrollY | ImGuiTableFlags.SizingFixedFit | ImGuiTableFlags.RowBg
        );
        if (table)
        {
            int id = 0;
    ...
    private void DrawAnalysis()
    {
        UiSharedService.DrawTree(
            ...
            {
                UiSharedService.TextWrapped(
                    "This tab shows you all files and their sizes that are currently in use through your character and associated entities in Laci"
                );
            }
        );
        ...
        if (isAnalyzing)
        {
            UiSharedService.ColorTextWrapped(
                $"Analyzing {_characterAnalyzer.CurrentFile}/{_characterAnalyzer.TotalFiles}",
                ImGuiColors.DalamudYellow
            );
        ...
        else
        {
            ...
            {
                UiSharedService.ColorTextWrapped(
                    "Some entries in the analysis have file size not determined yet, press the button below to analyze your current data",
                    ImGuiColors.DalamudYellow
                );
                if (
                    _uiSharedService.IconTextButton(
                        FontAwesomeIcon.PlayCircle,
                        "Start analysis (missing entries)"
                    )
                )
                {
            ...
            else
            {
                if (
                    _uiSharedService.IconTextButton(
                        FontAwesomeIcon.PlayCircle,
                        "Start analysis (recalculate all entries)"
                    )
                )
                {
        ...
        {
            string text = "";
            var groupedfiles = _cachedAnalysis
                .Values.SelectMany(f => f.Values)
            ...
            text = string.Join(
                ...
                groupedfiles
                    ...
                    .Select(f =>
                        ...
                        + f.Count()
                        + " files, size: "
                        + UiSharedService.ByteToString(f.Sum(v => v.OriginalSize))
                        + ", compressed: "
        ...
        ImGui.SameLine();
        ImGui.TextUnformatted(
            UiSharedService.ByteToString(
                _cachedAnalysis!.Sum(c => c.Value.Sum(c => c.Value.OriginalSize))
            )
        );
        ...
        ImGui.TextUnformatted(
            UiSharedService.ByteToString(
                _cachedAnalysis!.Sum(c => c.Value.Sum(c => c.Value.CompressedSize))
            )
        );
        ImGui.TextUnformatted(
            $"Total modded model triangles: {_cachedAnalysis.Sum(c => c.Value.Sum(f => f.Value.Triangles))}"
        );
        ImGui.Separator();
        ...
        foreach (var kvp in _cachedAnalysis)
        {
            ...
            if (kvp.Value.Any(f => !f.Value.IsComputed))
                tabText += " (!)";
            using var tab = ImRaii.TabItem(tabText + "###" + kvp.Key.ToString());
            if (tab.Success)
            {
                var groupedfiles = kvp
                ...
                ImGui.SameLine();
                ImGui.TextUnformatted(
                    UiSharedService.ByteToString(kvp.Value.Sum(c => c.Value.OriginalSize))
                );
                ImGui.TextUnformatted($"{kvp.Key} size (compressed for up/download only):");
                ImGui.SameLine();
                ImGui.TextUnformatted(
                    UiSharedService.ByteToString(kvp.Value.Sum(c => c.Value.CompressedSize))
                );
                ImGui.Separator();

                var vramUsage = groupedfiles.SingleOrDefault(v =>
                    string.Equals(v.Key, "tex", StringComparison.Ordinal)
                );
                if (vramUsage != null)
                {
                    ...
                    if (
                        _playerPerformanceConfig.Current.WarnOnExceedingThresholds
                        || _playerPerformanceConfig.Current.ShowPerformanceIndicator
                    )
                    {
                        using var _ = ImRaii.PushIndent(10f);
                        var currentVramWarning = _playerPerformanceConfig
                            .Current
                            .VRAMSizeWarningThresholdMiB;
                        ImGui.TextUnformatted(
                            $"Configured VRAM warning threshold: {currentVramWarning} MiB."
                        );
                        if (currentVramWarning * 1024 * 1024 < actualVramUsage)
                        {
                            UiSharedService.ColorText(
                                $"You exceed your own threshold by "
                ...
                {
                    using var _ = ImRaii.PushIndent(10f);
                    var currentTriWarning = _playerPerformanceConfig
                        .Current
                        .TrisWarningThresholdThousands;
                    ImGui.TextUnformatted(
                        $"Configured triangle warning threshold: {currentTriWarning * 1000} triangles."
                    );
                ...
                foreach (
                    IGrouping<string, CharacterAnalyzer.FileDataEntry>? fileGroup in groupedfiles
                )
                {
                    string fileGroupText = fileGroup.Key + " [" + fileGroup.Count() + "]";
                    var requiresCompute = fileGroup.Any(k => !k.IsComputed);
                    using var tabcol = ImRaii.PushColor(
                        ImGuiCol.Tab,
                    ...
                    using (
                        var textcol = ImRaii.PushColor(
                            ImGuiCol.Text,
                            UiSharedService.Color(new(0, 0, 0, 1)),
                            requiresCompute
                                && !string.Equals(
                                    _selectedFileTypeTab,
                                    fileGroup.Key,
                                    StringComparison.Ordinal
                                )
                        )
                    ...
                    ImGui.SameLine();
                    ImGui.TextUnformatted(
                        UiSharedService.ByteToString(fileGroup.Sum(c => c.OriginalSize))
                    );

                    ImGui.TextUnformatted(
                        $"{fileGroup.Key} files size (compressed for up/download only):"
                    );
                    ImGui.SameLine();
                    ImGui.TextUnformatted(
                        UiSharedService.ByteToString(fileGroup.Sum(c => c.CompressedSize))
                    );
                    ...
                    if (string.Equals(_selectedFileTypeTab, "tex", StringComparison.Ordinal))
                    {
                        ...
                        {
                            UiSharedService.ColorText(
                                "WARNING BC7 CONVERSION:",
                                ImGuiColors.DalamudYellow
                            );
                            ImGui.SameLine();
                            UiSharedService.ColorText(
                                "Converting textures to BC7 is irreversible!",
                                ImGuiColors.DalamudRed
                            );
                            UiSharedService.ColorTextWrapped(
                            ...
                            {
                                _conversionCancellationTokenSource =
                                    _conversionCancellationTokenSource.CancelRecreate();
                                _conversionTask = _ipcManager.Penumbra.ConvertTextureFiles(
                                    _logger,
                                    _texturesToConvert,
                                    _conversionProgress,
        ...
        if (
            _cachedAnalysis[_selectedObjectTab]
                .TryGetValue(_selectedHash, out CharacterAnalyzer.FileDataEntry? item)
        )
        {
    ...
    private void DrawTable(IGrouping<string, CharacterAnalyzer.FileDataEntry> fileGroup)
    {
        ...
        if (sortSpecs.SpecsDirty)
        {
            ...
            if (idx == 0 && sortSpecs.Specs.SortDirection == ImGuiSortDirection.Ascending)
                _cachedAnalysis![_selectedObjectTab] = _cachedAnalysis[_selectedObjectTab]
                    .OrderBy(k => k.Key, StringComparer.Ordinal)
            ...
            if (
                ...
                _cachedAnalysis![_selectedObjectTab] = _cachedAnalysis[_selectedObjectTab]
                    .OrderByDescending(k => k.Value.Triangles)
                    .ToDictionary(d => d.Key, d => d.Value, StringComparer.Ordinal);
            if (
                string.Equals(fileGroup.Key, "tex", StringComparison.Ordinal)
                && idx == 5
                && sortSpecs.Specs.SortDirection == ImGuiSortDirection.Ascending
            )
                _cachedAnalysis![_selectedObjectTab] = _cachedAnalysis[_selectedObjectTab]
                    .OrderBy(k => k.Value.Format.Value, StringComparer.Ordinal)
                    .ToDictionary(d => d.Key, d => d.Value, StringComparer.Ordinal);
            if (
                string.Equals(fileGroup.Key, "tex", StringComparison.Ordinal)
                && idx == 5
                ...
                _cachedAnalysis![_selectedObjectTab] = _cachedAnalysis[_selectedObjectTab]
                    .OrderByDescending(k => k.Value.Format.Value, StringComparer.Ordinal)
                    .ToDictionary(d => d.Key, d => d.Value, StringComparer.Ordinal);
        ...
        {
            using var text = ImRaii.PushColor(
                ImGuiCol.Text,
                new Vector4(0, 0, 0, 1),
                string.Equals(item.Hash, _selectedHash, StringComparison.Ordinal)
            );
            using var text2 = ImRaii.PushColor(
                ImGuiCol.Text,
                new Vector4(1, 1, 1, 1),
                !item.IsComputed
            );
            ...
            if (!item.IsComputed)
            {
                ImGui.TableSetBgColor(
                    ...
                    UiSharedService.Color(ImGuiColors.DalamudRed)
                );
                ImGui.TableSetBgColor(
