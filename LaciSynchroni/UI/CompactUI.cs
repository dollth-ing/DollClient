...
public class CompactUi : WindowMediatorSubscriberBase
{
        ...
        : base(logger, mediator, "###LaciSynchroniMainUI", performanceCollectorService)
    {
...
#if DEBUG
        WindowName =
            $"Laci Synchroni Dev Build ({ver.ToString()})###LaciSynchroniMainUI";
        Toggle();
#else
        WindowName =
            $"Laci Synchroni {ver.ToString()}###LaciSynchroniMainUI";
#endif
        ...
        Mediator.Subscribe<RefreshUiMessage>(
            this,
            (msg) => _drawFolders = GetDrawFolders().ToList()
        );
    ...
    protected override void DrawInternal()
    {
        ...
        //    }
        //    UiSharedService.ColorTextWrapped($"Your Laci Synchroni installation is out of date, the current version is {ver.Major}.{ver.Minor}.{ver.Build}. " +
        ...
        if (!_ipcManager.Initialized)
        {
            ...
            using (_uiSharedService.UidFont.Push())
            {
                var uidTextSize = ImGui.CalcTextSize(unsupported);
                ImGui.SetCursorPosX(
                    (ImGui.GetWindowContentRegionMax().X + ImGui.GetWindowContentRegionMin().X) / 2
                        - uidTextSize.X / 2
                );
                ImGui.AlignTextToFramePadding();
                ImGui.TextColored(ImGuiColors.DalamudRed, unsupported);
            ...
            UiSharedService.ColorTextWrapped(
                $"One or more Plugins essential for Laci operation are unavailable. Enable or update following plugins:",
        ...
        {
            using (ImRaii.PushId("serverpicker"))
                _uiSharedService.DrawServiceSelection(
                    hideServiceCreation: true,
                    showMinifiedConnect: true,
        ...
        if (
            ImGui.BeginPopupModal(
                "Set Notes for New User",
                ref _showModalForUserAddition,
        ...
        )
        {
            ...
            else
            {
                ...
                ImGui.InputTextWithHint(
                    "##noteforuser",
                    $"Note for {_lastAddedUser.UserData.AliasOrUID}",
                    ref _lastAddedUserComment,
                    100
                );
                if (_uiSharedService.IconTextButton(FontAwesomeIcon.Save, "Save Note"))
                {
                    _serverManager.SetNoteForUid(
                        _lastAddedUser.UserData.UID,
    ...
    private void DrawServerStatus()
    {
...
#else
        string shardConnection = string.Equals(
            _apiController.ServerInfo.ShardName,
            "Main",
        ...
        var printShard =
            !string.IsNullOrEmpty(_apiController.ServerInfo.ShardName)
            && shardConnection != string.Empty;
        ...
        if (!_serverConfigService.Current.ShowServerPickerInMainMenu)
        {
            ImGui.SetCursorPosX(
                ImGui.GetWindowContentRegionMin().X
                    ...
                    - ImGui.CalcTextSize(_serverManager.CurrentServer.ServerName).X / 2
            );
            ImGui.AlignTextToFramePadding();
        ...
        else
        {
            ImGui.SetCursorPosX(
                ImGui.GetWindowContentRegionMin().X
                    + UiSharedService.GetWindowContentRegionWidth() / 2
                    - ImGui.CalcTextSize("Not connected to any server").X / 2
            );
            ImGui.AlignTextToFramePadding();
        ...
        if (!_serverConfigService.Current.ShowServerPickerInMainMenu)
        {
            ...
            if (printShard)
            {
                ImGui.SetCursorPosY(
                    ImGui.GetCursorPosY()
                        - ((userSize.Y + textSize.Y) / 2 + shardTextSize.Y) / 2
    ...
    private void DrawModLoad()
    {
        ...
        if (config.TrisAutoPauseThresholdThousands > 0)
        {
            ...
            ImGuiHelpers.ScaledRelativeSameLine(180, ImGui.GetStyle().ItemSpacing.X);
            var calculatedTriangles = (
                (float)playerLoadTriangles / (config.TrisAutoPauseThresholdThousands * 1000)
            );
    ...
    private IEnumerable<IDrawFolder> GetDrawFolders()
    {
        ...
        var filteredPairs = allPairs
            .Where(p =>
            {
                if (_tabMenu.Filter.IsNullOrEmpty())
                    return true;
                return p.Key.UserData.AliasOrUID.Contains(
                        _tabMenu.Filter,
                        StringComparison.OrdinalIgnoreCase
                    )
                    || (
                        p.Key.GetNote()
                            ?.Contains(_tabMenu.Filter, StringComparison.OrdinalIgnoreCase) ?? false
                    )
                    || (
                        p.Key.PlayerName?.Contains(
                            _tabMenu.Filter,
                            StringComparison.OrdinalIgnoreCase
                        ) ?? false
                    );
        ...
        string? AlphabeticalSort(KeyValuePair<Pair, List<GroupFullInfoDto>> u) =>
            (
                ...
                && !string.IsNullOrEmpty(u.Key.PlayerName)
                    ? (
                        _configService.Current.PreferNotesOverNamesForVisible
                            ? u.Key.GetNote()
                            : u.Key.PlayerName
                    )
        ...
        ImmutableList<Pair> ImmutablePairList(
            IEnumerable<KeyValuePair<Pair, List<GroupFullInfoDto>>> u
        ...
        bool FilterGroupUsers(
            KeyValuePair<Pair, List<GroupFullInfoDto>> u,
        ...
        if (_configService.Current.ShowVisibleUsersSeparately)
        {
            ...
            var filteredVisiblePairs = BasicSortedDictionary(
                filteredPairs.Where(FilterVisibleUsers)
            );

            drawFolders.Add(
                _drawEntityFactory.CreateDrawTagFolder(
        ...
        foreach (
            var group in _pairManager
                .GroupPairs.Select(g => g.Key)
        ...
        )
        {
            ...
            groupFolders.Add(
                _drawEntityFactory.CreateDrawGroupFolder(group, filteredGroupPairs, allGroupPairs)
            );
        }

        if (_configService.Current.GroupUpSyncshells)
            drawFolders.Add(
                new DrawGroupedGroupFolder(groupFolders, _tagHandler, _uiSharedService)
            );
        else
        ...
        foreach (var tag in tags)
        {
            ...
            var filteredTagPairs = BasicSortedDictionary(
                filteredPairs.Where(u => FilterTagusers(u, tag) && FilterOnlineOrPausedSelf(u))
            );

            drawFolders.Add(
                _drawEntityFactory.CreateDrawTagFolder(tag, filteredTagPairs, allTagPairs)
            );
        }

        var allOnlineNotTaggedPairs = ImmutablePairList(allPairs.Where(FilterNotTaggedUsers));
        var onlineNotTaggedPairs = BasicSortedDictionary(
            filteredPairs.Where(u => FilterNotTaggedUsers(u) && FilterOnlineOrPausedSelf(u))
        );

        drawFolders.Add(
            _drawEntityFactory.CreateDrawTagFolder(
                (
                    _configService.Current.ShowOfflineUsersSeparately
        ...
        if (_configService.Current.ShowOfflineUsersSeparately)
        {
            ...
            var filteredOfflinePairs = BasicSortedDictionary(
                filteredPairs.Where(FilterOfflineUsers)
            );

            drawFolders.Add(
                _drawEntityFactory.CreateDrawTagFolder(
                    TagHandler.CustomOfflineTag,
                    filteredOfflinePairs,
                    allOfflinePairs
                )
            );
            if (_configService.Current.ShowSyncshellOfflineUsersSeparately)
            {
                var allOfflineSyncshellUsers = ImmutablePairList(
                    allPairs.Where(FilterOfflineSyncshellUsers)
                );
                var filteredOfflineSyncshellUsers = BasicSortedDictionary(
                    filteredPairs.Where(FilterOfflineSyncshellUsers)
                );

                drawFolders.Add(
                    _drawEntityFactory.CreateDrawTagFolder(
                        TagHandler.CustomOfflineSyncshellTag,
    ...
    private string GetServerError()
    {
        return _apiController.ServerState switch
        {
            ...
            ServerState.Disconnected =>
                "You are currently disconnected from the Laci Synchroni server.",
            ServerState.Disconnecting => "Disconnecting from the server",
            ServerState.Unauthorized => "Server Response: " + _apiController.AuthFailureMessage,
            ServerState.Offline => "Your selected Laci Synchroni server is currently offline.",
            ...
            ServerState.NoAutoLogon =>
                "This character has automatic login into Laci disabled. Press the connect button to connect to Laci.",
            _ => string.Empty,
        };
    }
    ...
    private void CheckForCharacterAnalysis()
    {
        if (_hasUpdate)
        {
            _cachedAnalysis = _characterAnalyzer.LastAnalysis.ToDictionary(
