...
public class DownloadUi : WindowMediatorSubscriberBase
{
    ...
    public DownloadUi(
        ...
        : base(logger, mediator, "Laci Synchroni Downloads", performanceCollectorService)
    {
        ...
        Mediator.Subscribe<PlayerUploadingMessage>(
            this,
            (msg) =>
            {
                if (msg.IsUploading)
    ...
    protected override void DrawInternal()
    {
        if (_configService.Current.ShowTransferWindow)
        {
            ...
            try
            {
                ...
                {
                    var dlSlot = item.Value.Count(c =>
                        c.Value.DownloadStatus == DownloadStatus.WaitingForSlot
                    );
                    var dlQueue = item.Value.Count(c =>
                        c.Value.DownloadStatus == DownloadStatus.WaitingForQueue
                    );
                    var dlProg = item.Value.Count(c =>
                        c.Value.DownloadStatus == DownloadStatus.Downloading
                    );
                    var dlDecomp = item.Value.Count(c =>
                        c.Value.DownloadStatus == DownloadStatus.Decompressing
                    );
        ...
        if (_configService.Current.ShowTransferBars)
        {
            ...
            foreach (var transfer in _currentDownloads.ToList())
            {
                ...
                var maxDlText =
                    $"{UiSharedService.ByteToString(totalBytes, addSuffix: false)}/{UiSharedService.ByteToString(totalBytes)}";
                var textSize = _configService.Current.TransferBarsShowText
                    ? ImGui.CalcTextSize(maxDlText)
                    : new Vector2(10, 10);

                int dlBarHeight =
                    _configService.Current.TransferBarsHeight > ((int)textSize.Y + 5)
                        ? _configService.Current.TransferBarsHeight
                        : (int)textSize.Y + 5;
                int dlBarWidth =
                    _configService.Current.TransferBarsWidth > ((int)textSize.X + 10)
                        ? _configService.Current.TransferBarsWidth
                        : (int)textSize.X + 10;

                var dlBarStart = new Vector2(
                    screenPos.X - dlBarWidth / 2f,
                    screenPos.Y - dlBarHeight / 2f
                );
                var dlBarEnd = new Vector2(
                    screenPos.X + dlBarWidth / 2f,
                    screenPos.Y + dlBarHeight / 2f
                );
                ...
                drawList.AddRectFilled(
                    dlBarStart with
                    {
                        X = dlBarStart.X - dlBarBorder - 1,
                        Y = dlBarStart.Y - dlBarBorder - 1,
                    },
                    dlBarEnd with
                    {
                        X = dlBarEnd.X + dlBarBorder + 1,
                        Y = dlBarEnd.Y + dlBarBorder + 1,
                    },
                ...
                drawList.AddRectFilled(
                    dlBarStart with
                    {
                        X = dlBarStart.X - dlBarBorder,
                        Y = dlBarStart.Y - dlBarBorder,
                    },
                    dlBarEnd with
                    {
                        X = dlBarEnd.X + dlBarBorder,
                        Y = dlBarEnd.Y + dlBarBorder,
                    },
                ...
                drawList.AddRectFilled(
                    ...
                    dlBarEnd with
                    {
                        X = dlBarStart.X + (float)(dlProgressPercent * dlBarWidth),
                    },
                    UiSharedService.Color(50, 205, 50, transparency),
                    1
                );
    ...
    public override bool DrawConditions()
    {
        ...
        if (!_configService.Current.ShowTransferWindow && !_configService.Current.ShowTransferBars)
            return false;
        if (
            !_currentDownloads.Any()
            && !_fileTransferManager.CurrentUploads.Any()
            && !_uploadingPlayers.Any()
        )
            return false;
        if (!IsOpen)
            return false;
