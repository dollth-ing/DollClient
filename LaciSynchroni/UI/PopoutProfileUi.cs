...
public class PopoutProfileUi : WindowMediatorSubscriberBase
{
    ...
    public PopoutProfileUi(
        ILogger<PopoutProfileUi> logger,
        ...
        : base(logger, mediator, "###LaciSynchroniPopoutProfileUI", performanceCollectorService)
    {
        ...
        Mediator.Subscribe<ProfilePopoutToggle>(
            this,
            (msg) =>
            {
        ...
        Mediator.Subscribe<CompactUiChange>(
            this,
            (msg) =>
            {
                if (msg.Size != Vector2.Zero)
                {
                    ...
                    Size = new(
                        256 + (padding.X * 2) + border,
                        msg.Size.Y / ImGuiHelpers.GlobalScale
                    );
                ...
                {
                    Position = new(
                        mainPos.X + _lastMainSize.X * ImGuiHelpers.GlobalScale,
                        mainPos.Y
                    );
                }
                else
    ...
    protected override void DrawInternal()
    {
        ...
        try
        {
            ...
            var syncProfile = _profileManager.GetSyncProfile(_pair.UserData);

            if (
                _textureWrap == null
                || !syncProfile.ImageData.Value.SequenceEqual(_lastProfilePicture)
            )
            {
                _textureWrap?.Dispose();
            ...
            if (
                _supporterTextureWrap == null
                || !syncProfile.SupporterImageData.Value.SequenceEqual(_lastSupporterPicture)
            )
            {
                _supporterTextureWrap?.Dispose();
                _supporterTextureWrap = null;
            ...
            UiSharedService.ColorText(
                status,
                (_pair.IsVisible || _pair.IsOnline)
            ...
            if (
                _pair.UserPair.IndividualPairStatus
            ...
            if (_pair.UserPair.Groups.Any())
            {
                ...
                foreach (var group in _pair.UserPair.Groups)
                {
                    ...
                    var groupName = _pairManager
                        .GroupPairs.First(f =>
                            string.Equals(f.Key.GID, group, StringComparison.Ordinal)
                        )
                        .Key.GroupAliasOrGID;
            ...
            while (textSize.Y > remaining && descText.Contains(' '))
            {
                ...
                textSize = ImGui.CalcTextSize(
                    descText
                        + $"...{Environment.NewLine}[Open Full Profile for complete description]",
                    wrapWidth: 256f * ImGuiHelpers.GlobalScale
                );
            }
            ...
            var stretchFactor = tallerThanWide
                ? 256f * ImGuiHelpers.GlobalScale / _textureWrap.Height
                : 256f * ImGuiHelpers.GlobalScale / _textureWrap.Width;
            ...
            drawList.AddImage(
                _textureWrap.Handle,
                new Vector2(
                    rectMin.X + padding + remainingWidth,
                    rectMin.Y + spacing.Y + imagePos.Y + remainingHeight
                ),
                new Vector2(
                    rectMin.X + padding + remainingWidth + newWidth,
                    rectMin.Y + spacing.Y + imagePos.Y + remainingHeight + newHeight
                )
            );
            if (_supporterTextureWrap != null)
            {
                ...
                drawList.AddImage(
                    _supporterTextureWrap.Handle,
                    new Vector2(
                        rectMax.X - iconSize - spacing.X,
                        rectMin.Y + (textPos / 2) - (iconSize / 2)
                    ),
                    new Vector2(
                        rectMax.X - spacing.X,
                        rectMin.Y + iconSize + (textPos / 2) - (iconSize / 2)
                    )
                );
            }
        }
