...
public sealed class DtrEntry : IDisposable, IHostedService
{
    ...
    public DtrEntry(
        ...
        IDtrBar dtrBar,
        ConfigurationServiceBase<SyncConfig> configService,
    ...
    private void Update()
    {
        ...
        if (_apiController.IsConnected)
        {
            ...
            if (pairCount > 0)
            {
                ...
                {
                    visiblePairs = _pairManager
                        .GetOnlineUserPairs()
                        .Where(x => x.IsVisible)
                        .Select(x =>
                            string.Format(
                                "{0} ({1})",
                                _configService.Current.PreferNoteInDtrTooltip
                                    ? x.GetNote() ?? x.PlayerName
                                    : x.PlayerName,
                ...
                tooltip =
                    $"Laci Synchroni: Connected{Environment.NewLine}----------{Environment.NewLine}{string.Join(Environment.NewLine, visiblePairs)}";
        ...
        new(
            unchecked(
                ...
                [
                    ...
                    0x05,
                    0xF6,
                    byte.Max((byte)color, 0x01),
                    byte.Max((byte)(color >> 8), 0x01),
                    byte.Max((byte)(color >> 16), 0x01),
                    0x03,
                ]
            )
        );
