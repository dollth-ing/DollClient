...
public class SettingsUi : WindowMediatorSubscriberBase
{
    ...
    private readonly ConcurrentDictionary<
        GameObjectHandler,
        Dictionary<string, FileDownloadStatus>
    > _currentDownloads = new();
    ...
    public SettingsUi(
        ...
        : base(logger, mediator, "Laci Synchroni Settings", performanceCollector)
    {
        ...
        Mediator.Subscribe<CharacterDataCreatedMessage>(
            this,
            (msg) => LastCreatedCharacterData = msg.CharacterData
        );
        Mediator.Subscribe<DownloadStartedMessage>(
            this,
            (msg) => _currentDownloads[msg.DownloadId] = msg.DownloadStatus
        );
        Mediator.Subscribe<DownloadFinishedMessage>(
            this,
            (msg) => _currentDownloads.TryRemove(msg.DownloadId, out _)
        );
    }
    ...
    private static bool InputDtrColors(string label, ref DtrEntry.Colors colors)
    {
        ...
        if (ImGui.IsItemHovered())
            ImGui.SetTooltip(
                "Foreground Color - Set to pure black (#000000) to use the default color"
            );

        ImGui.SameLine(0.0f, innerSpacing);
        ret |= ImGui.ColorEdit3(
            "###glow",
            ref glowColor,
            ImGuiColorEditFlags.NoInputs | ImGuiColorEditFlags.NoLabel | ImGuiColorEditFlags.Uint8
        );
        ...
        static Vector3 ConvertColor(uint color) =>
            unchecked(
                new((byte)color / 255.0f, (byte)(color >> 8) / 255.0f, (byte)(color >> 16) / 255.0f)
            );
    ...
    private void DrawBlockedTransfers()
    {
        ...
        UiSharedService.ColorTextWrapped(
            "Files that you attempted to upload or download that were forbidden to be transferred by their creators will appear here. "
                ...
                + "Ask your paired friend to send you the mod in question through other means, acquire the mod yourself or pester the mod creator to allow it to be sent over Laci.",
    ...
    private void DrawCurrentTransfers()
    {
        ...
        _uiShared.DrawCombo(
            "###speed",
            [DownloadSpeeds.Bps, DownloadSpeeds.KBps, DownloadSpeeds.MBps],
            (s) =>
                s switch
                {
        ...
        _uiShared.DrawHelpText(
            "Shows download text (amount of MiB downloaded) in the transfer bars"
        );
        ...
        if (
            ImGui.Checkbox(
                "Show 'Uploading' text below players that are currently uploading",
                ref showUploading
            )
        )
        {
        ...
        if (_apiController.IsConnected)
        {
            ...
            if (tree)
            {
                if (
                    _downloadServersTask == null
                    || (
                        (_downloadServersTask?.IsCompleted ?? false)
                        && (!_downloadServersTask?.IsCompletedSuccessfully ?? false)
                    )
                )
                {
                    if (
                        _uiShared.IconTextButton(
                            FontAwesomeIcon.GroupArrowsRotate,
                            "Update Download Server List"
                        )
                    )
                    {
                ...
                if (
                    _downloadServersTask != null
                    && _downloadServersTask.IsCompleted
                ...
                )
                {
                    if (_speedTestTask == null || _speedTestTask.IsCompleted)
                    {
                        ...
                        {
                            _speedTestTask = RunSpeedTest(
                                _downloadServersTask.Result!,
                    ...
                    {
                        UiSharedService.ColorTextWrapped(
                            "Running Speedtest to File Servers...",
                    ...
                    if (_speedTestTask != null && _speedTestTask.IsCompleted)
                    {
                        ...
                        {
                            UiSharedService.ColorTextWrapped(
                                "Speedtest completed with no results",
                                ImGuiColors.DalamudYellow
                            );
                        }
                    }
        ...
        {
            if (
                ApiController.ServerState is ServerState.Connected
                && ImGui.BeginTabItem("Transfers")
            )
            {
                ...
                if (ImGui.BeginTable("UploadsTable", 3))
                {
                    ...
                    {
                        var color = UiSharedService.UploadColor(
                ...
                if (ImGui.BeginTable("DownloadsTable", 4))
                {
                    ...
                    foreach (var transfer in _currentDownloads.ToArray())
                    {
                        ...
                        {
                            var color = UiSharedService.UploadColor(
                                (entry.Value.TransferredBytes, entry.Value.TotalBytes)
                            );
                            ImGui.TableNextColumn();
                            ...
                            ImGui.TextUnformatted(
                                entry.Value.TransferredFiles + "/" + entry.Value.TotalFiles
                            );
                            ImGui.TableNextColumn();
                            ImGui.TextUnformatted(
                                UiSharedService.ByteToString(entry.Value.TransferredBytes)
                                    + "/"
        ...
        foreach (var server in servers)
        {
            ...
            try
            {
                result = await _fileTransferOrchestrator
                    .SendRequestAsync(
                        HttpMethod.Get,
                        new Uri(new Uri(server), "speedtest/run"),
                        token,
                ...
                _logger.LogInformation(
                    "Downloaded {bytes} from {server} in {time}",
                    UiSharedService.ByteToString(readBytes),
                    server,
                    st.Elapsed
                );
    ...
    private async Task<List<string>?> GetDownloadServerList()
    {
        try
        {
            var result = await _fileTransferOrchestrator
                .SendRequestAsync(
                    HttpMethod.Get,
                    new Uri(_fileTransferOrchestrator.FilesCdnUri!, "files/downloadServers"),
    ...
    private void DrawDebug()
    {
        ...
        if (
            _uiShared.IconTextButton(
                ...
                "[DEBUG] Copy Last created Character Data to clipboard"
            )
        )
        {
        ...
        UiSharedService.AttachToolTip(
            "Use this when reporting mods being rejected from the server."
        );
        ...
        using (ImRaii.Disabled(!logPerformance))
        {
            if (
                _uiShared.IconTextButton(
                    ...
                    "Print Performance Stats to /xllog"
                )
            )
            {
            ...
            if (
                _uiShared.IconTextButton(
                    FontAwesomeIcon.StickyNote,
                    "Print Performance Stats (last 60s) to /xllog"
                )
            )
            {
    ...
    private void DrawFileStorageSettings()
    {
        ...
        if (_uiShared.IconTextButton(FontAwesomeIcon.Running, "Open Laci Character Data Hub"))
        {
        ...
        UiSharedService.TextWrapped(
            "Laci stores downloaded files from paired people permanently. This is to improve loading performance and requiring less downloads. "
        ...
        ImGui.AlignTextToFramePadding();
        ImGui.TextUnformatted(
            "Monitoring Penumbra Folder: "
                + (_cacheMonitor.PenumbraWatcher?.Path ?? "Not monitoring")
        );
        if (string.IsNullOrEmpty(_cacheMonitor.PenumbraWatcher?.Path))
        {
            ...
            if (
                _uiShared.IconTextButton(
                    FontAwesomeIcon.ArrowsToCircle,
                    "Try to reinitialize Monitor"
                )
            )
            {
        ...
        ImGui.AlignTextToFramePadding();
        ImGui.TextUnformatted(
            "Monitoring Laci Storage Folder: "
                + (_cacheMonitor.SyncWatcher?.Path ?? "Not monitoring")
        );
        if (string.IsNullOrEmpty(_cacheMonitor.SyncWatcher?.Path))
        {
            ...
            {
                _cacheMonitor.StartSyncWatcher(_configService.Current.CacheFolder);
            }
        }
        if (_cacheMonitor.SyncWatcher == null || _cacheMonitor.PenumbraWatcher == null)
        {
            ...
            UiSharedService.AttachToolTip(
                "Attempts to resume monitoring for both Penumbra and Laci Storage. "
        ...
        else
        {
            ...
            UiSharedService.AttachToolTip(
                "Stops the monitoring for both Penumbra and Laci Storage. "
                    + "Do not stop the monitoring, unless you plan to move the Penumbra and Laci Storage folders, to ensure correct functionality of Laci."
                    + Environment.NewLine
        ...
        {
            UiSharedService.ColorTextWrapped(
                "Hint: To free up space when using Laci consider enabling the File Compactor",
                ImGuiColors.DalamudYellow
            );
        }
        ...
        if (!_fileCompactor.MassCompactRunning)
        {
            ...
            UiSharedService.AttachToolTip(
                "This will run compression on all files in your current Laci Storage."
                    + Environment.NewLine
            ...
            UiSharedService.AttachToolTip(
                "This will run decompression on all files in your current Laci Storage."
            );
        }
        else
        {
            UiSharedService.ColorText(
                $"File compactor currently running ({_fileCompactor.Progress})",
                ImGuiColors.DalamudYellow
            );
        }
        if (isLinux || !_cacheMonitor.StorageisNTFS)
        {
            ImGui.EndDisabled();
            ImGui.TextUnformatted(
                "The file compactor is only available on Windows and NTFS drives."
            );
        }
        ...
        ImGui.Separator();
        UiSharedService.TextWrapped(
            "File Storage validation can make sure that all files in your local Laci Storage are valid. "
        ...
        if (_validationTask != null)
        {
            using (ImRaii.PushIndent(20f))
            {
                ...
                else
                {
                    ...
                    UiSharedService.TextWrapped(
                        $"Current item: {_currentProgress.Item3.ResolvedFilepath}"
                    );
                }
            }
        }
        ...
        UiSharedService.TextWrapped(
            "I understand that: "
                ...
                + Environment.NewLine
                + "- This is not a step to try to fix sync issues."
                + Environment.NewLine
        ...
        if (!_readClearCache)
            ImGui.BeginDisabled();
        if (
            _uiShared.IconTextButton(FontAwesomeIcon.Trash, "Clear local storage")
        ...
        UiSharedService.AttachToolTip(
            "You normally do not need to do this. THIS IS NOT SOMETHING YOU SHOULD BE DOING TO TRY TO FIX SYNC ISSUES."
                ...
                + Environment.NewLine
                + "Laci storage is self-clearing and will not surpass the limit you have set it to."
                + Environment.NewLine
    ...
    private void DrawGeneral()
    {
        ...
        if (
            _uiShared.IconTextButton(
                FontAwesomeIcon.StickyNote,
                "Export all your user notes to clipboard"
            )
        )
        {
            ImGui.SetClipboardText(
                UiSharedService.GetNotes(
                    _pairManager
                        .DirectPairs.UnionBy(
                            _pairManager.GroupPairs.SelectMany(p => p.Value),
                            p => p.UserData,
                            UserDataComparer.Instance
                        )
                        .ToList()
                )
            );
        ...
        if (_uiShared.IconTextButton(FontAwesomeIcon.FileImport, "Import notes from clipboard"))
        {
            ...
            _notesSuccessfullyApplied = _uiShared.ApplyNotesFromClipboard(
                notes,
                _overwriteExistingLabels
            );
        }
        ...
        {
            UiSharedService.ColorTextWrapped(
                "User Notes successfully imported",
        ...
        if (
            ImGui.Checkbox("Automatically populate notes using player names", ref autoPopulateNotes)
        )
        {
        ...
        var showNameInsteadOfNotes = _configService
            .Current
            .ShowCharacterNameInsteadOfNotesForVisible;
        ...
        _uiShared.DrawHelpText(
            "This will add Laci related right click menu entries in the game UI on paired players."
        );

        if (
            ImGui.Checkbox(
                "Display status and visible pair count in Server Info Bar",
                ref enableDtrEntry
            )
        )
        {
        ...
        _uiShared.DrawHelpText(
            "This will add Laci connection status and visible pair count in the Server Info Bar.\nYou can further configure this through your Dalamud Settings."
        );
        ...
        using (ImRaii.Disabled(!enableDtrEntry))
        {
            ...
            if (
                ImGui.Checkbox(
                    "Prefer notes over player names in tooltip",
                    ref preferNoteInDtrTooltip
                )
            )
            {
            ...
            if (
                ImGui.Checkbox(
                    "Color-code the Server Info Bar entry according to status",
                    ref useColorsInDtr
                )
            )
            {
        ...
        using (ImRaii.Disabled(!showOfflineSeparate))
        {
            ...
            if (
                ImGui.Checkbox(
                    "Show separate Offline group for Syncshell users",
                    ref syncshellOfflineSeparate
                )
            )
            {
                _configService.Current.ShowSyncshellOfflineUsersSeparately =
                    syncshellOfflineSeparate;
                _configService.Save();
                Mediator.Publish(new RefreshUiMessage());
            }
        }
        ...
        ImGui.Indent();
        if (!_configService.Current.ShowCharacterNameInsteadOfNotesForVisible)
            ImGui.BeginDisabled();
        if (
            ImGui.Checkbox(
                "Prefer notes over player names for visible players",
                ref preferNotesInsteadOfName
            )
        )
        {
        ...
        _uiShared.DrawHelpText(
            "If you set a note for a player it will be shown instead of the player name"
        );
        if (!_configService.Current.ShowCharacterNameInsteadOfNotesForVisible)
            ImGui.EndDisabled();
        ImGui.Unindent();
        ...
        if (
            ImGui.Checkbox(
                "Set visible pairs as focus targets when clicking the eye",
                ref useFocusTarget
            )
        )
        {
        ...
        var onlineNotifsPairsOnly = _configService
            .Current
        ...
        _uiShared.DrawCombo(
            "Info Notification Display##settingsUi",
        ...
        _uiShared.DrawCombo(
            "Warning Notification Display##settingsUi",
            (NotificationLocation[])Enum.GetValues(typeof(NotificationLocation)),
            (i) => i.ToString(),
            (i) =>
            {
        ...
        _uiShared.DrawCombo(
            "Error Notification Display##settingsUi",
            (NotificationLocation[])Enum.GetValues(typeof(NotificationLocation)),
            (i) => i.ToString(),
            (i) =>
            {
        ...
        {
            _configService.Current.ShowOnlineNotificationsOnlyForIndividualPairs =
                onlineNotifsPairsOnly;
            _configService.Save();
        }
        _uiShared.DrawHelpText(
            "Enabling this will only show online notifications (type: Info) for individual pairs."
        );
    ...
    private void DrawPerformance()
    {
        ...
        ImGui.Separator();
        ImGui.Dummy(new Vector2(10));
        bool showPlayerPerformanceIndicator = _playerPerformanceConfigService
            .Current
            .ShowPlayerPerformanceInMainUi;
        bool showPerformanceIndicator = _playerPerformanceConfigService
            .Current
            .ShowPerformanceIndicator;
        if (
            ImGui.Checkbox(
                "Show Character Load Data in Main Menu",
                ref showPlayerPerformanceIndicator
            )
        )
        {
            _playerPerformanceConfigService.Current.ShowPlayerPerformanceInMainUi =
                showPlayerPerformanceIndicator;
            _playerPerformanceConfigService.Save();
        }
        if (ImGui.Checkbox("Show performance indicator", ref showPerformanceIndicator))
        {
            _playerPerformanceConfigService.Current.ShowPerformanceIndicator =
                showPerformanceIndicator;
            _playerPerformanceConfigService.Save();
        }
        _uiShared.DrawHelpText(
            "Will show a performance indicator when players exceed defined thresholds in Laci UI."
                + Environment.NewLine
                + "Will use warning thresholds."
        );
        bool warnOnExceedingThresholds = _playerPerformanceConfigService
            .Current
            .WarnOnExceedingThresholds;
        if (
            ImGui.Checkbox(
                "Warn on loading in players exceeding performance thresholds",
                ref warnOnExceedingThresholds
            )
        )
        {
            _playerPerformanceConfigService.Current.WarnOnExceedingThresholds =
                warnOnExceedingThresholds;
            _playerPerformanceConfigService.Save();
        }
        _uiShared.DrawHelpText(
            "Laci will print a warning in chat once per session of meeting those people. Will not warn on players with preferred permissions."
        );
        using (ImRaii.Disabled(!warnOnExceedingThresholds && !showPerformanceIndicator))
        {
            using var indent = ImRaii.PushIndent();
            var warnOnPref = _playerPerformanceConfigService
                .Current
                .WarnOnPreferredPermissionsExceedingThresholds;
            if (
                ImGui.Checkbox(
                    "Warn/Indicate also on players with preferred permissions",
                    ref warnOnPref
                )
            )
            {
                _playerPerformanceConfigService
                    .Current
                    .WarnOnPreferredPermissionsExceedingThresholds = warnOnPref;
                _playerPerformanceConfigService.Save();
            }
            _uiShared.DrawHelpText(
                "Laci will also print warnings and show performance indicator for players where you enabled preferred permissions. If warning in general is disabled, this will not produce any warnings."
            );
        }
        ...
        ImGui.Dummy(new Vector2(10));
        bool autoPause = _playerPerformanceConfigService
            .Current
            .AutoPausePlayersExceedingThresholds;
        bool autoPauseEveryone = _playerPerformanceConfigService
            .Current
        ...
        using (ImRaii.Disabled(!autoPause))
        {
            ...
            if (
                ImGui.Checkbox(
                    "Automatically pause also players with preferred permissions",
        ...
        using (ImRaii.Disabled(string.IsNullOrEmpty(_uidToAddForIgnore)))
        {
            ...
            {
                if (
                    !_playerPerformanceConfigService.Current.UIDsToIgnore.Contains(
                        _uidToAddForIgnore,
    ...
    private void DrawServerConfiguration()
    {
        ...
        if (ApiController.ServerAlive)
        {
            ...
            if (
                ImGui.BeginPopupModal(
                    ...
                    ref _deleteFilesPopupModalShown,
            ...
            _uiShared.DrawHelpText(
                "Completely deletes your account and all uploaded files to the service."
            );

            if (
                ImGui.BeginPopupModal(
                    "Delete your account?",
                    ref _deleteAccountPopupModalShown,
                    UiSharedService.PopupWindowFlags
                )
            )
            {
        ...
        _uiShared.DrawHelpText(
            "This will allow sending census data to the currently connected service."
                + UiSharedService.TooltipSeparator
        ...
        if (ImGui.BeginTabBar("serverTabBar"))
        {
            if (ImGui.BeginTabItem("Character Management"))
            {
                ...
                {
                    UiSharedService.ColorTextWrapped(
                        "Characters listed here will automatically connect to the selected Laci service with the settings as provided below."
                    ...
                    if (
                        selectedServer.UseOAuth2 && !string.IsNullOrEmpty(selectedServer.OAuthToken)
                    )
                    {
                        bool hasSetSecretKeysButNoUid = selectedServer.Authentications.Exists(u =>
                        ...
                        if (hasSetSecretKeysButNoUid)
                        {
                            ...
                            )
                            {
                                ...
                                {
                                    _secretKeysConversionTask = ConvertSecretKeysToUIDs(
                                        selectedServer,
                                        _secretKeysConversionCts.Token
                                    );
                                }
                            }
                            if (
                            ...
                            if (
                                _secretKeysConversionTask != null
                                && _secretKeysConversionTask.IsCompletedSuccessfully
                            )
                            {
                                ...
                                string text =
                                    $"Conversion has completed: {_secretKeysConversionTask.Result.Result}";
                                if (textColor == null)
                                {
                                ...
                                if (
                                    !_secretKeysConversionTask.Result.Success
                    ...
                    if (
                        !selectedServer.Authentications.Exists(a =>
                            string.Equals(a.CharacterName, youName, StringComparison.Ordinal)
                            && a.WorldId == youWorld
                        )
                    )
                    {
                        _uiShared.BigText(
                            "Your Character is not Configured",
                            ImGuiColors.DalamudRed
                        );
                        UiSharedService.ColorTextWrapped(
                        ...
                        var authWithCid = selectedServer.Authentications.Find(f =>
                            f.LastSeenCID == youCid
                        );
                        if (authWithCid != null)
                        {
                            ImGuiHelpers.ScaledDummy(5);
                            ...
                            using (ImRaii.PushIndent(10f))
                                UiSharedService.ColorText(
                                    "Entry: "
                                        + authWithCid.CharacterName
                                        + " - "
                                        + _dalamudUtilService.WorldData.Value[
                                            (ushort)authWithCid.WorldId
                                        ],
                    ...
                    foreach (var item in selectedServer.Authentications.ToList())
                    {
                        ...
                        var data = _uiShared
                            .WorldData.OrderBy(u => u.Value, StringComparer.Ordinal)
                        ...
                        if (misManaged)
                        {
                            text +=
                                " [MISMANAGED ("
                                + (selectedServer.UseOAuth2 ? "No UID Set" : "No Secret Key Set")
                                + ")]";
                            color = ImGuiColors.DalamudRed;
                        }
                        if (
                            selectedServer
                                .Authentications.Where(e => e != item)
                                .Any(e =>
                                    string.Equals(
                                        e.CharacterName,
                                        item.CharacterName,
                        ...
                        _uiShared.DrawCombo(
                            ...
                            EqualityComparer<KeyValuePair<ushort, string>>.Default.Equals(
                                data.FirstOrDefault(f => f.Key == worldIdx),
                                default
                            )
                                ? data.First()
                        ...
                        {
                            _uiShared.DrawCombo(
                                "Secret Key###" + item.CharacterName + i,
                                keys,
                                ...
                                EqualityComparer<KeyValuePair<int, SecretKey>>.Default.Equals(
                                    keys.FirstOrDefault(f => f.Key == item.SecretKeyIdx),
                                    default
                                )
                                    ? keys.First()
                        ...
                        {
                            _uiShared.DrawUIDComboForAuthentication(
                                i,
                                item,
                        ...
                        if (ImGui.Checkbox("Automatically login to Laci", ref isAutoLogin))
                        {
                            item.AutoLogin = isAutoLogin;
                            _serverConfigurationManager.Save();
                        }
                        _uiShared.DrawHelpText(
                            "When enabled and logging into this character in XIV, Laci will automatically connect to the current service."
                        );
                        if (
                            _uiShared.IconTextButton(FontAwesomeIcon.Trash, "Delete Character")
                            && UiSharedService.CtrlPressed()
                        )
                    ...
                    if (
                        !selectedServer.Authentications.Exists(c =>
                ...
                {
                    UiSharedService.ColorTextWrapped(
                        "You need to add a Secret Key first before adding Characters.",
                        ImGuiColors.DalamudYellow
                    );
                }
            ...
            if (!useOauth && ImGui.BeginTabItem("Secret Key Management"))
            {
                foreach (var item in selectedServer.SecretKeys.ToList())
                {
                    ...
                    {
                        if (
                            _uiShared.IconTextButton(FontAwesomeIcon.Trash, "Delete Secret Key")
                            && UiSharedService.CtrlPressed()
                        )
                        {
                    ...
                    {
                        UiSharedService.ColorTextWrapped(
                            "This key is in use and cannot be deleted",
                            ImGuiColors.DalamudYellow
                        );
                    }
                ...
                if (_uiShared.IconTextButton(FontAwesomeIcon.Plus, "Add new Secret Key"))
                {
                    selectedServer.SecretKeys.Add(
                        selectedServer.SecretKeys.Any()
                            ? selectedServer.SecretKeys.Max(p => p.Key) + 1
                            : 0,
            ...
            if (ImGui.BeginTabItem("Service Configuration"))
            {
                ...
                var isMain = string.Equals(
                    serverName,
                    ApiController.MainServer,
                    StringComparison.OrdinalIgnoreCase
                );
                ...
                if (_dalamudUtilService.IsWine)
                {
                    ...
                    _uiShared.DrawHelpText(
                        "On wine, Laci will automatically fall back to ServerSentEvents/LongPolling, even if WebSockets is selected. "
                            + "WebSockets are known to crash XIV entirely on wine 8.5 shipped with Dalamud. "
                            + "Only enable this if you are not running wine 8.5."
                            + Environment.NewLine
                ...
                if (useOauth)
                {
                    ...
                    if (
                        string.IsNullOrEmpty(
                            _serverConfigurationManager.GetDiscordUserFromToken(selectedServer)
                        )
                    )
                    {
                    ...
                    if (
                        ...
                        && selectedServer.Authentications.TrueForAll(u =>
                ...
                if (!isMain && selectedServer != _serverConfigurationManager.CurrentServer)
                {
                    ImGui.Separator();
                    if (
                        _uiShared.IconTextButton(FontAwesomeIcon.Trash, "Delete Service")
                        && UiSharedService.CtrlPressed()
                    )
                    {
            ...
            if (ImGui.BeginTabItem("Permission Settings"))
            {
                ...
                if (
                    selectedServer == _serverConfigurationManager.CurrentServer
                ...
                )
                {
                    ...
                    if (
                        ImGui.Checkbox(
                            "Individually set permissions become preferred permissions",
                    ...
                    if (
                        ImGui.Checkbox(
                            "Disable individual pair sounds",
                            ref disableIndividualSounds
                        )
                    )
                    {
                    ...
                    _uiShared.DrawHelpText(
                        "This setting will disable sound sync for all new individual pairs."
                    );
                    if (
                        ImGui.Checkbox(
                            "Disable individual pair animations",
                            ref disableIndividualAnimations
                        )
                    )
                    {
                    ...
                    _uiShared.DrawHelpText(
                        "This setting will disable animation sync for all new individual pairs."
                    );
                    ...
                    _uiShared.DrawHelpText(
                        "This setting will disable VFX sync for all new individual pairs."
                    );
                    ImGuiHelpers.ScaledDummy(5f);
                    ...
                    if (
                        ImGui.Checkbox(
                            "Disable Syncshell pair animations",
                            ref disableGroupAnimations
                        )
                    )
                    {
                ...
                {
                    UiSharedService.ColorTextWrapped(
                        "Default Permission Settings unavailable for this service. "
    ...
    private Task<(bool Success, bool PartialSuccess, string Result)>? _secretKeysConversionTask =
        null;
    ...
    {
        List<Authentication> failedConversions = serverStorage
            .Authentications.Where(u => u.SecretKeyIdx == -1 && string.IsNullOrEmpty(u.UID))
            .ToList();
        List<Authentication> conversionsToAttempt = serverStorage
            .Authentications.Where(u => u.SecretKeyIdx != -1 && string.IsNullOrEmpty(u.UID))
            .ToList();
        ...
        var baseUri = serverStorage
            .ServerUri.Replace("wss://", "https://")
            .Replace("ws://", "http://");
        ...
        requestMessage.Headers.Authorization = new AuthenticationHeaderValue(
            "Bearer",
            serverStorage.OAuthToken
        );
        requestMessage.Content = requestContent;

        using var response = await _httpClient
            .SendAsync(requestMessage, token)
            .ConfigureAwait(false);
        ...
        if (secretKeyUidMapping == null)
        {
            return (
                false,
                false,
                $"Failed to parse the server response. Failed to convert all entries."
            );
        }
        ...
        {
            if (
                !secretKeyUidMapping.TryGetValue(entry.Key, out var assignedUid)
                || string.IsNullOrEmpty(assignedUid)
            )
            {
        ...
        sb.Append(
            $"Successfully converted {successfulConversions.Count} entries." + Environment.NewLine
        );
        if (failedConversions.Count > 0)
        {
            sb.Append(
                $"Failed to convert {failedConversions.Count} entries, assign those manually: "
            );
    ...
    private void DrawSettingsContent()
    {
        ...
        {
            ImGui.TextUnformatted(
                "Service " + _serverConfigurationManager.CurrentServer!.ServerName + ":"
            );
            ImGui.SameLine();
