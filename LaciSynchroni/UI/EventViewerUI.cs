...
internal class EventViewerUI : WindowMediatorSubscriberBase
{
    ...
    public EventViewerUI(
        ILogger<EventViewerUI> logger,
        ...
        : base(logger, mediator, "Event Viewer", performanceCollectorService)
    {
        ...
        SizeConstraints = new() { MinimumSize = new(600, 500), MaximumSize = new(1000, 2000) };
        _filteredEvents = RecreateFilter();
    }
            ...
            CurrentEvents
                .Where(f =>
                    (
                        ...
                        || (
                            f.EventSource.Contains(
                                _filterFreeText,
                                StringComparison.OrdinalIgnoreCase
                            )
                            || f.Character.Contains(
                                _filterFreeText,
                                StringComparison.OrdinalIgnoreCase
                            )
    ...
    {
        CurrentEvents = _eventAggregator
            .EventList.Value.OrderByDescending(f => f.EventTime)
            .ToList();
        ClearFilters();
    }
    ...
    protected override void DrawInternal()
    {
        ...
        if (_eventAggregator.NewEventsAvailable)
        {
            ...
            UiSharedService.ColorTextWrapped(
                "New events are available, press refresh to update",
        ...
        using var table = ImRaii.Table(
            "eventTable",
            6,
            ImGuiTableFlags.SizingFixedFit | ImGuiTableFlags.ScrollY | ImGuiTableFlags.RowBg,
