...
public class DrawGroupedGroupFolder : IDrawFolder
{
    ...
    public int OnlinePairs =>
        _groups
            .SelectMany(g => g.DrawPairs)
            .Where(g => g.Pair.IsOnline)
            .DistinctBy(g => g.Pair.UserData.UID)
            .Count();
    public int TotalPairs => _groups.Sum(g => g.TotalPairs);

    public DrawGroupedGroupFolder(
        IEnumerable<IDrawFolder> groups,
    ...
    public void Draw()
    {
        ...
        )
        {
            ...
            var icon = _tagHandler.IsTagOpen(_id)
            ...
            UiSharedService.AttachToolTip(
                OnlinePairs
                    + " online in all of your joined syncshells"
                    + Environment.NewLine
                    + TotalPairs
                    + " pairs combined in all of your joined syncshells"
            );
            ImGui.SameLine();
            ImGui.AlignTextToFramePadding();
