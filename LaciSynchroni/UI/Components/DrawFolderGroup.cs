...
public class DrawFolderGroup : DrawFolderBase
{
    ...
    public DrawFolderGroup(
        string id,
        GroupFullInfoDto groupFullInfoDto,
    ...
    protected override float DrawIcon()
    {
        ...
        _uiSharedService.IconText(
            _groupFullInfoDto.GroupPermissions.IsDisableInvites()
        ...
        if (_groupFullInfoDto.GroupPermissions.IsDisableInvites())
        {
            UiSharedService.AttachToolTip(
                "Syncshell " + _groupFullInfoDto.GroupAliasOrGID + " is closed for invites"
            );
        }
        ...
        using (
            ImRaii.PushStyle(
                ...
                {
                    X = ImGui.GetStyle().ItemSpacing.X / 2f,
                }
            )
        )
        {
        ...
        UiSharedService.AttachToolTip(
            OnlinePairs + " online" + Environment.NewLine + TotalPairs + " total"
        );
        ...
        if (IsOwner)
        {
            ...
            UiSharedService.AttachToolTip(
                "You are the owner of " + _groupFullInfoDto.GroupAliasOrGID
            );
        }
        else if (IsModerator)
        {
            ...
            UiSharedService.AttachToolTip(
                "You are a moderator in " + _groupFullInfoDto.GroupAliasOrGID
            );
        }
        else if (IsPinned)
        {
    ...
    protected override void DrawMenu(float menuWidth)
    {
        ...
        )
        {
            ...
            ImGui.SetClipboardText(
                UiSharedService.GetNotes(DrawPairs.Select(k => k.Pair).ToList())
            );
        }
        UiSharedService.AttachToolTip(
            "Copies all your notes for all users in this Syncshell to the clipboard."
                + Environment.NewLine
        ...
        UiSharedService.AttachToolTip(
            "Hold CTRL and click to leave this Syncshell"
                + (
                    !string.Equals(
                        _groupFullInfoDto.OwnerUID,
                        _apiController.UID,
        ...
        if (
            (
                _groupFullInfoDto.GroupPermissions.IsPreferDisableAnimations() != disableAnims
            ...
            && _uiSharedService.IconTextButton(
                ...
                "Align with suggested permissions",
                menuWidth,
                true
            )
        )
        {
            ...
            _ = _apiController.GroupChangeIndividualPermissionState(
                new(_groupFullInfoDto.Group, new(_apiController.UID), perm)
            );
            ImGui.CloseCurrentPopup();
        }
        ...
        if (
            _uiSharedService.IconTextButton(
                disableSounds ? FontAwesomeIcon.VolumeUp : FontAwesomeIcon.VolumeOff,
                disableSounds ? "Enable Sound Sync" : "Disable Sound Sync",
                menuWidth,
                true
            )
        )
        ...
        if (
            _uiSharedService.IconTextButton(
                disableAnims ? FontAwesomeIcon.Running : FontAwesomeIcon.Stop,
                disableAnims ? "Enable Animation Sync" : "Disable Animation Sync",
                menuWidth,
                true
            )
        )
        ...
        if (
            _uiSharedService.IconTextButton(
                disableVfx ? FontAwesomeIcon.Sun : FontAwesomeIcon.Circle,
                disableVfx ? "Enable VFX Sync" : "Disable VFX Sync",
                menuWidth,
                true
            )
        )
        ...
        if (IsModerator || IsOwner)
        {
            ...
            {
                ImGui.CloseCurrentPopup();
                _syncMediator.Publish(new OpenSyncshellAdminPanel(_groupFullInfoDto));
            }
        }
    }
    ...
    protected override float DrawRightSide(float currentRightSideX)
    {
        ...
        FontAwesomeIcon pauseIcon = _groupFullInfoDto.GroupUserPermissions.IsPaused()
        ...
        _uiSharedService.IconText(
            FontAwesomeIcon.UsersCog,
            (
                _groupFullInfoDto.GroupPermissions.IsPreferDisableAnimations()
                    != individualAnimDisabled
                || _groupFullInfoDto.GroupPermissions.IsPreferDisableSounds()
        ...
        if (ImGui.IsItemHovered())
        {
            ...
            _uiSharedService.BooleanToColoredIcon(
                !_groupFullInfoDto.GroupPermissions.IsPreferDisableVFX(),
                inline: false
            );
            ImGui.SameLine(40 * ImGuiHelpers.GlobalScale);
            ImGui.AlignTextToFramePadding();
        ...
        if (_uiSharedService.IconButton(pauseIcon))
        {
            ...
            _ = _apiController.GroupChangeIndividualPermissionState(
                new GroupPairUserPermissionDto(
                    _groupFullInfoDto.Group,
