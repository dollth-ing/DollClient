...
public class SelectTagForPairUi
{
    ...
    public void Draw()
    {
        ...
        if (ImGui.BeginPopup(popupName))
        {
            ...
            var childSize =
                new Vector2(0, childHeight > 100 ? 100 : childHeight) * ImGuiHelpers.GlobalScale;
            ...
            if (ImGui.BeginChild(name + "##listGroups", childSize))
            {
                ...
                {
                    using (ImRaii.PushId($"groups-pair-{_pair.UserData.UID}-{tag}"))
    ...
    {
        if (
            !_tagNameToAdd.IsNullOrWhitespace()
            && _tagNameToAdd
                is not (
                    TagHandler.CustomOfflineTag
                    or TagHandler.CustomOnlineTag
