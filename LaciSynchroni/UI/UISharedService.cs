...
public partial class UiSharedService : DisposableMediatorSubscriberBase
{
        ...
        : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<DelayedFrameworkUpdateMessage>(
            this,
            (_) =>
            {
        ...
        {
            e.OnPreBuild(tk =>
                tk.AddDalamudAssetFont(Dalamud.DalamudAsset.NotoSansJpMedium, new() { SizePx = 35 })
            );
        });
        GameFont = _pluginInterface.UiBuilder.FontAtlas.NewGameFontHandle(
            new(GameFontFamilyAndSize.Axis12)
        );
    ...
    public bool HasValidPenumbraModPath =>
        !(_ipcManager.Penumbra.ModDirectory ?? string.Empty).IsNullOrEmpty()
    ...
    public static bool CtrlPressed() =>
        (GetKeyState(0xA2) & 0x8000) != 0 || (GetKeyState(0xA3) & 0x8000) != 0;

    public static void DrawGrouped(
        Action imguiDrawAction,
        float rounding = 5f,
    ...
    public static void DrawGroupedCenteredColorText(
        string text,
        Vector4 color,
        float? maxWidth = null
    )
    ...
    )
    {
        ...
        using (ImRaii.PushColor(ImGuiCol.Text, outlineColor))
        {
            ...
            ImGui.SetCursorPos(
                original with
                {
                    X = original.X - thickness,
                    Y = original.Y - thickness,
                }
            );
            ImGui.TextUnformatted(text);
            ImGui.SetCursorPos(
    ...
    )
    {
        ...
        drawList.AddText(textPos with { X = textPos.X + thickness }, outlineColor, text);
        drawList.AddText(
            new Vector2(textPos.X - thickness, textPos.Y - thickness),
            outlineColor,
            text
        );
        drawList.AddText(
    ...
    public static Vector4 GetBoolColor(bool input) =>
        input ? ImGuiColors.ParsedGreen : ImGuiColors.DalamudRed;
    ...
    public static void SetScaledWindowSize(
        float width,
        float height,
    ...
    public static bool ShiftPressed() =>
        (GetKeyState(0xA1) & 0x8000) != 0 || (GetKeyState(0xA0) & 0x8000) != 0;
    ...
    public static Vector4 UploadColor((long, long) data) =>
        data.Item1 == 0 ? ImGuiColors.DalamudGrey
        : data.Item1 == data.Item2 ? ImGuiColors.ParsedGreen
        : ImGuiColors.DalamudYellow;

    public bool ApplyNotesFromClipboard(string notes, bool overwrite)
    {
        var splitNotes = notes
            .Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries)
            .ToList();
        var splitNotesStart = splitNotes.FirstOrDefault();
        var splitNotesEnd = splitNotes.LastOrDefault();
        if (
            !string.Equals(splitNotesStart, _notesStart, StringComparison.Ordinal)
            || !string.Equals(splitNotesEnd, _notesEnd, StringComparison.Ordinal)
        )
        {
            return false;
        }

        splitNotes.RemoveAll(n =>
            string.Equals(n, _notesStart, StringComparison.Ordinal)
            || string.Equals(n, _notesEnd, StringComparison.Ordinal)
        );
    ...
    {
        ColorTextWrapped(
            "Note: The storage folder should be somewhere close to root (i.e. C:\\LaciStorage) in a new empty folder. DO NOT point this to your game folder. DO NOT point this to your Penumbra folder.",
            ImGuiColors.DalamudYellow
        );
        var cacheDirectory = _configService.Current.CacheFolder;
        ImGui.InputText(
            "Storage Folder##cache",
            ref cacheDirectory,
            255,
        ...
        using (ImRaii.Disabled(_cacheMonitor.SyncWatcher != null))
        {
            if (IconButton(FontAwesomeIcon.Folder))
            {
                FileDialogManager.OpenFolderDialog(
                    "Pick Laci Synchroni Storage Folder",
                    (success, path) =>
                    {
                        if (!success)
                            return;
                        ...
                        _isPenumbraDirectory = string.Equals(
                            ...
                            _ipcManager.Penumbra.ModDirectory?.ToLowerInvariant(),
                        ...
                        foreach (var file in files)
                        {
                            ...
                            )
                            {
                                ...
                                Logger.LogWarning(
                                    "Found illegal file in {path}: {file}",
                                    path,
                                    file
                                );
                                break;
                        ...
                        if (dirs.Any())
                        {
                            _cacheDirectoryHasOtherFilesThanCache = true;
                            Logger.LogWarning(
                                "Found folders in {path} not belonging to Laci: {dirs}",
                                path,
                                string.Join(", ", dirs)
                            );
                        }
                        ...
                        )
                        {
                            ...
                            _configService.Save();
                            _cacheMonitor.StartSyncWatcher(path);
        ...
        {
            AttachToolTip(
                "Stop the Monitoring before changing the Storage folder. As long as monitoring is active, you cannot change the Storage folder location."
            );
        }
        ...
        {
            ColorTextWrapped(
                "Do not point the storage path directly to the Penumbra directory. If necessary, make a subfolder in it.",
        ...
        {
            ColorTextWrapped(
                "Do not point the storage path to a folder in OneDrive. Do not use OneDrive folders for any Mod related functionality.",
                ImGuiColors.DalamudRed
            );
        }
        ...
        {
            ColorTextWrapped(
                "The folder you selected does not exist or cannot be written to. Please provide a valid path.",
                ImGuiColors.DalamudRed
            );
        }
        ...
        {
            ColorTextWrapped(
                "Your selected directory has files or directories inside that are not Laci related. Use an empty directory or a previous Laci storage directory only.",
                ImGuiColors.DalamudRed
            );
        }
        ...
        {
            ColorTextWrapped(
                "Your selected directory contains illegal characters unreadable by FFXIV. "
                    + "Restrict yourself to latin letters (A-Z), underscores (_), dashes (-) and arabic numbers (0-9).",
                ImGuiColors.DalamudRed
            );
        }
        ...
        if (
            ImGui.SliderFloat("Maximum Storage Size in GiB", ref maxCacheSize, 1f, 200f, "%.2f GiB")
        )
        {
        ...
        DrawHelpText(
            "The storage is automatically governed by Laci. It will clear itself automatically once it reaches the set capacity by removing the oldest unused files. You typically do not need to clear it yourself."
        );
    }

    public T? DrawCombo<T>(
        string comboName,
    ...
    )
    {
        ...
        if (
            !_selectedComboItems.TryGetValue(comboName, out var selectedItem)
            && selectedItem == null
        )
        {
        ...
        if (
            ImGui.BeginCombo(
                comboName,
                selectedItem == null ? "Unset Value" : toName((T?)selectedItem)
            )
        )
        {
    ...
    public void DrawFileScanState()
    {
        ...
        if (_cacheMonitor.IsScanRunning)
        {
            ...
            AttachToolTip(
                "Note: it is possible to have more files in storage than scanned in, "
            ...
            ImGui.TextUnformatted(
                "Halted ("
                    + string.Join(
                        ", ",
                        _cacheMonitor
                            .HaltScanLocks.Where(f => f.Value > 0)
                            .Select(locker => locker.Key + ": " + locker.Value + " halt requests")
                    )
                    + ")"
            );
            ImGui.SameLine();
    ...
    public void DrawOAuth(ServerStorage selectedServer)
    {
        ...
        if (oauthToken == null)
        {
            if (_discordOAuthCheck == null)
            {
                if (
                    IconTextButton(
                        ...
                        "Check if Server supports Discord OAuth2"
                    )
                )
                {
                    _discordOAuthCheck = _serverConfigurationManager.CheckDiscordOAuth(
            ...
            else
            {
                ...
                {
                    ColorTextWrapped(
                        $"Checking OAuth2 compatibility with {selectedServer.ServerUri}",
                        ImGuiColors.DalamudYellow
                    );
                }
                else
                {
                    ...
                    {
                        ColorTextWrapped(
                            "Server is compatible with Discord OAuth2",
                    ...
                    {
                        ColorTextWrapped(
                            "Server is not compatible with Discord OAuth2",
                            ImGuiColors.DalamudRed
                        );
                    }
                }
            ...
            if (_discordOAuthCheck != null && _discordOAuthCheck.IsCompleted)
            {
                ...
                {
                    _discordOAuthGetCode = _serverConfigurationManager.GetDiscordOAuthToken(
                        _discordOAuthCheck.Result!,
                        selectedServer.ServerUri,
                        _discordOAuthGetCts.Token
                    );
                }
                else if (_discordOAuthGetCode != null && !_discordOAuthGetCode.IsCompleted)
                {
                    TextWrapped(
                        "A browser window has been opened, follow it to authenticate. Click the button below if you accidentally closed the window and need to restart the authentication."
                    );
                ...
                else if (_discordOAuthGetCode != null && _discordOAuthGetCode.IsCompleted)
                {
                    ...
                    {
                        ColorTextWrapped(
                            "Failed, please check /xllog for more information",
                            ImGuiColors.DalamudRed
                        );
                    }
                }
        ...
        if (oauthToken != null)
        {
            ...
            {
                ColorTextWrapped(
                    $"OAuth2 is enabled, linked to: Discord User {_serverConfigurationManager.GetDiscordUserFromToken(selectedServer)}",
                    ImGuiColors.HealerGreen
                );
                TextWrapped(
                    $"The OAuth2 token will expire on {tokenExpiry:yyyy-MM-dd} and automatically renew itself during login on or after {(tokenExpiry - TimeSpan.FromDays(7)):yyyy-MM-dd}."
                );
                using (ImRaii.Disabled(!CtrlPressed()))
                {
                    if (
                        IconTextButton(FontAwesomeIcon.Exclamation, "Renew OAuth2 token manually")
                        && CtrlPressed()
                    )
                    {
                        _ = _tokenProvider
                            .TryUpdateOAuth2LoginTokenAsync(selectedServer, forced: true)
                            .ContinueWith((_) => _apiController.CreateConnectionsAsync());
                    }
                }
                DrawHelpText(
                    "Hold CTRL to manually refresh your OAuth2 token. Normally you do not need to do this."
                );
                ImGuiHelpers.ScaledDummy(10f);

                if (
                    (_discordOAuthUIDs == null || _discordOAuthUIDs.IsCompleted)
                    && IconTextButton(FontAwesomeIcon.Question, "Check Discord Connection")
                )
                {
                    _discordOAuthUIDs = _serverConfigurationManager.GetUIDsWithDiscordToken(
                        selectedServer.ServerUri,
                        oauthToken
                    );
                }
                else if (_discordOAuthUIDs != null)
                {
                    ...
                    else
                    {
                        ...
                        var primaryUid =
                            _discordOAuthUIDs.Result?.FirstOrDefault()
                            ?? new KeyValuePair<string, string>(string.Empty, string.Empty);
                        var vanity = string.IsNullOrEmpty(primaryUid.Value)
                            ? "-"
                            : primaryUid.Value;
                        if (foundUids > 0)
                        {
                            ColorTextWrapped(
                                $"Found {foundUids} associated UIDs on the server, Primary UID: {primaryUid.Key} (Vanity UID: {vanity})",
                                ImGuiColors.HealerGreen
                            );
                        }
                        else
                        {
                            ColorTextWrapped(
                                $"Found no UIDs associated to this linked OAuth2 account",
                                ImGuiColors.DalamudRed
                            );
                        }
                    }
            ...
            {
                ColorTextWrapped(
                    "The OAuth2 token is stale and expired. Please renew the OAuth2 connection.",
                    ImGuiColors.DalamudRed
                );
                if (IconTextButton(FontAwesomeIcon.Exclamation, "Renew OAuth2 connection"))
                {
                    ...
                    _ = _serverConfigurationManager
                        .CheckDiscordOAuth(selectedServer.ServerUri)
                        .ContinueWith(
                            async (urlTask) =>
                            {
                                var url = await urlTask.ConfigureAwait(false);
                                var token = await _serverConfigurationManager
    ...
    public bool DrawOtherPluginState()
    {
        ...
        AttachToolTip(
            $"Penumbra is "
                + (_penumbraExists ? "available and up to date." : "unavailable or not up to date.")
        );
        ...
        AttachToolTip(
            $"SimpleHeels is "
                + (_heelsExists ? "available and up to date." : "unavailable or not up to date.")
        );
        ...
        AttachToolTip(
            $"Moodles is "
                + (_moodlesExists ? "available and up to date." : "unavailable or not up to date.")
        );
        ...
        AttachToolTip(
            $"PetNicknames is "
                + (_petNamesExists ? "available and up to date." : "unavailable or not up to date.")
        );
        ...
        AttachToolTip(
            $"Brio is "
                + (_brioExists ? "available and up to date." : "unavailable or not up to date.")
        );
        ...
        if (!_penumbraExists || !_glamourerExists)
        {
            ImGui.TextColored(
                ImGuiColors.DalamudRed,
                "You need to install both Penumbra and Glamourer and keep them up to date to use Laci Synchroni."
            );
            return false;
        }
    ...
    public int DrawServiceSelection(
        bool selectOnChange = false,
        bool showConnect = true,
        bool hideServiceCreation = false,
        bool hideSelectLabel = false,
        bool showMinifiedConnect = false
    )
    {
        ...
        {
            _serverSelectionIndex = Array.IndexOf(
                _serverConfigurationManager.GetServerApiUrls(),
                _serverConfigurationManager.CurrentApiUrl
            );
        }
        ...
        var buttonSize = (
            !showConnect ? new Vector2(0, 0)
            : showMinifiedConnect ? GetIconButtonSize(FontAwesomeIcon.Link)
            : new Vector2(GetIconTextButtonSize(FontAwesomeIcon.Link, "Reconnect"), 0)
        ).X;
        buttonSize += hideSelectLabel ? 0 : ImGui.CalcTextSize("Select Service").X;

        ImGui.SetNextItemWidth(
            ImGui.GetContentRegionAvail().X - buttonSize - ImGui.GetStyle().ItemSpacing.X
        );
        if (
            ImGui.BeginCombo(
                hideSelectLabel ? "" : "Select Service",
                comboEntries[_serverSelectionIndex]
            )
        )
        {
            for (int i = 0; i < comboEntries.Length; i++)
        ...
        if (showConnect)
        {
            ...
            bool isConnectingOrConnected =
                isCurrentServer
                && _apiController.ServerState
                    is ServerState.Connected
                        or ServerState.Connecting
                        or ServerState.Reconnecting;
            ImGui.SameLine();
            if (showMinifiedConnect)
            {
                var color = GetBoolColor(!isConnectingOrConnected);
                var connectedIcon = isConnectingOrConnected
                ...
                using (ImRaii.PushColor(ImGuiCol.Text, color))
                {
                    if (IconButton(connectedIcon))
                    {
                        if (
                            ...
                            && _serverConfigurationManager.CurrentServer != null
        ...
        if (!hideServiceCreation && ImGui.TreeNode("Add Custom Service"))
        {
            ...
            if (
                IconTextButton(FontAwesomeIcon.Plus, "Add Custom Service")
                && !string.IsNullOrEmpty(_customServerUri)
                && !string.IsNullOrEmpty(_customServerName)
            )
            {
                _serverConfigurationManager.AddServer(
                    new ServerStorage()
                    {
    ...
    public void DrawUIDComboForAuthentication(
        int indexOffset,
        Authentication item,
        string serverUri,
        ILogger? logger = null
    )
    {
        ...
        {
            var aliasPairs =
                _discordOAuthUIDs?.Result?.Select(t => new UIDAliasPair(t.Key, t.Value)).ToList()
                ?? [new UIDAliasPair(item.UID ?? null, null)];
            var uidComboName =
                "UID###"
                + item.CharacterName
                + item.WorldId
            ...
            DrawCombo(
                ...
                aliasPairs.Find(f => string.Equals(f.UID, item.UID, StringComparison.Ordinal))
                    ?? default
            );
        }
        ...
        {
            AttachToolTip(
                "Use the button above to update your UIDs from the service before you can assign UIDs to characters."
            );
        }
    }
    ...
    public void DrawUnlinkOAuthButton(ServerStorage selectedServer)
    {
        ...
        {
            if (
                IconTextButton(FontAwesomeIcon.Trash, "Unlink OAuth2 Connection")
    ...
    public void DrawUpdateOAuthUIDsButton(ServerStorage selectedServer)
    {
        ...
        if (
            !string.IsNullOrEmpty(selectedServer.OAuthToken)
            && !_oauthTokenExpiry.TryGetValue(selectedServer.OAuthToken, out tokenExpiry)
        )
        {
            try
            {
        ...
        {
            ColorTextWrapped(
                "You have no OAuth token or the OAuth token is expired. Please use the Service Configuration to link your OAuth2 account or refresh the token.",
                ImGuiColors.DalamudRed
            );
        }
    }
    ...
    public void IconText(FontAwesomeIcon icon, Vector4? color = null)
    {
        IconText(
            icon,
            color == null ? ImGui.GetColorU32(ImGuiCol.Text) : ImGui.GetColorU32(color.Value)
        );
    }

    public bool IconTextButton(
    ...
    private bool IconTextButtonInternal(
        FontAwesomeIcon icon,
        string text,
    ...
    public sealed record IconScaleData(
        Vector2 IconSize,
        Vector2 NormalizedIconScale,
        float OffsetX,
        float IconScaling
    );
