...
public class IdDisplayHandler
{
    ...
    public void DrawGroupText(
        string id,
        GroupFullInfoDto group,
        float textPosX,
        Func<float> editBoxWidth
    )
    {
    ...
    public void DrawPairText(string id, Pair pair, float textPosX, Func<float> editBoxWidth)
    {
        ...
        if (!string.Equals(_editEntry, pair.UserData.UID, StringComparison.Ordinal))
        {
            ...
            if (ImGui.IsItemHovered())
            {
                ...
                {
                    _popupTime = DateTime.UtcNow.AddSeconds(
                        _syncConfigService.Current.ProfileDelay
                    );
                }
                ...
                {
                    ImGui.SetTooltip(
                        "Left click to switch between UID display and nick"
                            + Environment.NewLine
                            + "Right click to change nick for "
    ...
    public (bool isUid, string text) GetPlayerText(Pair pair)
    {
        ...
        if (
            _syncConfigService.Current.ShowCharacterNameInsteadOfNotesForVisible
            && pair.IsVisible
            && !showUidInsteadOfName
        )
        {
