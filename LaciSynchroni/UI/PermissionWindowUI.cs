...
public class PermissionWindowUI : WindowMediatorSubscriberBase
{
    ...
    public PermissionWindowUI(
        ILogger<PermissionWindowUI> logger,
        Pair pair,
        ...
        )
    {
        ...
        SizeConstraints = new() { MinimumSize = new(450, 100), MaximumSize = new(450, 500) };
        IsOpen = true;
    }
    ...
    protected override void DrawInternal()
    {
        ...
        _uiSharedService.DrawHelpText(
            "Preferred Permissions, when enabled, will exclude this user from any permission changes on any syncshells you share with this user."
        );
        ...
        _uiSharedService.DrawHelpText(
            "Pausing will completely cease any sync with this user."
        ...
        using (ImRaii.PushIndent(indentSize, false))
        {
            ...
            ImGui.Text(
                Pair.UserData.AliasOrUID
                    + " has "
                    + (!otherIsPaused ? "not " : string.Empty)
                    + "paused you"
            );
        }
        ...
        _uiSharedService.DrawHelpText(
            "Disabling sounds will remove all sounds synced with this user on both sides."
                + UiSharedService.TooltipSeparator
        ...
        using (ImRaii.PushIndent(indentSize, false))
        {
            ...
            ImGui.Text(
                Pair.UserData.AliasOrUID
                    + " has "
                    + (!otherDisableSounds ? "not " : string.Empty)
                    + "disabled sound sync with you"
            );
        }
        ...
        _uiSharedService.DrawHelpText(
            "Disabling sounds will remove all animations synced with this user on both sides."
                + UiSharedService.TooltipSeparator
        ...
        using (ImRaii.PushIndent(indentSize, false))
        {
            ...
            ImGui.Text(
                Pair.UserData.AliasOrUID
                    + " has "
                    + (!otherDisableAnimations ? "not " : string.Empty)
                    + "disabled animation sync with you"
            );
        }
        ...
        _uiSharedService.DrawHelpText(
            "Disabling sounds will remove all VFX synced with this user on both sides."
                + UiSharedService.TooltipSeparator
        ...
        using (ImRaii.PushIndent(indentSize, false))
        {
            ...
            ImGui.Text(
                Pair.UserData.AliasOrUID
                    + " has "
                    + (!otherDisableVFX ? "not " : string.Empty)
                    + "disabled VFX sync with you"
            );
        }
        ...
        using (ImRaii.Disabled(!hasChanges))
            ...
            {
                _ = _apiController.SetBulkPermissions(
                    new(
                        new(StringComparer.Ordinal) { { Pair.UserData.UID, _ownPermissions } },
        ...
        var rightSideButtons =
            _uiSharedService.GetIconTextButtonSize(Dalamud.Interface.FontAwesomeIcon.Undo, "Revert")
            + _uiSharedService.GetIconTextButtonSize(
                Dalamud.Interface.FontAwesomeIcon.ArrowsSpin,
                "Reset to Default"
            );
        var availableWidth =
        ...
        )
        {
            ...
            _ownPermissions.SetSticky(
                Pair.IsDirectlyPaired || defaultPermissions.IndividualIsSticky
            );
            _ownPermissions.SetPaused(false);
            _ownPermissions.SetDisableVFX(
                Pair.IsDirectlyPaired
                    ? defaultPermissions.DisableIndividualVFX
                    : defaultPermissions.DisableGroupVFX
            );
            _ownPermissions.SetDisableSounds(
                Pair.IsDirectlyPaired
                    ? defaultPermissions.DisableIndividualSounds
                    : defaultPermissions.DisableGroupSounds
            );
            _ownPermissions.SetDisableAnimations(
                Pair.IsDirectlyPaired
                    ? defaultPermissions.DisableIndividualAnimations
                    : defaultPermissions.DisableGroupAnimations
            );
            _ = _apiController.SetBulkPermissions(
                new(
                    new(StringComparer.Ordinal) { { Pair.UserData.UID, _ownPermissions } },
        ...
        UiSharedService.AttachToolTip(
            "This will set all permissions to your defined default permissions in the Laci Settings"
        );

        var ySize =
            ImGui.GetCursorPosY()
            + style.FramePadding.Y * ImGuiHelpers.GlobalScale
