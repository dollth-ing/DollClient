...
internal sealed partial class CharaDataHubUi
{
    ...
    private void GposeMetaInfoAction(
        Action<CharaDataMetaInfoExtendedDto?> gposeActionDraw,
        string actionDescription,
        CharaDataMetaInfoExtendedDto? dto,
    ...
    private void GposePoseAction(
        Action poseActionDraw,
    ...
    private void UpdateFilteredFavorites()
    {
        _ = Task.Run(async () =>
        {
            ...
            Dictionary<
                string,
                (CharaDataFavorite, CharaDataMetaInfoExtendedDto?, bool)
            > newFiltered = [];
            foreach (var favorite in _configService.Current.FavoriteCodes)
            {
                ...
                bool addFavorite =
                    ...
                    && (
                        ...
                        || (
                            favorite.Value.CustomDescription.Contains(
                                _filterDescription,
                                StringComparison.OrdinalIgnoreCase
                            )
                            || (
                                metaInfo != null
                                && metaInfo!.Description.Contains(
                                    _filterDescription,
                                    StringComparison.OrdinalIgnoreCase
                                )
                            )
                    ...
                    && (!_filterPoseOnly || (metaInfo != null && metaInfo!.HasPoses))
                    && (!_filterWorldOnly || (metaInfo != null && metaInfo!.HasWorldData));
                if (addFavorite)
                {
    ...
    private void UpdateFilteredItems()
    {
        if (_charaDataManager.GetSharedWithYouTask == null)
        {
            _filteredDict = _charaDataManager
                .SharedWithYouData.SelectMany(k => k.Value)
                .Where(k =>
                    (!_sharedWithYouDownloadableFilter || k.CanBeDownloaded)
                    && (
                        string.IsNullOrEmpty(_sharedWithYouDescriptionFilter)
                        || k.Description.Contains(
                            _sharedWithYouDescriptionFilter,
                            StringComparison.OrdinalIgnoreCase
                        )
                    )
                )
